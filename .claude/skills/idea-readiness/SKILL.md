---
name: idea-readiness
description: Readiness gate for idea generation. Use before /idea-generate, or whenever plans/context may be stale. Audit business-plan freshness, outcome clarity, code-to-plan traceability, tooling/data availability, and market-research freshness; fail closed on critical gaps; ask targeted questions to collect missing information and update source docs before allowing idea generation.
---

# Ideas Readiness

Run a fail-closed readiness audit before generating ideas. Do not run `/idea-generate` until critical readiness gaps are closed.

## Invocation

```bash
/idea-readiness
/idea-readiness --scope=BRIK
/idea-readiness --max-age-days=21
/idea-readiness --research-max-age-days=30
```

Notes:
- `--scope=<BIZ>` limits checks to one business; default is all active businesses.
- `--max-age-days` controls plan/profile staleness threshold; default is `21`.
- `--research-max-age-days` controls market research freshness threshold; default is `30`.
...
- Audit market research freshness per business.
...
Required for market-research freshness gate:
- Latest pointer: `docs/business-os/market-research/<BIZ>/latest.user.md`
- Prompt template for missing/stale cases:
...
| `RG-07 Market Research Freshness` | Current business market intelligence exists and is fresh | Missing `latest.user.md`, status not active, or market-research last-reviewed older than research-max-age-days |
...
4. Resolve `research-max-age-days` (default `30`).

### Stage 2: Gate Audit Per Business

For each business:
1. Run `RG-01` through `RG-07`.
...
3. Any unmapped path or unmapped decision impact is an `RG-04 block` until resolved.
...
Write:
- `docs/business-os/readiness/<YYYY-MM-DD>-idea-readiness.user.md`
- `docs/business-os/readiness/<YYYY-MM-DD>-missing-context-register.user.md`

Render HTML companions (required):
- Run:
  - `pnpm docs:render-user-html -- docs/business-os/readiness/<YYYY-MM-DD>-idea-readiness.user.md docs/business-os/readiness/<YYYY-MM-DD>-missing-context-register.user.md`
- Result:
  - `docs/business-os/readiness/<YYYY-MM-DD>-idea-readiness.user.html`
  - `docs/business-os/readiness/<YYYY-MM-DD>-missing-context-register.user.html`
...
### Missing/stale market research (`RG-07`)

1. Confirm business input packet: idea, products, initial ICP, channels, budget, stock timeline.
2. Run Deep Research using template:
   - `docs/business-os/market-research/_templates/deep-research-market-intelligence-prompt.md`
3. Save output at:
   - `docs/business-os/market-research/<BIZ>/<YYYY-MM-DD>-market-intelligence.user.md`
4. Update pointer:
   - `docs/business-os/market-research/<BIZ>/latest.user.md` with `Status: Active`

## Prompt Presentation Rule (RG-07)

When `RG-07` is a block, readiness output must include:
1. The reason market research is blocked (missing/stale/inactive).
2. The exact template path.
3. A fully filled copy-paste prompt block for Deep Research.

## Readiness Report Contract

`<YYYY-MM-DD>-idea-readiness.user.md` must include:
1. Run status: `ready` or `blocked`
2. Scope and staleness thresholds used
3. Gate table per business (`RG-01..RG-07`)
4. Hard blockers with evidence
5. Warnings with evidence
6. Questions asked and answers received
7. Exact docs updated
8. Go/No-go statement for `/idea-generate`
9. For `RG-07` blockers: filled Deep Research prompt blocks
...
> "Readiness status: READY. `/idea-generate` may run with listed warnings acknowledged."
...
6. Ignores missing/stale market research for market-facing businesses.

## Integration Contract with Ideas Generation

Before running `/idea-generate`:
1. Run `/idea-readiness`.
2. If status is `BLOCKED`, stop and close gaps first.
3. If status is `READY`, proceed to `/idea-generate`.
