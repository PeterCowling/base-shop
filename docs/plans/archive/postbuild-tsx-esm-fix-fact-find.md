...
- **Blocking items:** None
- **Recommended next step:** Proceed to `/lp-do-plan` with Solution A as the primary approach (scripts-specific tsconfig + `tsx --tsconfig ...` for postbuild), with Solution B only if we explicitly decide that `apps/brikette/tsconfig.json` can be safely changed without harming editor/typecheck expectations.
