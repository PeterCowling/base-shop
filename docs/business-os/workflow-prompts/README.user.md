...
| Measure (S1B) | `pre-website-measurement-bootstrap-prompt.md` | pre-website | Comprehensive Phase 0 access bundle (P0-01..P0-12), Phase 1 agent config (GA4/GSC/DNS/GitHub/Code), Phase 2 staging verification. 7 Derived Policies. Front-loads all credential creation so agents run Phase 1 unattended. |
| Measure / Results (S1B / S2A) — quality audit | `measurement-quality-audit-prompt.md` | website-live | For existing live sites entering the loop. Idempotent gap audit against 7 Derived Policies. Does not provision; only reports current vs required state. Use when GA4+Cloudflare are already set up. |
| Results (S2A) — data request | `historical-data-request-prompt.md` | website-live | |
| Results (S2A) — baseline | `existing-business-historical-baseline-prompt.md` | website-live | |
...
| Do (DO) | no-prompt-required | all | Skill-driven: `/lp-do-fact-find`, `/lp-do-plan`, `/lp-do-build` |
