...
- [x] TASK-02: Apply zone settings (ssl=strict, min_tls=1.2, always_use_https=on)
- [x] TASK-03: Checkpoint — verify zone health post-settings change
...
| TASK-02 | IMPLEMENT | Apply zone settings (ssl/tls/https) | 85% | S | Complete (2026-03-06) | TASK-01 | TASK-03 |
| TASK-03 | CHECKPOINT | Verify zone health post-settings | 95% | S | Complete (2026-03-06) | TASK-02 | TASK-04 |
...
- **Build evidence (2026-03-06):**
  - Dry-run confirmed: ssl=flexible, min_tls_version=1.0, always_use_https=on (already set).
  - Live run: ssl=strict ✓ (confirmed), min_tls_version=1.2 ✓ (confirmed). always_use_https was already on; PATCH failed 403 (token lacked that specific sub-permission) but value already correct — dry-run after confirms all three at target.
  - Zone ID: 25b082bdadbb0541c0f34c2bf0d21cc4. Managed cert status: active (type: universal).
...
- **Build evidence (2026-03-06):**
  - TC-01: dry-run post-apply confirms ssl=strict, min_tls_version=1.2, always_use_https=on — all at target.
  - TC-02: `curl -sv https://guests.hostel-positano.com` → TLSv1.3, HTTP/2 200, cert CN=guests.hostel-positano.com, issuer Google Trust Services ✓
  - TC-02: `curl -I http://guests.hostel-positano.com` → 301 Moved Permanently → https ✓
  - TC-02: `curl -sv https://www.hostel-positano.com` → TLSv1.3, HTTP/2 301 (brikette redirect loop, not a 5xx — brikette unaffected) ✓
  - TC-03: `curl --tls-max 1.1 https://guests.hostel-positano.com` → SSL error: tlsv1 alert protocol version (TLS 1.0/1.1 rejected) ✓
  - No 525/526 errors observed. Checkpoint passed — HSTS deployment (TASK-04) cleared to deploy.
...
- [x] `hostel-positano.com` zone: ssl=strict, min_tls_version=1.2, always_use_https=on (confirmed via script read-back and curl smoke tests)
- [x] `apps/prime/public/_headers` exists and all six security headers present (deploys with next merge to main)
- [x] `curl --tls-max 1.1 https://guests.hostel-positano.com` → SSL error: tlsv1 alert protocol version (TLS 1.0/1.1 rejected) ✓
- [x] `curl -I http://guests.hostel-positano.com` → 301 redirect to HTTPS ✓
- [x] CI pipeline for `prime` on `main` extended with `healthcheck-custom-domain` job (deploys with next merge)
- [x] `docs/plans/prime-edge-tls-hardening/adr-route-boundary.md` exists documenting route boundary
- [ ] Brikette CI healthcheck — to be confirmed on next CI run (zone change applied; brikette TLS smoke test passed manually)
