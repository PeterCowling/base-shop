...
Confidence-Method: All tasks are doc edits with verified evidence from lp-do-fact-find. No runtime code changes.
...
## Fact-Find Reference
- Related brief: `docs/plans/idea-generate-process-hardening-lp-do-fact-find.md`
- Findings addressed by this plan: F1, F3, F4, F5, F6, F7.
- Finding deferred: F2 (non-idempotent POST creates).

## Existing System Notes
- Orchestrator: `.claude/skills/idea-generate/SKILL.md`
- Cabinet contracts:
...
- **Deliverable:** Updated `.claude/skills/idea-generate/SKILL.md`.
- **Affects:** `.claude/skills/idea-generate/SKILL.md`
- **Depends on:** -
- **Blocks:** TASK-03
- **Confidence:** HIGH — all edits are localized, evidence-backed, and verified in lp-do-fact-find.
- **Specific changes:**
...
- **Validation:**
  - `rg -n "Technical cabinet deferred|CS-13" .claude/skills/idea-generate/SKILL.md` → zero matches.
  - `rg -n "Originator-Lens: engineering" .claude/skills/idea-generate/SKILL.md` → zero matches.
  - `rg -n "Originator-Lens: code-review" .claude/skills/idea-generate/SKILL.md` → matches at L620.
...
- **Depends on:** -
- **Blocks:** TASK-03
- **Confidence:** HIGH — contract deltas are explicit and verified in lp-do-fact-find.
- **Specific changes:**
...
- **Validation:**
  - `rg -n "gate-unresolved" .claude/skills/idea-generate/SKILL.md` → matches in hold-tag section.
  - `rg -n "gate-unresolved" .claude/skills/_shared/cabinet/data-gap-lifecycle.md` → matches (existing, confirms alignment).
  - `rg -n "stage7b" .claude/skills/idea-generate/SKILL.md` → matches in both invocation and contract sections.
  - `rg -n "location" .claude/skills/idea-generate/SKILL.md` → resurfacing section specifies query behavior.
- **Notes:** Fact-find F3, F4, F6.

### TASK-03: Add contract drift-check script
- **Deliverable:** `scripts/check-idea-generate-contracts.sh` — a shell script that greps for the specific invariants fixed in TASK-01 and TASK-02.
- **Affects:** `scripts/check-idea-generate-contracts.sh` (new file)
...
The lp-do-fact-find's F2 finding (non-idempotent POST creates + retry guidance = duplicate entity risk) is real but is a fundamentally different class of work from the doc fixes above. It requires:
...
This deserves its own lp-do-fact-find and plan. The current retry guidance in the orchestrator (3 attempts with backoff) is a tolerable risk given that:
...
**Recommended next step:** `/lp-do-fact-find api-idempotency-agent-endpoints` when this becomes a priority.
