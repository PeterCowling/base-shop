...
- Stage 7 must resolve `{{CARD_ID}}` using this map before any fact-find stage-doc upsert.

**Stance sensitivity:** INVARIANT (card creation mechanics do not change by stance)

### Stage 7: Fact-Find Seeding (Global Top-K)

Use manifest `factfind_upsert` operations to seed fact-find stage docs for selected cards.
...
K=3 (default, max 3 fact-find stage docs per sweep)
...
- Budget: max 1 existing `P1|P2` card without a fact-find stage doc per sweep.
...
- [ ] **Card ID resolution honored:** fact-find stage-doc writes occur only after `{{CARD_ID}}` resolution via card-id-map
...
8. **Created fact-find stage docs for low-priority cards.** Only global top K (default K=3) get fact-find stage docs.
...
| Card ID unresolved for fact-find stage-doc upsert | Record failed `factfind_upsert` operation, skip write, keep run partial with reconciliation note. |
...
| Top-priority card with fact-find stage doc | `/lp-do-fact-find <card-id>` | Deep-dive investigation |
| Card without fact-find stage doc | `/lp-do-fact-find <card-id>` | Manual /lp-do-fact-find if desired |
| Card ready for planning | `/lp-do-plan <slug>` | After /lp-do-fact-find completes |
...
**Dry run:**
> "Cabinet sweep dry-run complete. Report: `docs/business-os/sweeps/<YYYY-MM-DD>-sweep.user.md`. Progress log: `docs/business-os/sweeps/<YYYY-MM-DD>-progress.user.md`. Persistence mode: dry-run (no ideas/cards/stage-doc writes). {X} presentable ideas, {Y} would-create cards, {K} would-seed fact-find stage docs."
...
- **Max 3 fact-find stage docs per sweep.** Prevents flooding the fact-finding queue.
...
- Top K cards receive fact-find stage docs for immediate investigation
...
- **v3.6** (2026-02-11): Added explicit API preflight mode contract and runner hardening: introduced `--allow-api-degraded` (dry-run only), strict live-mode preflight requirement, `API-Preflight-Mode` frontmatter/reporting, degraded fallback/error-handling rules, and recommended env-loading wrapper `scripts/run-idea-generate.sh` with initiator-agent-preserving execution (codex/claude auto-select).
