...
exports.createInventoryItemDelegate = createInventoryItemDelegate;
function createInventoryItemDelegate() {
    const inventoryItems = [];
    return {
        findMany: async ({ where: { shopId } }) => inventoryItems.filter((i) => i.shopId === shopId),
        ...
        findUnique: async ({ where: { shopId_sku_variantKey } }) => {
            ...
            return (inventoryItems.find((i) => i.shopId === shopId && i.sku === sku && i.variantKey === variantKey) || null);
        },
        delete: async ({ where: { shopId_sku_variantKey } }) => {
            const { shopId, sku, variantKey } = shopId_sku_variantKey;
            const idx = inventoryItems.findIndex((i) => i.shopId === shopId && i.sku === sku && i.variantKey === variantKey);
            if (idx < 0)
                throw new Error("InventoryItem not found"); // i18n-exempt -- DS-0001 Internal error message, not UI copy
