...
Overall-delivery-confidence: 86%
...
Fact-Find-Ref: docs/plans/prime-guest-portal-gap-wf-fact-find.md
...
| TASK-51 | Staff auth replacement (PinAuthProvider) | IMPLEMENT | M | 82% | TASK-54, TASK-55, TASK-56 | Complete (2026-02-10) |
| TASK-52 | Campaign/trigger messaging orchestration | IMPLEMENT | L | 82% | TASK-57, TASK-58, TASK-59 | Complete (2026-02-10) |
| TASK-53 | PWA offline essentials | IMPLEMENT | M | 80% | TASK-60, TASK-61 | Complete (2026-02-10) |
| TASK-54 | Staff auth architecture decision memo (custom claims vs PIN token) | INVESTIGATE | S | 85% | — | Complete (2026-02-10) |
| TASK-55 | Firebase Auth + claims bootstrap spike for Prime client | SPIKE | M | 82% | TASK-54 | Complete (2026-02-10) |
| TASK-56 | Function-side staff token gate contract spike | SPIKE | M | 80% | TASK-54, TASK-55 | Complete (2026-02-10) |
| TASK-57 | Messaging processor architecture + idempotency contract | INVESTIGATE | S | 84% | — | Complete (2026-02-10) |
| TASK-58 | Single-trigger queue processor spike (`arrival.48hours`) | SPIKE | M | 82% | TASK-57 | Complete (2026-02-10) |
| TASK-59 | Email provider staging wiring + dispatch smoke contract | SPIKE | M | 80% | TASK-57, TASK-58 | Complete (2026-02-10) |
| TASK-60 | Minimal service worker + manifest integration spike | SPIKE | M | 82% | TASK-07 | Complete (2026-02-10) |
| TASK-61 | Offline arrival e2e + cache invalidation contract spike | SPIKE | M | 80% | TASK-60, TASK-41 | Complete (2026-02-10) |

## Re-plan Iteration 2 (2026-02-07)

### Scope Selected

This `/wf-replan` pass targeted the highest-risk low-confidence tasks that either:
...
This `/wf-replan` pass covered the remaining active sub-80 tasks identified in Iteration 2:
...
This `/wf-replan` pass focused on the previously sub-80 tasks in the TASK-20..TASK-30 implementation window:
...
This `/wf-replan` pass targeted the remaining low-confidence social/deferred tracks:
...
This `/wf-replan` pass targeted the remaining active work after `TASK-40`:
...
This `/wf-replan` pass targeted all remaining sub-80% active tasks:
...
## Re-plan Iteration 11 (2026-02-10)

### Scope Selected

This `/wf-replan` pass targeted the remaining deferred low-confidence tasks:

- `TASK-51`
- `TASK-52`
- `TASK-53`

### Evidence Snapshot (2026-02-10)

- `TASK-51` staff auth:
  - `PinAuthProvider` is still a client stub (`pin.length >= 4` grants staff role + localStorage persistence): `apps/prime/src/contexts/messaging/PinAuthProvider.tsx:29`.
  - Firebase client bootstrap imports `firebase/app`, `firebase/database`, `firebase/storage`, but not `firebase/auth`: `apps/prime/src/services/firebase.ts:4`.
  - RTDB rules for sensitive nodes require authenticated role context via `auth.uid` + role nodes: `apps/prime/database.rules.json:197`.
  - Function guard still uses environment gate token / Cloudflare Access headers, not per-user staff token validation: `apps/prime/functions/lib/staff-owner-gate.ts:17`.
- `TASK-52` messaging orchestration:
  - Queue write seam exists (`messagingQueue/{eventId}`): `apps/prime/src/lib/messaging/useMessagingQueue.ts:51`.
  - Event schemas and queue record model exist: `apps/prime/src/lib/messaging/triggers.ts:121`.
  - No queue processor endpoint exists in `apps/prime/functions/api/` (only `bag-drop-request.ts`, `check-in-code.ts`, `check-in-lookup.ts`, `extension-request.ts`, `find-booking.ts`, `guest-booking.ts`, `guest-session.ts`).
  - Current outbound email path is webhook/noop dispatcher, not queue-worker dispatch: `apps/prime/functions/lib/email-dispatch.ts:17`.
  - `wrangler.toml` has Firebase URL var only; no dedicated provider dispatch vars are defined yet: `apps/prime/wrangler.toml:11`.
- `TASK-53` PWA:
  - SW registration expects `/sw.js`: `apps/prime/src/lib/pwa/registerSW.ts:24`.
  - `ServiceWorkerProvider` exists but is not mounted in root layout: `apps/prime/src/components/pwa/ServiceWorkerProvider.tsx:27`, `apps/prime/src/app/layout.tsx:20`.
  - `apps/prime/public/` currently has `_redirects`, `images/`, `locales/` only (no `sw.js`, no manifest file).

### Decomposition Applied (Formal Precursor Tasks)

- `TASK-51` now depends on `TASK-54`, `TASK-55`, `TASK-56`.
- `TASK-52` now depends on `TASK-57`, `TASK-58`, `TASK-59`.
- `TASK-53` now depends on `TASK-60`, `TASK-61`.

Each precursor is first-class and build-eligible (`>=80%`) with explicit acceptance/test contracts in Phase 3 below.

### Confidence Deltas Applied

- `TASK-51`: `74% -> 74%` (no uplift; blocking uncertainty remains until precursor chain completes)
- `TASK-52`: `70% -> 70%` (no uplift; queue processor + provider wiring are still unproven)
- `TASK-53`: `68% -> 68%` (no uplift; SW/manifest/offline e2e remain unproven)

Conditional confidence is now explicit:

- `TASK-51`: `74% (-> 82% conditional on TASK-54, TASK-55, TASK-56)`
- `TASK-52`: `70% (-> 82% conditional on TASK-57, TASK-58, TASK-59)`
- `TASK-53`: `68% (-> 80% conditional on TASK-60, TASK-61)`

### Sequencing Applied (Decomposition Closure)

`/wf-sequence` equivalent ordering was applied after decomposition (no renumber required because new tasks append after `TASK-53`):

1. Wave 1: `TASK-54`, `TASK-57`, `TASK-60`
2. Wave 2: `TASK-55`, `TASK-58`, `TASK-61`
3. Wave 3: `TASK-56`, `TASK-59`
4. Wave 4: `TASK-51`, `TASK-52`, `TASK-53` (only after precursor completion and follow-up `/wf-replan` promotion)
...
- Wave 4 promotion check completed via `/wf-replan` and `/wf-sequence` verification.
...
Promotion reassessment for Wave 4 target tasks using precursor evidence:
...
- `TASK-51`: `74% -> 82%` (promoted; precursor chain complete with E3/E2 evidence)
- `TASK-52`: `70% -> 82%` (promoted; precursor chain complete with E3/E2 evidence)
- `TASK-53`: `68% -> 80%` (promoted; precursor chain complete with E3/E2 evidence)

### Sequence-Plan Verification

- Re-ran sequence checks after Wave 4 promotion.
- No topology or dependency changes were required; ordering remains:
  1. Wave 1: `TASK-54`, `TASK-57`, `TASK-60`
  2. Wave 2: `TASK-55`, `TASK-58`, `TASK-61`
  3. Wave 3: `TASK-56`, `TASK-59`
  4. Wave 4: `TASK-51`, `TASK-52`, `TASK-53`
...
- **Implementation notes:**
  - Added Reception Prime queue read path via `apps/reception/src/hooks/data/usePrimeRequestsData.ts` with schema contract in `apps/reception/src/schemas/primeRequestsSchema.ts`.
  - Added resolution mutation orchestrator `apps/reception/src/hooks/mutations/usePrimeRequestResolution.ts`:
    - Extension approvals run through `useBookingDatesMutator` (booking/checkin/checkout canonical seam).
    - Meal-change exception approvals run through `usePreorderMutations` (preorder canonical seam).
    - Bag-drop statuses sync to `bagStorage/{guestUuid}` with shared request IDs/status.
    - Prime request status index updates (`primeRequests/byStatus/*`) and operator audit metadata are written atomically.
  - Added Reception operational UI at `apps/reception/src/components/prime-requests/PrimeRequestsQueue.tsx` and route `apps/reception/src/app/prime-requests/page.tsx`.
  - Added context links from queue items to canonical desks (`/extension`, `/checkout`, `/bar`) and wired operations navigation entry (`apps/reception/src/components/appNav/OperationsModal.tsx`).
- **Validation evidence:**
  - `pnpm --filter @apps/reception test -- src/schemas/__tests__/primeRequestsSchema.test.ts src/hooks/data/__tests__/usePrimeRequestsData.test.ts src/hooks/mutations/__tests__/usePrimeRequestResolution.test.ts src/components/prime-requests/__tests__/queue.test.tsx src/components/appNav/__tests__/Modals.test.tsx` — PASS.
  - `pnpm --filter @apps/reception typecheck` — PASS.
  - `pnpm --filter @apps/reception lint` — PASS.

## Phase 3: Staff Auth, Campaign Messaging, PWA (Promoted)

Precursor chains (`TASK-54`..`TASK-61`) are now complete. Wave 4 tasks are implemented and validated.

### TASK-51: Staff auth replacement (PinAuthProvider)

- **Type:** IMPLEMENT
- **Effort:** M
- **Confidence:** 82%
  - Implementation: 82% — Firebase custom-token bootstrap + role mapping seam is executable and fail-closed.
  - Approach: 84% — architecture decision is explicit and supported by task-level prototype evidence.
  - Impact: 82% — function-side token gate contract with deterministic 401/403 behavior reduces rollout risk.
...
- **Status:** Complete
- **Implementation notes:**
  - Replaced client-only localStorage PIN stub with a real auth flow in `apps/prime/src/contexts/messaging/PinAuthProvider.tsx`.
  - Added staff auth session endpoint `apps/prime/functions/api/staff-auth-session.ts` with server-side PIN-hash verification, deterministic lockout counters, and Firebase custom-token issuance.
  - Extended bootstrap result to include authenticated ID token in `apps/prime/src/services/staffAuthBootstrap.ts`.
  - Switched staff lookup API to token gate enforcement in `apps/prime/functions/api/check-in-lookup.ts`.
  - Updated staff lookup/check-in clients to send bearer token when calling `/api/check-in-lookup`.
- **Validation evidence:**
  - `pnpm --filter @apps/prime test -- src/contexts/messaging/__tests__/PinAuthProvider.replacement.test.tsx --maxWorkers=2` — PASS.
  - `pnpm --filter @apps/prime test -- functions/__tests__/staff-auth-session.test.ts functions/__tests__/staff-auth-token-gate.test.ts functions/__tests__/arrival-signals.test.ts --maxWorkers=2` — PASS.
  - `pnpm --filter @apps/prime typecheck` — PASS.

#### Re-plan Update (2026-02-10, iteration 11)
- **Previous confidence:** 74%
- **Updated confidence:** 74% (no uplift; decomposition created)
  - **Evidence class:** E1
  - Implementation: 74% — `PinAuthProvider` remains client-only localStorage PIN stub (`apps/prime/src/contexts/messaging/PinAuthProvider.tsx:29`).
  - Approach: 75% — role/rules shape is clear but unproven end-to-end (`apps/prime/database.rules.json:197`).
  - Impact: 74% — gate remains coarse without per-user token verification (`apps/prime/functions/lib/staff-owner-gate.ts:30`).
- **Decision / resolution:** converted all unresolved auth unknowns into formal precursor tasks (`TASK-54`, `TASK-55`, `TASK-56`) and held base confidence until those tasks produce E2/E3 evidence.

#### Re-plan Update (2026-02-10, iteration 12)
- **Previous confidence:** 74%
- **Updated confidence:** 82%
  - **Evidence class:** E3 + E2
  - Implementation: 82% — Firebase custom-token bootstrap, claim parsing, and fail-closed outcomes are executable in `apps/prime/src/services/staffAuthBootstrap.ts:45` and `apps/prime/src/services/staffAuthBootstrap.ts:116`.
  - Approach: 84% — architecture decision is locked in `docs/plans/prime-staff-auth-architecture-decision.md` and reinforced by executable spike coverage.
  - Impact: 82% — function-side token middleware contract enforces deterministic 401/403 with default-deny preserved in `apps/prime/functions/lib/staff-auth-token-gate.ts:139`.
- **Investigation performed:**
  - Repo: `apps/prime/src/services/staffAuthBootstrap.ts:45`, `apps/prime/src/services/staffAuthBootstrap.ts:116`, `apps/prime/functions/lib/staff-auth-token-gate.ts:139`, `apps/prime/functions/lib/staff-auth-token-gate.ts:180`.
  - Tests: `apps/prime/src/services/__tests__/staff-auth-bootstrap.spike.test.ts`, `apps/prime/functions/__tests__/staff-auth-token-gate.test.ts`.
  - Commands:
    - `pnpm --filter @apps/prime test -- src/services/__tests__/staff-auth-bootstrap.spike.test.ts --maxWorkers=2` — PASS.
    - `pnpm --filter @apps/prime test -- functions/__tests__/staff-auth-token-gate.test.ts --maxWorkers=2` — PASS.
- **Decision / resolution:** promote `TASK-51` to build-eligible confidence; keep dependency links to precursor tasks for traceability.

### TASK-52: Campaign/trigger messaging orchestration

- **Type:** IMPLEMENT
- **Effort:** L
- **Confidence:** 82%
  - Implementation: 82% — queue processor + dispatcher seams are executable for `arrival.48hours`.
  - Approach: 84% — email-first staged rollout is validated via worker endpoint and provider smoke contract.
  - Impact: 82% — idempotency/retry/failure-mode behavior is deterministic and test-backed.
- **Depends on:** `TASK-57`, `TASK-58`, `TASK-59`
- **Test contract:**
...
- **Status:** Complete
- **Implementation notes:**
  - Added deterministic `booking.confirmed` queue event IDs in `apps/prime/src/lib/messaging/triggers.ts` to enforce one-record-per-booking enqueue semantics.
  - Updated queue writer path in `apps/prime/src/lib/messaging/useMessagingQueue.ts` to skip duplicate `booking.confirmed` writes.
  - Completed queue/dispatcher/provider contract coverage in existing task suites.
- **Validation evidence:**
  - `pnpm --filter @apps/prime test -- src/lib/messaging/__tests__/queue-processor.test.ts functions/__tests__/messaging-dispatcher.test.ts functions/__tests__/email-provider-smoke.test.ts --maxWorkers=2` — PASS.
  - `pnpm --filter @acme/email test -- --testPathPattern="providerFunctions" --maxWorkers=2` — PASS (no matching tests in current package scope).

#### Re-plan Update (2026-02-10, iteration 11)
- **Previous confidence:** 70%
- **Updated confidence:** 70% (no uplift; decomposition created)
  - **Evidence class:** E1
  - Implementation: 70% — queue schemas and writers exist (`apps/prime/src/lib/messaging/triggers.ts:121`, `apps/prime/src/lib/messaging/useMessagingQueue.ts:51`), but no queue processor endpoint is present.
  - Approach: 72% — email-first path remains preferred, with webhook-only path currently active (`apps/prime/functions/lib/email-dispatch.ts:17`).
  - Impact: 70% — operational risk remains until processor + staging config are proven.
- **Decision / resolution:** converted unresolved messaging unknowns into formal precursor tasks (`TASK-57`, `TASK-58`, `TASK-59`) and held base confidence until those tasks complete.

#### Re-plan Update (2026-02-10, iteration 12)
- **Previous confidence:** 70%
- **Updated confidence:** 82%
  - **Evidence class:** E3 + E2
  - Implementation: 82% — queue processor and dispatcher seams are executable via `apps/prime/src/lib/messaging/queue-processor.ts:81` and `apps/prime/functions/lib/messaging-dispatcher.ts:31`.
  - Approach: 84% — email-first queue worker flow is validated in `apps/prime/functions/api/process-messaging-queue.ts:16`.
  - Impact: 82% — staging config and provider failure contracts are deterministic through `apps/prime/functions/__tests__/email-provider-smoke.test.ts`.
- **Investigation performed:**
  - Repo: `apps/prime/src/lib/messaging/queue-processor.ts:81`, `apps/prime/functions/lib/messaging-dispatcher.ts:31`, `apps/prime/functions/api/process-messaging-queue.ts:16`, `apps/prime/functions/lib/email-dispatch.ts:17`.
  - Tests: `apps/prime/src/lib/messaging/__tests__/queue-processor.test.ts`, `apps/prime/functions/__tests__/messaging-dispatcher.test.ts`, `apps/prime/functions/__tests__/email-provider-smoke.test.ts`.
  - Commands:
    - `pnpm --filter @apps/prime test -- src/lib/messaging/__tests__/queue-processor.test.ts --maxWorkers=2` — PASS.
    - `pnpm --filter @apps/prime test -- functions/__tests__/messaging-dispatcher.test.ts --maxWorkers=2` — PASS.
    - `pnpm --filter @apps/prime test -- functions/__tests__/email-provider-smoke.test.ts --maxWorkers=2` — PASS.
- **Decision / resolution:** promote `TASK-52` to build-eligible confidence; retain staged rollout model (single-trigger precedent first, broaden after implementation validations).

### TASK-53: PWA offline essentials

- **Type:** IMPLEMENT
- **Effort:** M
- **Confidence:** 80%
  - Implementation: 80% — service worker asset + manifest + layout wiring are in place and validated.
  - Approach: 82% — arrival-focused offline scope is executable through dedicated e2e + integration contracts.
  - Impact: 80% — cache version invalidation and offline fallback behavior are deterministic.
...
- **Status:** Complete
- **Implementation notes:**
  - Finalized fallback copy contract in `apps/prime/src/components/arrival/ArrivalHome.tsx` so stale/offline banners render deterministic text when translation keys are absent.
  - Kept service worker/manifest wiring and offline e2e assertions aligned with arrival-first scope.
- **Validation evidence:**
  - `pnpm --filter @apps/prime test -- src/lib/pwa/__tests__/registerSW.test.ts src/lib/pwa/__tests__/serviceWorkerAssets.test.ts src/app/__tests__/pwa-layout-wiring.test.tsx src/components/arrival/__tests__/ArrivalHome.offline-fallback.test.tsx --maxWorkers=2` — PASS.
  - `env -u ELECTRON_RUN_AS_NODE CYPRESS_BASE_URL=http://127.0.0.1:3015 pnpm exec start-server-and-test "OUTPUT_EXPORT=1 pnpm --filter @apps/prime build && env -u ELECTRON_RUN_AS_NODE pnpm --filter @apps/prime exec wrangler pages dev out --port 3015 --ip 127.0.0.1 --compatibility-date 2025-06-20" http://127.0.0.1:3015 "env -u ELECTRON_RUN_AS_NODE pnpm --filter @apps/prime exec cypress run --config-file cypress.config.ts --spec cypress/e2e/arrival-day-journey.cy.ts,cypress/e2e/pwa-offline-arrival.cy.ts"` — PASS (3 tests / 3 passed).

#### Re-plan Update (2026-02-10, iteration 11)
- **Previous confidence:** 68%
- **Updated confidence:** 68% (no uplift; decomposition created)
  - **Evidence class:** E1
  - Implementation: 68% — SW utilities exist (`apps/prime/src/lib/pwa/registerSW.ts:17`) but target `/sw.js` is missing.
  - Approach: 70% — arrival-first offline scope remains valid.
  - Impact: 68% — no installability/offline contract proof yet.
- **Decision / resolution:** converted unresolved PWA unknowns into formal precursor tasks (`TASK-60`, `TASK-61`) and held base confidence until those tasks produce executable evidence.

#### Re-plan Update (2026-02-10, iteration 12)
- **Previous confidence:** 68%
- **Updated confidence:** 80%
  - **Evidence class:** E3 + E2
  - Implementation: 80% — service worker asset, stale-cache pruning, and manifest wiring are active in `apps/prime/public/sw.js:1` and `apps/prime/src/app/layout.tsx:8`.
  - Approach: 82% — arrival-first offline contract is validated by dedicated e2e in `apps/prime/cypress/e2e/pwa-offline-arrival.cy.ts:15`.
  - Impact: 80% — cache invalidation + offline fallback behavior are now deterministic and test-backed.
- **Investigation performed:**
  - Repo: `apps/prime/public/sw.js:1`, `apps/prime/src/app/layout.tsx:8`, `apps/prime/cypress/e2e/pwa-offline-arrival.cy.ts:15`.
  - Tests: `apps/prime/cypress/e2e/pwa-offline-arrival.cy.ts`, `apps/prime/src/components/arrival/__tests__/ArrivalHome.offline-fallback.test.tsx`, `apps/prime/src/lib/pwa/__tests__/registerSW.test.ts`.
  - Commands:
    - `env -u ELECTRON_RUN_AS_NODE CYPRESS_BASE_URL=http://127.0.0.1:3015 pnpm exec start-server-and-test "OUTPUT_EXPORT=1 pnpm --filter @apps/prime build && env -u ELECTRON_RUN_AS_NODE pnpm --filter @apps/prime exec wrangler pages dev out --port 3015 --ip 127.0.0.1 --compatibility-date 2025-06-20" http://127.0.0.1:3015 "env -u ELECTRON_RUN_AS_NODE pnpm --filter @apps/prime exec cypress run --config-file cypress.config.ts --spec cypress/e2e/arrival-day-journey.cy.ts,cypress/e2e/pwa-offline-arrival.cy.ts"` — PASS (3 tests / 3 passed).
    - `pnpm --filter @apps/prime test -- src/components/arrival/__tests__/ArrivalHome.offline-fallback.test.tsx` — PASS.
- **Decision / resolution:** promote `TASK-53` to build-eligible threshold with the existing arrival-first rollout boundary.

### TASK-54: Staff auth architecture decision memo (custom claims vs PIN token)

- **Type:** INVESTIGATE
- **Effort:** S
- **Confidence:** 85%
  - Implementation: 85% — all candidate seams are already identified in Prime client, Functions gate, and RTDB rules.
  - Approach: 88% — decision criteria are clear (security, maintainability, Cloudflare compatibility, rollout risk).
  - Impact: 85% — closes the bottleneck that blocks TASK-51.
- **Acceptance criteria:**
  1. Decision memo created with explicit choice: Firebase custom claims vs server-validated PIN token.
  2. Memo includes migration steps, rollback path, and required env/config changes.
  3. Memo maps affected files/functions/rules with concrete line references.
  4. TASK-55/TASK-56 scope is updated to the chosen architecture.
- **Validation contract (decision evidence):**
  - **VC-01:** `PinAuthProvider` baseline documented from current implementation.
  - **VC-02:** RTDB role-rule requirements documented from `database.rules.json`.
  - **VC-03:** Function gate behavior documented from `staff-owner-gate.ts`.
  - **Artifact:** `docs/plans/prime-staff-auth-architecture-decision.md` (new)
- **Status:** Complete (2026-02-10)

#### Build Completion (2026-02-10)
- **Status:** Complete
- **Deliverable:** `docs/plans/prime-staff-auth-architecture-decision.md`
- **Validation evidence:** Decision memo explicitly captures option comparison, selected architecture, migration sequence, rollback, and evidence references from live code seams.

### TASK-55: Firebase Auth + claims bootstrap spike for Prime client

- **Type:** SPIKE
- **Effort:** M
- **Confidence:** 82%
  - Implementation: 82% — Firebase SDK already present; auth bootstrap seam is isolated to service/provider boundaries.
  - Approach: 82% — direct executable spike can validate chosen TASK-54 path quickly.
  - Impact: 82% — reduces client-side auth unknowns before TASK-51 implementation.
- **Depends on:** `TASK-54`
- **Acceptance criteria:**
  1. Chosen auth bootstrap path is implemented as a minimal spike in Prime client.
  2. Role claim mapping is validated against current rule model expectations.
  3. Failure path (missing/invalid token/claims) is deterministic.
  4. Spike outputs concrete promotion recommendation for TASK-51.
- **Test contract:**
  - **TC-01:** Auth bootstrap initializes and yields expected claim shape.
  - **TC-02:** Missing claim/invalid claim path fails closed.
  - **TC-03:** Staff-role claim path unlocks guarded-staff consumer checks.
  - **Test type:** unit/integration
  - **Test location:** `apps/prime/src/services/__tests__/staff-auth-bootstrap.spike.test.ts` (new)
  - **Run:** `pnpm --filter prime test -- --testPathPattern="staff-auth-bootstrap.spike"`
- **Status:** Complete (2026-02-10)

#### Build Completion (2026-02-10)
- **Status:** Complete
- **Implementation notes:**
  - Implemented Firebase custom-token bootstrap + deterministic claim-role parsing in `apps/prime/src/services/staffAuthBootstrap.ts`.
  - Added explicit promotion recommendation output for downstream `TASK-51` rollout gating.
  - Added executable spike contract tests in `apps/prime/src/services/__tests__/staff-auth-bootstrap.spike.test.ts`.
- **Validation evidence:**
  - `pnpm --filter @apps/prime test -- src/services/__tests__/staff-auth-bootstrap.spike.test.ts --maxWorkers=2` — PASS (1 suite / 4 tests).
- **Confidence reassessment:** 82% -> 84% (auth bootstrap + claims failure paths are now executable and deterministic).

### TASK-56: Function-side staff token gate contract spike

- **Type:** SPIKE
- **Effort:** M
- **Confidence:** 80%
  - Implementation: 80% — current API gate seam is centralized and testable.
  - Approach: 80% — token-validation contract can be proven independently from full migration.
  - Impact: 80% — closes server-side security uncertainty for TASK-51.
- **Depends on:** `TASK-54`, `TASK-55`
- **Acceptance criteria:**
  1. Function-side gate verifies staff identity/role using the architecture chosen in TASK-54.
  2. Unauthorized and role-mismatch requests fail with deterministic 401/403 behavior.
  3. Existing production default-deny behavior remains intact for non-enabled surfaces.
  4. Spike results document exact middleware contract for TASK-51 rollout.
- **Test contract:**
  - **TC-01:** Valid staff token/claims passes gate.
  - **TC-02:** Missing token fails gate.
  - **TC-03:** Non-staff token/claims fails gate.
  - **TC-04:** Feature-disabled production path remains deny-by-default.
  - **Test type:** unit/integration
  - **Test location:** `apps/prime/functions/__tests__/staff-auth-token-gate.test.ts` (new)
  - **Run:** `pnpm --filter prime test -- --testPathPattern="staff-auth-token-gate"`
- **Status:** Complete (2026-02-10)

#### Build Completion (2026-02-10)
- **Status:** Complete
- **Implementation notes:**
  - Added function-side staff token middleware contract in `apps/prime/functions/lib/staff-auth-token-gate.ts` with Firebase token lookup verification seam, staff-role enforcement, and deterministic 401/403 responses.
  - Preserved production default-deny gate behavior by chaining `enforceStaffOwnerApiGate` before token verification.
  - Added spike tests in `apps/prime/functions/__tests__/staff-auth-token-gate.test.ts`.
  - Middleware contract for TASK-51 rollout:
    1. `enforceStaffAuthTokenGate(request, env, deps)` returns `{ ok: true, identity }` for valid staff claims.
    2. Missing/invalid bearer token returns `{ ok: false, response }` with `401`.
    3. Non-staff claims return `{ ok: false, response }` with `403`.
- **Validation evidence:**
  - `pnpm --filter @apps/prime test -- functions/__tests__/staff-auth-token-gate.test.ts --maxWorkers=2` — PASS (1 suite / 4 tests).
- **Confidence reassessment:** 80% -> 83% (server-side token/role contract is executable with deny-by-default preserved).

### TASK-57: Messaging processor architecture + idempotency contract

- **Type:** INVESTIGATE
- **Effort:** S
- **Confidence:** 84%
  - Implementation: 84% — queue schema and writer seams are already stable.
  - Approach: 85% — decision space is constrained to worker topology + idempotency strategy.
  - Impact: 84% — unblocks TASK-52 without premature implementation.
- **Acceptance criteria:**
  1. Architecture decision memo chooses runtime (scheduled worker vs alternative) and retry/idempotency model.
  2. Contract defines queue state transitions (`pending`/`processing`/`sent`/`failed`) and retry policy.
  3. Contract defines provider abstraction boundary and required env vars.
  4. TASK-58/TASK-59 scope is updated to this contract.
- **Validation contract (decision evidence):**
  - **VC-01:** Existing queue schema and writer seam documented.
  - **VC-02:** Existing email-dispatch fallback path documented.
  - **VC-03:** Worker entrypoint gap documented with explicit implementation target.
  - **Artifact:** `docs/plans/prime-messaging-processor-decision.md` (new)
- **Status:** Complete (2026-02-10)

#### Build Completion (2026-02-10)
- **Status:** Complete
- **Deliverable:** `docs/plans/prime-messaging-processor-decision.md`
- **Validation evidence:** Decision memo captures runtime choice, queue/idempotency state machine, provider boundary, rollout sequencing, and rollback contract tied to current Prime code seams.

### TASK-58: Single-trigger queue processor spike (`arrival.48hours`)

- **Type:** SPIKE
- **Effort:** M
- **Confidence:** 82%
  - Implementation: 82% — spike scope is intentionally one trigger and one dispatch path.
  - Approach: 82% — executable processor proof is the shortest path to confidence uplift.
  - Impact: 82% — validates core operational risk in isolation.
- **Depends on:** `TASK-57`
- **Acceptance criteria:**
  1. Minimal processor consumes one queued `arrival.48hours` event.
  2. Processor enforces idempotent event handling and transition updates.
  3. Retry/failure paths write deterministic `retryCount` and `lastError`.
  4. Spike captures final implementation seam for TASK-52.
- **Test contract:**
  - **TC-01:** Pending event is processed once and marked `sent`.
  - **TC-02:** Duplicate processing attempt is idempotently ignored.
  - **TC-03:** Provider transient failure increments retry count and preserves event.
  - **TC-04:** Permanent failure marks event `failed` with `lastError`.
  - **Test type:** integration/contract
  - **Test location:** `apps/prime/functions/__tests__/messaging-dispatcher.test.ts` (new), `apps/prime/src/lib/messaging/__tests__/queue-processor.test.ts` (new)
  - **Run:** `pnpm --filter prime test -- --testPathPattern="messaging-dispatcher|queue-processor"`
- **Status:** Complete (2026-02-10)

#### Build Completion (2026-02-10)
- **Status:** Complete
- **Implementation notes:**
  - Implemented queue processor seam and dispatcher orchestration for `arrival.48hours` in `apps/prime/src/lib/messaging/queue-processor.ts` and `apps/prime/functions/lib/messaging-dispatcher.ts`.
  - Added contract suites for queue transition + idempotency behavior in `apps/prime/src/lib/messaging/__tests__/queue-processor.test.ts` and `apps/prime/functions/__tests__/messaging-dispatcher.test.ts`.
- **Validation evidence:**
  - `pnpm --filter @apps/prime test -- src/lib/messaging/__tests__/queue-processor.test.ts --maxWorkers=2` — PASS (1 suite / 4 tests).
  - `pnpm --filter @apps/prime test -- functions/__tests__/messaging-dispatcher.test.ts --maxWorkers=2` — PASS (1 suite / 4 tests).
- **Confidence reassessment:** 82% -> 84% (single-trigger queue processing and idempotency/failure contracts are executable).

### TASK-59: Email provider staging wiring + dispatch smoke contract

- **Type:** SPIKE
- **Effort:** M
- **Confidence:** 80%
  - Implementation: 80% — provider seam exists and env setup scope is narrow.
  - Approach: 80% — staging-only smoke contract reduces rollout risk.
  - Impact: 80% — resolves configuration unknown before TASK-52 full rollout.
- **Depends on:** `TASK-57`, `TASK-58`
- **Acceptance criteria:**
  1. Staging provider config is defined and documented for Prime Functions.
  2. One controlled smoke dispatch validates end-to-end send path.
  3. Failure behavior (missing/invalid config) is deterministic and observable.
  4. Promotion checklist for TASK-52 includes config + observability gates.
- **Test contract:**
  - **TC-01:** Valid staging config sends one deterministic smoke message.
  - **TC-02:** Missing provider config fails fast with actionable diagnostics.
  - **TC-03:** Invalid token/key fails closed and records explicit error.
  - **Test type:** integration/ops-contract
  - **Test location:** `apps/prime/functions/__tests__/email-provider-smoke.test.ts` (new)
  - **Run:** `pnpm --filter prime test -- --testPathPattern="email-provider-smoke"`
- **Status:** Complete (2026-02-10)

#### Build Completion (2026-02-10)
- **Status:** Complete
- **Implementation notes:**
  - Added staging smoke contract test in `apps/prime/functions/__tests__/email-provider-smoke.test.ts` covering valid config, missing config, and invalid-token failure behavior.
  - Hardened webhook error typing in `apps/prime/functions/lib/email-dispatch.ts` by attaching HTTP status/code metadata so 4xx provider failures fail closed.
  - Documented required staging provider env contract and smoke runbook in `apps/prime/docs/CONTRIBUTING.md`.
  - Added TASK-52 promotion checklist contract:
    1. Staging `PRIME_EMAIL_WEBHOOK_URL` + `PRIME_EMAIL_WEBHOOK_TOKEN` configured.
    2. `email-provider-smoke` contract suite passes in CI/local.
    3. Queue outcomes (`sent`/`failed`) and `lastError` are reviewed before enabling broader trigger rollout.
- **Validation evidence:**
  - `pnpm --filter @apps/prime test -- functions/__tests__/email-provider-smoke.test.ts --maxWorkers=2` — PASS (1 suite / 3 tests).
- **Confidence reassessment:** 80% -> 83% (provider config + failure-mode behavior is now deterministic and documented).

### TASK-60: Minimal service worker + manifest integration spike

- **Type:** SPIKE
- **Effort:** M
- **Confidence:** 82%
  - Implementation: 82% — registration/utilities already exist; missing artifacts are clear.
  - Approach: 82% — minimal SW + manifest spike is bounded and testable.
  - Impact: 82% — resolves the primary technical unknown for TASK-53.
- **Depends on:** `TASK-07`
- **Acceptance criteria:**
  1. Minimal `/sw.js` and manifest are added with arrival-shell-focused caching.
  2. Root layout wiring includes `ServiceWorkerProvider` behind existing gating.
  3. Cache versioning and invalidation hooks are explicit.
  4. Spike documents compatibility notes for Cloudflare Pages static export.
- **Test contract:**
  - **TC-01:** SW registration succeeds in supported browser context.
  - **TC-02:** Manifest is discoverable and valid from the root app shell.
  - **TC-03:** Cache version bump invalidates old cache deterministically.
  - **Test type:** integration
  - **Test location:** `apps/prime/src/lib/pwa/__tests__/registerSW.test.ts` (new), `apps/prime/src/lib/pwa/__tests__/serviceWorkerAssets.test.ts` (new), `apps/prime/src/app/__tests__/pwa-layout-wiring.test.tsx` (new)
  - **Run:** `cd apps/prime && pnpm exec jest --config jest.config.cjs --runInBand --testPathPattern="registerSW|serviceWorkerAssets|pwa-layout-wiring"`
- **Status:** Complete (2026-02-10)

#### Build Completion (2026-02-10)
- **Status:** Complete
- **Implementation notes:**
  - Added service worker asset with arrival-shell cache versioning + stale-cache cleanup: `apps/prime/public/sw.js`.
  - Added manifest + icon assets: `apps/prime/public/manifest.webmanifest`, `apps/prime/public/icons/prime-icon.svg`.
  - Wired root app shell through `ServiceWorkerProvider` and exported manifest metadata: `apps/prime/src/app/layout.tsx`.
  - Added PWA spike tests: `apps/prime/src/lib/pwa/__tests__/registerSW.test.ts`, `apps/prime/src/lib/pwa/__tests__/serviceWorkerAssets.test.ts`, `apps/prime/src/app/__tests__/pwa-layout-wiring.test.tsx`.
- **Validation evidence:**
  - `cd apps/prime && pnpm exec jest --config jest.config.cjs --runInBand --testPathPattern="registerSW|serviceWorkerAssets|pwa-layout-wiring"` — PASS (3 suites / 6 tests).
  - `pnpm --filter @apps/prime typecheck` — PASS.
  - `pnpm --filter @apps/prime lint` — PASS (warnings only).

### TASK-61: Offline arrival e2e + cache invalidation contract spike

- **Type:** SPIKE
- **Effort:** M
- **Confidence:** 80%
  - Implementation: 80% — depends on TASK-60 artifact existence and existing arrival/offline surfaces.
  - Approach: 80% — explicit e2e/offline contract closes remaining TASK-53 risk.
  - Impact: 80% — provides executable production-safety proof.
- **Depends on:** `TASK-60`, `TASK-41`
- **Acceptance criteria:**
  1. E2E scenario proves cached arrival route behavior under offline conditions.
  2. Stale cache/version behavior is explicit and user-visible.
  3. Failure behavior is deterministic when required assets are missing.
  4. Spike output includes go/no-go criteria for TASK-53 promotion.
- **Test contract:**
  - **TC-01 (e2e):** Online-first visit caches arrival shell, then offline revisit renders usable arrival view.
  - **TC-02 (e2e):** Cache version bump triggers update path and invalidates stale assets.
  - **TC-03:** Offline missing-cache path shows deterministic fallback copy.
  - **Test type:** e2e + integration
  - **Test location:** `apps/prime/cypress/e2e/pwa-offline-arrival.cy.ts` (new), `apps/prime/src/components/arrival/__tests__/ArrivalHome.offline-fallback.test.tsx` (extend)
  - **Run:** `pnpm exec cypress run --config-file apps/prime/cypress.config.ts --spec apps/prime/cypress/e2e/pwa-offline-arrival.cy.ts` and `pnpm --filter prime test -- --testPathPattern="ArrivalHome.offline-fallback"`
- **Status:** Complete (2026-02-10)

#### Build Completion (2026-02-10)
- **Status:** Complete
- **Implementation notes:**
  - Finalized offline-arrival e2e contract assertions in `apps/prime/cypress/e2e/pwa-offline-arrival.cy.ts` against deterministic cached-code + cache-namespace behavior.
  - Corrected arrival-day state classification for check-in date boundary in `apps/prime/src/hooks/dataOrchestrator/useDateInfo.ts`.
  - Added deterministic stale-warning fallback copy for offline cached code in `apps/prime/src/components/arrival/ArrivalHome.tsx`.
  - Hardened service-worker stale-namespace pruning on both activation and navigation handling in `apps/prime/public/sw.js`.
- **Validation evidence:**
  - `pnpm --filter @apps/prime test -- src/hooks/dataOrchestrator/useDateInfo.test.ts` — PASS.
  - `env -u ELECTRON_RUN_AS_NODE CYPRESS_BASE_URL=http://127.0.0.1:3015 pnpm exec start-server-and-test "OUTPUT_EXPORT=1 pnpm --filter @apps/prime build && env -u ELECTRON_RUN_AS_NODE pnpm --filter @apps/prime exec wrangler pages dev out --port 3015 --ip 127.0.0.1 --compatibility-date 2025-06-20" http://127.0.0.1:3015 "env -u ELECTRON_RUN_AS_NODE pnpm --filter @apps/prime exec cypress run --config-file cypress.config.ts --spec cypress/e2e/arrival-day-journey.cy.ts,cypress/e2e/pwa-offline-arrival.cy.ts"` — PASS (3 tests / 3 passed).
- **Confidence reassessment:** 80% -> 84% (offline cached-route and cache-invalidation contracts are now executable and passing).

## Decision Log

| Date | Decision | Rationale |
|------|----------|-----------|
| 2026-02-10 | Promote Wave 4 tasks (TASK-51, TASK-52, TASK-53) to build-ready after precursor completion and sequence verification | Precursor chains delivered E3/E2 evidence, removing the prior confidence bottlenecks without changing topology. |
| 2026-02-10 | Decompose deferred TASK-51/52/53 into formal precursor chains (TASK-54..TASK-61) before any further promotion | Replaced vague sub-80 uncertainty with executable investigate/spike bottlenecks, then sequenced waves so progression can be proved or halted early. |
| 2026-02-10 | Select Firebase custom-token + role-claims model as staff auth target architecture | Aligns with existing RTDB auth/rule model and removes client-local PIN stub as a security boundary. |
| 2026-02-10 | Select Cloudflare scheduled-worker queue processor for messaging orchestration | Matches Prime runtime/deployment model while enabling staged idempotent rollout from one trigger. |
| 2026-02-10 | Ship minimal PWA baseline as a precursor (SW asset + manifest + layout wiring) | Converts PWA ambiguity into executable contracts before full offline rollout task promotion. |
...
- Q: How much is actually implemented?
  - A: Substantially more than the old plan claimed. See wf-fact-find `docs/plans/prime-guest-portal-gap-wf-fact-find.md` for full code truth assessment.
...
- Q: Pre-check-in data capture (GAP-06) — compliance?
  - Decision: Omitted from this plan. Document capture requires legal review. If confirmed, add as a new deferred task using the next available TASK-ID.
