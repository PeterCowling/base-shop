...
- Enable humans and agents to generate, work up, wf-fact-find, plan, execute, and reflect on ideas/opportunities
...
- **Agent layer:** Extends existing `.claude/skills/` pattern
  ...
  - Reuses existing `/wf-fact-find`, `/wf-plan`, `/wf-build`, `/reflect` for engineering cards
...
3. Card progresses through lanes → stage docs (`wf-fact-find.md`, `plan.md`, `build.md`, `reflect.md`) maintained
...
- **Previous confidence:** 85% (initial wf-replan), then 45% (after review), then 30% (after adding BOS-00-F blocker)
...
- **Updated confidence:** 85%
  ...
  - Approach: 85% — Minimal placeholder approach confirmed: frontmatter + 1-2 sentence placeholder body; agent skill populates details during wf-fact-find/plan/build/reflect phases
...
- **Decision / resolution:**
  - **Stage doc templates (minimal placeholders):**
    - `wf-fact-find.(user|agent).md`: Frontmatter: Type: Fact-Find, Status: Draft, Card-ID: <ID>. Body: "Fact-finding in progress."
  ...
  - **Validation rules (prerequisite checks):**
    ...
    - Future: could add "can't move to Planned without completing wf-fact-find" validation if needed
- **Changes to task:**
...
- **Depends on:** BOS-07
- **Confidence:** 88%
  - Implementation: 90% — Skill structure clear from `.claude/skills/wf-fact-find/SKILL.md` (100 lines; name, description, workflow sections); logic straightforward
...
- **Investigation performed:**
  - Repo: `.claude/skills/wf-fact-find/SKILL.md` (template: frontmatter, Operating Mode, Workflow, Outputs)
