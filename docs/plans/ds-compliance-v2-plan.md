...
|---|---|---|---:|---:|---|---|---|
| DS-01 | IMPLEMENT | Fix design-system primitive warnings | 92% | M | Pending | - | DS-08 |
| DS-02 | IMPLEMENT | Fix no-hardcoded-copy warnings (BOS + guide-system) | 90% | M | Pending | - | DS-08 |
| DS-03 | IMPLEMENT | Fix min-tap-size warnings (BOS + design-system) | 88% | S | Pending | - | DS-08 |
| DS-04 | IMPLEMENT | Restructure reception ESLint config | 88% | S | Pending | - | DS-05, DS-06 |
| DS-05 | IMPLEMENT | Fix reception token violations and escalate | 85% | M | Pending | DS-04 | DS-08 |
| DS-06 | IMPLEMENT | Fix reception spacing + arbitrary-tailwind and escalate | 80% | M | Pending | DS-04 | DS-08 |
| DS-07 | IMPLEMENT | Escalate Prime non-color rules to error | 95% | S | Pending | - | DS-08 |
| DS-08 | IMPLEMENT | Final lint verification and cleanup | 95% | S | Pending | DS-01–DS-07 | - |
...
- [ ] `pnpm lint` passes with 0 DS warnings (excluding deferred rules)
- [ ] All non-deferred DS rules at `error` level for all production code
- [ ] Reception's `offAllDsRules` spread replaced with selective per-rule overrides
- [ ] Prime's warn-level overrides removed (rules inherit error from CMS/UI/Apps block)
- [ ] Deferred rules documented: reception layout-primitives (off), reception hardcoded-copy (off), BOS guides (offAllDsRules)
- [ ] No regressions — `pnpm lint` exits 0
