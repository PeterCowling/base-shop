...
Related-Plan: docs/plans/email-autodraft-world-class-upgrade-plan.md  <!-- not yet created; will be generated by /plan-feature -->
Business-Unit: BRIK
...
- Existing label-driven operational flow remains the live baseline (`.claude/skills/process-emails/SKILL.md`).
...
| Operator friction time cost | ~8-15 minutes per 10-email session | Medium-Low | `[Estimate]` based on workflow step decomposition from `.claude/skills/process-emails/SKILL.md:25` onward | Not instrumented in telemetry; treat as directional until measured. |
...
- `.claude/skills/process-emails/SKILL.md`
...
- `[Fact]` Process flow requires multiple sequential stages per email (`.claude/skills/process-emails/SKILL.md:25`, `.claude/skills/process-emails/SKILL.md:80`, `.claude/skills/process-emails/SKILL.md:157`).
...
## Provisional Confidence Inputs (for /plan-feature)
...
- Confidence should be recalculated in `/plan-feature` after open questions are answered.

## Planning Boundary (Fact-Find vs Planning)
The following are explicitly planning-owned and intentionally not finalized in this fact-find:
...
These are retained as planning inputs only. They are intentionally non-atomic and must be decomposed in `/plan-feature`.
...
- Hypothesis A8: Improve `/process-emails` operator ergonomics (evidence display, batch handling for low-risk routines, escalation-first queueing).
