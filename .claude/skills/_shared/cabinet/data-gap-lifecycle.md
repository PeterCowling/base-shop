...
For DGPs tagged `gate-unresolved`, the lp-fact-find should target the specific incomplete artifacts:
...
**Resolution path:** When the missing artifact data is gathered (via lp-fact-find or next sweep), the idea re-enters Stage 4 for Contrarian Gate re-evaluation. If the gate now returns PASS, the idea proceeds to Munger-Buffett verdict and onward through the pipeline.
...
**Output**: Investigation produces a **lp-fact-find stage doc** (not a build card). The lp-fact-find doc answers the blocking questions listed in the DGP's "Blocking Questions" section.
...
- Skipping the lp-fact-find stage of the feature workflow
...
DGP (decision-gap idea)
    |
    v
Investigation card (lp-fact-find task)
    |
    v
...
Agent completes lp-fact-find, produces stage doc with findings:
...
- **v1.2** (2026-02-10): Contrarian Gate integration: added Contrarian Gate → DGP path for UNRESOLVED ideas (presentable but gate-blocked), +15 VOI boost for gate DGPs, gate resolution lp-fact-find template, promotion path requires gate re-evaluation to PASS.
