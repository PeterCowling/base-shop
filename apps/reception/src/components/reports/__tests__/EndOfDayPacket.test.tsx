...
const mockUseEndOfDayReportData = vi.fn();
const mockTillDataProvider = vi.fn();
const mockSafeDataProvider = vi.fn();
...
vi.mock("../../../context/SafeDataContext", () => ({
  SafeDataProvider: ({
    children,
    startAt,
    endAt,
...
vi.mock("firebase/database", () => {
  getMock = vi.fn();
  return {
    ref: vi.fn((db: unknown, path: string) => ({ path })),
    query: vi.fn((refObj: unknown, ...rest: unknown[]) => ({ ref: refObj, rest })),
    orderByChild: vi.fn((field: string) => ({ op: "orderByChild", field })),
    startAt: vi.fn((val: unknown) => ({ op: "startAt", val })),
    endAt: vi.fn((val: unknown) => ({ op: "endAt", val })),
    get: (...args: unknown[]) => getMock(...args),
  };
});

vi.mock("../../../hooks/data/useEndOfDayReportData", () => ({
