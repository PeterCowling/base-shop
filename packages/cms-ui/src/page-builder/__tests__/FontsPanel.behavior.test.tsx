...
jest.mock("@acme/design-system/shadcn", () => ({ __esModule: true, Dialog: ({ children }: any) => <div>{children}</div>, DialogContent: ({ children }: any) => <div role="dialog">{children}</div>, DialogTitle: ({ children }: any) => <div>{children}</div> }));
