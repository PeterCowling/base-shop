...
Relates-to: .claude/skills/ideas-go-faster/SKILL.md
...
|---|---|---|---|
| LC-01 | Stance recorded | Sweep report frontmatter (`Stance`) + stance propagation note | `check-ideas-go-faster-output.sh` frontmatter checks |
| LC-02 | Persistence mode recorded | Sweep report frontmatter (`Persistence-Mode`, `Dry-Run-Writes-Blocked`) | `check-ideas-go-faster-output.sh` frontmatter checks |
| LC-03 | Assumptions source recorded | Sweep report frontmatter (`Assumptions-Source`) + assumptions precedence contract | `check-ideas-go-faster-output.sh` frontmatter checks |
| LC-04 | Verbosity mode recorded | Sweep report frontmatter (`Verbosity-Mode`) + word-limit contract | `check-ideas-go-faster-output.sh` frontmatter checks |
...
| LC-06 | Who Said What section present | Report section `Who Said What (F1)` | `check-ideas-go-faster-output.sh` section + attribution token checks |
| LC-07 | Generation cadence executed | Stage 1 divergence/deepening contract + report Generation Phase section | `check-ideas-go-faster-contracts.sh` cadence checks (F8) |
| LC-08 | Depth gate executed | Stage 2 depth gate required fields (`Evidence-Pointers`, `Falsification-Test`, `First-Signal`) | `check-ideas-go-faster-contracts.sh` depth checks (F9) |
...
| LC-10 | Assumption challenge executed when assumptions provided | Stage 2 assumption challenge verdict contract + report section `Assumption Challenge (F3)` | `check-ideas-go-faster-output.sh` verdict-token check when source != none |
| LC-11 | Clustering executed | Stage 3 clustering contract + report section | Contracted in SKILL.md; reviewed in run output |
| LC-12 | Tool-Gap Register present | Report section `Tool-Gap Register (F2)` | `check-ideas-go-faster-output.sh` section + gap/evidence checks |
...
| LC-16 | Kill/Hold rationale present | Report section `Kill/Hold Rationale (F4)` | `check-ideas-go-faster-output.sh` rationale token checks |
| LC-17 | Economics gate executed | Stage 5 economics blocker + report section `Economics Gate (F5)` | `check-ideas-go-faster-output.sh` required economics token checks |
| LC-18 | 3A manifest emitted | Stage 5.5 `commit-manifest.json` contract | `check-ideas-go-faster-output.sh` artifact + schema checks |
| LC-19 | Cards created or previewed | Stage 6 live/dry-run persistence contract | `check-ideas-go-faster-output.sh` ledger mode checks |
...
| LC-21 | 3B write ledger emitted | Stage 6 `write-ledger.jsonl` contract | `check-ideas-go-faster-output.sh` artifact + schema checks |
| LC-22 | Card ID resolution honored | Stage 7 `{{CARD_ID}}` injection via card-id-map contract | `check-ideas-go-faster-output.sh` manifest factfind dependency/placeholder checks |
| LC-23 | Fact-find seeding or previewed | Stage 7 fact-find seeding (live) / preview (dry-run) contract | Contracted in SKILL.md; run output section required in review |
...
| LR-08 | Created fact-find docs for low-priority cards | Stage 7 top-K constraint + red flag #8 |
