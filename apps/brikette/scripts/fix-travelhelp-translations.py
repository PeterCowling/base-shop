#!/usr/bin/env python3
"""Fix travelHelp translations by copying EN structure and preserving existing translations."""
...
# Read EN source
en_path = Path("src/locales/en/guides/content/travelHelp.json")
with open(en_path) as f:
    en_data = json.load(f)

# Locales to fix (currently have condensed 2-section structures)
locales_to_fix = ["hi", "hu", "it", "ja", "ko", "no", "pl", "pt", "ru", "sv", "vi", "zh"]

for locale in locales_to_fix:
    locale_path = Path(f"src/locales/{locale}/guides/content/travelHelp.json")
    ...
    # Preserve FAQs if they exist
    if "faqsTitle" in locale_data:
        fixed_data["faqsTitle"] = locale_data["faqsTitle"]
    if "faqs" in locale_data:
        fixed_data["faqs"] = locale_data["faqs"]
