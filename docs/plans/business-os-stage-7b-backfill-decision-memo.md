...
- Approve an implementation-ready optional contract for later activation:
  - max **1** existing P1/P2 card without a lp-fact-find doc per sweep,
...
- No existing `lp-fact-find` stage doc.
...
- Stage 7b output is separate section:
  ...
  - `Backfill-Reason: oldest eligible P1/P2 without lp-fact-find`
...
- Existing eligible backlog cards (no lp-fact-find):
