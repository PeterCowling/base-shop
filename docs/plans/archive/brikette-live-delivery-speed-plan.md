...
## Fact-Find Reference
- Related brief: `docs/plans/brikette-live-delivery-speed-wf-fact-find.md`
...
- **Rollout / rollback:**
  ...
  - Rollback: revert loader changes; accept larger bundle until wf-replanned.
...
- **Implementation notes (2026-02-05):**
  ...
  - Automation: added `scripts/post-ops-deploy-cache-check.sh` and wired it into `.github/workflows/reusable-app.yml` so staging/production deploys fail if cache header contracts regress.
...
- **Root cause (confirmed via investigation):**
  ...
  - Next.js `output: 'export'` DOES copy `public/` to `out/` correctly — the earlier wf-replan claim that `_headers` wasn't deployed was wrong. It IS deployed, just ignored due to rule count.
...
- **Test contract:**
  ...
  - **Run:** `BASE_URL=https://www.hostel-positano.com sh scripts/post-ops-deploy-cache-check.sh` or manual curls per TASK-06 TCs.
...
#### Re-plan Update (2026-02-05 — Initial)
- **Previous confidence:** n/a (new task split from TASK-07 during wf-replan)
- **Updated confidence:** 82%
...
- **Decision / resolution:**
  ...
  - For DECISION tasks, "Needs-Input" is the appropriate status (not a blocker requiring wf-replanning).
...
- **Confidence:** 88%
  ...
  - Approach: 90% — investigate-before-implement is the right approach given the complexity revealed by wf-replan.
...
- **Investigation performed (pre-investigation findings from wf-replan):**
...
Based on wf-replan findings, the highest-leverage sequence is:
...
## Plan Changelog
- 2026-02-05: Plan created from wf-fact-find; i18n bundle fix requires investigation before implementation.
