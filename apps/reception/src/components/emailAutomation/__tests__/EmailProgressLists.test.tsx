...
describe("EmailProgressLists", () => {
  it("groups rows by currentCode and handles button clicks", async () => {
    const logNextActivity = vi.fn(() => Promise.resolve());
    const logConfirmActivity = vi.fn(() => Promise.resolve());
  ...
  it("omits optional chips when data missing", () => {
    ...
    render(
      <EmailProgressLists
        emailData={[item]}
        logNextActivity={vi.fn()}
        logConfirmActivity={vi.fn()}
      />
    );
  ...
  it("shows errors and keeps row when actions fail", async () => {
    const logNextActivity = vi.fn(() => Promise.reject(new Error("fail")));
    const logConfirmActivity = vi.fn(() => Promise.reject(new Error("fail")));
