 ...
 */
export function isoDateInNDays(days) {
 ...
 * a minimum of `1` day.
 */
export function calculateRentalDays(returnDate) {
    if (!returnDate)
        return 1;
 ...
 */
export function formatTimestamp(ts, locale) {
 ...
 * input.
 */
export function parseTargetDate(targetDate, timezone) {
    if (!targetDate)
        return null;
    try {
        const date = timezone
