...
| B4 | **CAP-07 measurement confirmed active** | For each business: `measurement-setup.user.md` exists at Active status and ≥1 confirmed conversion event is documented. | Measurement setup doc missing, at Draft, or confirmed conversion event count = 0. | Operator | Create REM task: "Advance measurement setup for [BIZ] to Active status." |
