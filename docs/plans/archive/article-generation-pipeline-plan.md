...
The work is divided into four delivery slices, each independently shippable: (1) centralise read path + data model, (2) translation + validation as shared tooling, (3) CMS authoring + workflow, (4) AI pipeline. This plan covers **Slice 1 in full detail** (ready to build) and **Slices 2-4 at seed level** (requiring `/lp-do-plan` once Slice 1 lands).
...
- Block type centralisation (deferred -- blocks stay app-specific per lp-do-fact-find default)
...
- Constraints:
  ...
  - Product repository pattern invariants must be followed (10 invariants from lp-do-fact-find)
...
- Related brief: `docs/plans/article-generation-pipeline-lp-do-fact-find.md`
- Key findings:
...
- **Confidence:** 90%
  ...
  - Approach: 90% — Types mirror resolved decisions from lp-do-fact-find (unified status enum, risk tier, schemaVersion, content-as-code). Block types as `unknown[]` is the simplest viable approach.
...
- **Notes / references:**
  ...
  - Status enum resolution: lp-do-fact-find "Resolved Questions" section
...
- **Depends on:** TASK-02
- **Confidence:** 85%
  - Implementation: 90% — Direct replication of `products.json.server.ts` pattern. Every helper function and CRUD operation is documented in lp-do-fact-find with exact signatures. Split content storage adds complexity vs products.
...
- **Notes / references:**
  ...
  - Status mapping: lp-do-fact-find "Status enum divergence" section
...
- **Documentation impact:** None
- **Notes / references:**
  - Rendering chain documented in lp-do-fact-find: route → `resolveGuideKeyFromSlug` → `loadGuideManifestOverridesFromFs` → `<GuideContent>` → `useTranslation("guides")` → `tGuides("content.{key}.{field}", { returnObjects: true })`
...
- **Depends on:** TASK-07
- **Confidence:** 70% ⚠️
  - Implementation: 80% — Slice boundaries are well-defined in lp-do-fact-find
...
- **Acceptance:**
  ...
  - Next slice's `/lp-do-plan` is scoped accordingly — run `/lp-do-plan` for Slice 2 when ready
...
- **Next action:** Run `/lp-do-plan` for Slice 2.

---

## Slice 2-4 Task Seeds (Non-binding)

These are carried over from the lp-do-fact-find brief. Each will get its own `/lp-do-plan` run when ready.
