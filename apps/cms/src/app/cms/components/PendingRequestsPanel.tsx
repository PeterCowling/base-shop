    ...
    <Card className="border border-border/60">
      <CardContent className="space-y-6">
        <div className="flex items-center justify-between gap-3">
          ...
          <Tag variant={requests.length === 0 ? "success" : "warning"}>
            {requests.length === 0
                    ...
                    {roles.map((role) => {
                      ...
                      return (
                        <label
                          key={role}
                          htmlFor={checkboxId}
                          className="flex items-center gap-2 rounded-lg border border-border/60 bg-background/40 px-3 py-2 text-sm text-foreground"
                        >
                          <Checkbox
                            id={checkboxId}
                            checked={checked}
