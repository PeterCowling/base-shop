...
- **Inventory consistency & concurrency**
  ...
  - **Best‑bet direction (current thinking):**
    ...
    - *Add‑to‑cart: per‑cart checks, no global reservation*
      - Cart endpoints (`@platform-core/cartApi` used by `packages/template-app/src/api/cart/route.ts` and the legacy cookie cart in `apps/cover-me-pretty/src/api/cart/route.ts`) enforce requested quantities against **product‑level `sku.stock`**, not live `InventoryItem.quantity`:
    ...
    - *Checkout and reservation semantics*
      - Rental flows:
        - After a successful Stripe Checkout, the template app’s `/api/rental` endpoint loads `InventoryItem[]` via `readInventory(shop)` and calls `reserveRentalInventory(shop, items, sku, from, to)` for each rented SKU (see `packages/platform-core/src/orders/rentalAllocation.ts`).
        - `reserveRentalInventory`:
          - Verifies the requested rental window is within the SKU’s `availability` and respects wear/maintenance limits (`wearAndTearLimit`, `maintenanceCycle`).
          - Picks a candidate `InventoryItem` with `quantity > 0` and then uses `updateInventoryItem` to decrement `quantity` and increment `wearCount` for that `(shop, sku, variantAttributes)` row.
        - For DB backends this decrement runs inside a Prisma transaction; for JSON it is guarded by the file lock.
        - Invariants for `updateInventoryItem` / `reserveRentalInventory`:
          - **No negative quantities**:
            - The mutate function used by `updateInventoryItem` must treat `current === undefined` or `current.quantity <= 0` as “no reservation” and return `current` unchanged for any decrement operation.
            - After any update, `quantity >= 0` must hold; callers must never assume a decrement succeeded unless `updateInventoryItem` returns a non‑`undefined` `InventoryItem` with a non‑negative quantity.
          - **Reservation success/failure**:
            - `reserveRentalInventory` must:
              - Only return a reserved item when the underlying `updateInventoryItem` decrement succeeded.
              - Return `null` when allocation fails (no stock, outside availability window, wear/maintenance constraints, or concurrent decrements winning the race).
            - Under contention, the **first successful reservation wins**; later attempts see `quantity <= 0` and fail cleanly without corrupting stock.
      - Sale flows:
        - For straight sale checkouts, the same pattern applies:
          - At checkout confirmation we reload inventory for the shop and, for each purchased SKU, call a helper analogous to `reserveRentalInventory` that decrements `quantity` via `updateInventoryItem` under the same invariants.
          - As with rentals, we rely on per‑item atomic updates plus a `current.quantity > 0` guard inside the mutate function to prevent overselling at the storage layer.
      - Reservation model for v1:
        - We **do not implement long‑lived per‑cart reservations** (no “hold this item for 10 minutes” while the user browses); inventory is only reserved when:
          - A checkout session is confirmed (rental or sale), or
          - An admin explicitly adjusts inventory via CMS or import tooling.
        - This means oversells are still theoretically possible if many customers check out simultaneously on a low‑stock SKU, but the race window is limited to checkout confirmation and guarded by atomic updates rather than every cart change.
    - *Surfacing failures and keeping stock consistent*
      ...
      - Preferred v1 behaviour when allocation fails at checkout:
        - Rental flows:
          - Stripe Checkout still succeeds if payment was authorised; we still create the `RentalOrder` via `addOrder` for traceability.
          - We mark the order as needing attention (e.g. with a dedicated `allocationFailed` flag and/or status `"needs_attention"`) and emit a structured log/metric including `{ shopId, sku, requestedQty, reason: "no_inventory" }`.
          - CMS surfaces these orders prominently (e.g. in an “Orders needing allocation” queue), and operators are expected to either:
            - Adjust the order (different date/size) and keep the payment, or
            - Trigger `refundOrder` to perform a full refund and communicate with the customer.
        - Sale flows (when modelled on the same infrastructure) can follow the same pattern but may bias toward auto‑refunding and sending a “sorry, out of stock” email, depending on shop expectations.
        - In all cases, the guarantee is: **we never silently drive inventory negative, and allocation failures are both visible in CMS and logged for operators.**
      ...
      - Tests and reconciliation jobs we expect:
        - **Concurrency tests** around `updateInventoryItem`/`reserveRentalInventory` that hammer a single SKU/variant with concurrent reservation attempts and assert:
          - `quantity` never goes negative in storage.
          - The number of successful reservations never exceeds the initial available quantity.
        - A periodic **catalogue vs inventory reconciliation job** that:
          - Aggregates `InventoryItem.quantity` per SKU and compares it against `sku.stock`.
          - Logs (and optionally alerts) when they drift beyond a threshold, so merchandisers or ops can correct `sku.stock` or investigate inventory anomalies.
- **Security & compliance**
  ...
  - **Best‑bet direction (current thinking):**
    ...
    - *Cart cookie integrity and scope*
      - Cart identity and state are protected via `@platform-core/cartCookie`:
        ...
        - Cookie values are HMAC‑signed strings produced by `encodeCartCookie` using `CART_COOKIE_SECRET` from `loadCoreEnv`; `decodeCartCookie` verifies signatures with `timingSafeEqual` and returns `null` on tampering or malformed JSON.
        - In the shared `cartApi` implementation, the cookie typically stores a **cart ID**, not arbitrary cart JSON; all cart mutations go through `CartStore` abstractions (memory/Redis) on the server.
        - In legacy handlers (e.g. `apps/cover-me-pretty/src/api/cart/route.ts`), the full cart JSON is signed and verified via the same helpers; the server never blindly trusts client‑provided cart state.
      - Best‑bet stance:
  ...
  - **Best‑bet direction (current thinking):**
    ...
    - *Cart identity today (anonymous vs authenticated)*
      - Cart identity is currently **cookie‑scoped**, not account‑scoped:
        - Anonymous carts are keyed by the signed `__Host-CART_ID` cookie (see `cartCookie.ts` / `cartStore.ts`), which either encodes a cart ID (shared cart store) or a signed cart JSON in legacy handlers.
    ...
    - *Converging legacy implementations*
      ...
      - Best‑bet convergence path (incremental):
        - **Step 1 – behavioural alignment via contract tests:**
          ...
          - Wire these tests into both `template-app` and `cover-me-pretty` to ensure they conform to the same contract.
        - **Step 2 – thin wrapper phase:**
          - Update `apps/cover-me-pretty/src/api/cart/route.ts` to:
            - Delegate to `@platform-core/cartApi` where possible, or at least reuse shared helpers (`decodeCartCookie`, shared schemas) while it keeps its cookie‑backed store.
          - Re‑run contract tests and adjust until behaviour (status codes, error bodies, cart shapes) matches the shared handlers.
        - **Step 3 – full re‑export:**
          - Once behaviour matches, simplify the route to a straight re‑export of `cartApi` handlers and switch to `CartStore` as the single backend.
          - From that point, all shops share the same implementation and any further evolution happens in `@platform-core/cartApi`.
    ...
    - *Negative paths and regression traps*
      - In addition to happy paths, a few failure modes should have dedicated tests:
        - Checkout:
          ...
          - Stock races at checkout result in a friendly error and logs/metrics; inventory never goes negative.
        - CMS publish:
