...
- [x] PRIME-QUALITY-01: Regression coverage for onboarding + quests
  - Scope:
  ...
  - **Status (2026-01-23):** COMPLETE
    - Created `GuestProfileStep.test.tsx` (10 tests): save/skip flows, intent selection, interest toggling, socialOptIn derivation
    - Created `SocialOptInStep.test.tsx` (9 tests): activity rendering, empty state fallback, save/skip, activity limit
    - Created `WelcomeHandoffStep.test.tsx` (7 tests): handoff completion, error resilience, disabled state
    - Created `QuestCard.test.tsx` (7 tests): loading, in-progress, all-complete, wait-for-unlock, badges
    - Created `SocialHighlightsCard.test.tsx` (13 tests): empty state guidebook CTA, activity display, intent-based CTAs
