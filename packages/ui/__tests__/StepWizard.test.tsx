...
import {
  ...
  useWizard,
  type WizardStep,
} from "../src/components/organisms/operations/StepWizard/StepWizard";
...
describe("StepWizard", () => {
  ...
  describe("navigation", () => {
    ...
    it("navigates to previous step when clicking Back", async () => {
      // Start on step 2 to test Back button
      render(
        <StepWizard steps={mockSteps} initialStep={1}>
          <StepContent stepId="step1"><div>Step 1 Content</div></StepContent>
          <StepContent stepId="step2"><div>Step 2 Content</div></StepContent>
          <StepContent stepId="step3"><div>Step 3 Content</div></StepContent>
          <StepActions />
        </StepWizard>
      );
    ...
    it("shows Complete button on last step", async () => {
      render(
        <StepWizard steps={mockSteps} initialStep={2}>
          <StepContent stepId="step1"><div>Step 1 Content</div></StepContent>
          <StepContent stepId="step2"><div>Step 2 Content</div></StepContent>
          <StepContent stepId="step3"><div>Step 3 Content</div></StepContent>
          <StepActions />
    ...
    it("calls onComplete when completing wizard", async () => {
      ...
      render(
        <StepWizard steps={mockSteps} initialStep={2} onComplete={onComplete}>
          <StepContent stepId="step1"><div>Step 1 Content</div></StepContent>
          <StepContent stepId="step2"><div>Step 2 Content</div></StepContent>
          <StepContent stepId="step3"><div>Step 3 Content</div></StepContent>
          <StepActions />
  ...
  describe("validation", () => {
    ...
    it("supports async validation", async () => {
      const stepsWithAsyncValidation: WizardStep[] = [
        {
          id: "step1",
          title: "Step 1",
          validate: () => new Promise((resolve) => setTimeout(() => resolve(true), 10)),
        },
        { id: "step2", title: "Step 2" },
      ];
      ...
      // Wait for navigation to complete after async validation
      await waitFor(
        () => {
          expect(screen.getByText("Content 2")).toBeInTheDocument();
        },
