...
export async function POST(request: Request) {
  ...
  const limit = rateLimit({
    key: `pm-login:${requestIp}`,
    windowMs: LOGIN_WINDOW_MS,
    max: LOGIN_MAX_ATTEMPTS,
  });

  if (!limit.allowed) {
    pmLog("warn", "rate_limited", { ip: requestIp });
  ...
  if (!valid) {
    pmLog("warn", "login_failed", { ip: requestIp });
    return withRateHeaders(
  ...
  const sessionToken = await issuePmSession();
  const response = NextResponse.json({ ok: true });
  setPmCookie(response, sessionToken);
