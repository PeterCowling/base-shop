...
const DiffSummaryView = ({ diff }: Props) => {
  ...
  return (
    <div className="rounded border bg-muted/40 p-2 text-sm text-muted-foreground">
      ...
      <span className="ms-2">{diff.modified} modified</span>
      <span className="ms-2">{diff.added} added</span>
      <span className="ms-2">{diff.removed} removed</span>
      {(diff.addedIds?.length ?? 0) > 0 && (
        <div className="mt-1">
          <span className="font-medium">Added:</span>
          <span className="ms-1">{diff.addedIds!.slice(0, 5).join(", ")}{diff.addedIds!.length > 5 ? "…" : ""}</span>
        </div>
      )}
      {(diff.removedIds?.length ?? 0) > 0 && (
        <div className="mt-1">
          <span className="font-medium">Removed:</span>
          <span className="ms-1">{diff.removedIds!.slice(0, 5).join(", ")}{diff.removedIds!.length > 5 ? "…" : ""}</span>
        </div>
      )}
