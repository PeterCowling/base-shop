...
| **MEASURE-00 — Problem framing and ICP** | Intake packet + latest operator learning | Maintains current problem framing context consumed by MARKET-01 and DO | Current problem framing artifact | Prevents market/build work from drifting away from actual customer problem | Feb 12 ✓ | Feb 12 ✓ | Feb 13 ✓ |
...
| DO build dispatch (first-build) | `plan.md` exists with `Status: Active` and build execution has not started | `/lp-do-build <biz>-website-v1-first-build` | active plan with executable tasks at threshold + canonical packet contract from `artifact-registry.md` | task-scoped implementation + validation evidence + `docs/business-os/startup-baselines/<BIZ>-content-packet.md` |
...
| WEBSITE-02 recurring iteration seed | upstream artifact delta detected after WEBSITE launch (offer/channels/logistics/packet sources) | `pnpm --filter scripts startup-loop:map-artifact-delta-to-website-backlog -- --business <BIZ> --changed <path> ...` | changed canonical artifact paths + logistics applicability flag | deterministic WEBSITE backlog seed JSON for `/lp-do-fact-find`/`/lp-do-plan` handover |
