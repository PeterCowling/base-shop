...
- **Acceptance:**
  ...
  - [ ] Consolidated blockers/unknowns table present with IDs, owners, due dates, and source references.
  - [ ] Contradictions (if any) render per TASK-02 policy.
  - [ ] Existing tab behavior and no-overflow guarantees remain intact.
- **Validation contract (VC-XX):**
...
- **Type:** CHECKPOINT
- **Deliverable:** Updated downstream plan evidence via `/lp-replan` after HEAD pilot and S10 prompt enforcement.
- **Execution-Skill:** lp-build
- **Execution-Track:** mixed
- **Effort:** S
- **Status:** Pending
...
- **Acceptance:**
  ...
  - `/lp-replan` run for TASK-10 scope with updated evidence.
...
- **Blocks:** -
- **Confidence:** 70%
  - Implementation: 70% - requires fresh evidence from checkpoint output.
  - Approach: 70% - rollout complexity across PET/BRIK currently estimated, not measured.
