...
```typescript
useEffect(() => {
  // Scans localStorage and retries pending orders
  // ...
}, [saveBreakfastPreorder]); // Dependency causes re-runs
```
...
// Add manual retry trigger if needed
const retryPendingOrders = useCallback(() => {
...
- Manual retry trigger available if needed
- No localStorage scans on every render
