...
TASK-05 unified terminology ("published" → "live") and TASK-06 added content filtering using `isGuideLive()`. Both depend on the GUIDES_INDEX as the status source. This lp-fact-find addresses the remaining gap.
...
- Recommended next step: Proceed to `/lp-plan guide-status-single-source`
