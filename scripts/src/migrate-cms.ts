...
let cliEnv: z.infer<typeof cliEnvSchema>;
try {
  cliEnv = cliEnvSchema.parse(env);
} catch (err) {
...
async function pushSchema(name: string, file: string): Promise<void> {
  const url = `${cliEnv.CMS_SPACE_URL}/schemas/${name}`;
  const body = await readFile(file, "utf8");
  const res = await fetch(url, {
    method: "PUT",
    headers: {
      Authorization: `Bearer ${cliEnv.CMS_ACCESS_TOKEN}`,
