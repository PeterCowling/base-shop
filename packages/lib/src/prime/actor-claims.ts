/**
 * HMAC-SHA256 actor claims signing and verification for Reception → Prime staff requests.
 *
 * This is the single authoritative implementation shared by both apps:
 * - Reception imports `signActorClaims` (sign-only — reception never needs to verify)
 * - Prime functions import both `signActorClaims` and `verifyActorClaims`
 *
 * Both apps previously defined this logic independently. Sharing the implementation
 * turns silent divergence (e.g. field-order change, wrong algorithm) into a build error.
 *
 * Header format: `x-prime-actor-claims: <b64url-payload>.<b64url-sig>`
 *
 * Payload schema: `{ uid: string, roles: string[], iat: number }`
 *   - Fixed field order for canonical serialization — MUST NOT change.
 *
 * Key requirement: `PRIME_ACTOR_CLAIMS_SECRET` MUST be distinct from the gateway
 * access token. Using the gateway token as the signing key would allow any
 * gateway-token holder to mint valid claims for any UID.
 *
 * Minimum secret length: 32 characters.
 *
 * Uses Web Crypto API (`crypto.subtle`) exclusively — compatible with both
 * Next.js server components and Cloudflare Workers.
 */

const ALGORITHM = { name: 'HMAC', hash: 'SHA-256' };

/** Clock-skew window for `verifyActorClaims`: ±5 minutes. */
const CLOCK_SKEW_SECONDS = 300;

/**
 * Verified actor identity returned by `verifyActorClaims`.
 */
 ...
 *
 * IMPORTANT: field order { uid, roles, iat } is load-bearing for HMAC correctness.
 * Changing the order would break all existing signed tokens.
 */
 ...
 * @param secret - Signing secret (min 32 chars); MUST differ from the gateway access token
 * @returns `<b64url-payload>.<b64url-sig>` header value for `x-prime-actor-claims`
 */
