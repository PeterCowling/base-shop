describe("Shop login", () => {
  it("logs in a customer and manages session", function () {
    cy.request("/login").then((resp) => {
      if (!resp.body.includes('name="customerId"')) {
        cy.log("Skipping shop-login spec: legacy shopper login form is not present");
        this.skip();
        return;
      }
  ...
  it("has accessible inputs and validation", function () {
    cy.request("/login").then((resp) => {
      if (!resp.body.includes('name="customerId"')) {
        cy.log("Skipping shop-login spec: legacy shopper login form is not present");
        this.skip();
