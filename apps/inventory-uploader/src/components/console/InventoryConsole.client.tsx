 ...
 */
export default function InventoryConsole({ onHeaderExtra: _onHeaderExtra }: InventoryConsoleProps) {
  const _state = useInventoryConsole();
  ...
  return (
    ...
    <div className="grid gap-6 sm:grid-cols-[320px_1fr]">
      ...
      <aside className="rounded-xl border border-gate-border bg-gate-surface p-4 shadow-elevation-2">
        ...
        <p className="text-sm text-gate-muted">Inventory list loading…</p>
      ...
      <div className="space-y-6">
        ...
        <p className="text-sm text-gate-muted">Select a SKU to view variants.</p>
      </div>
    </div>
  );
}
