...
| **Outputs / artifacts** | WEBSITE-01 framework packet (first build) + canonical `website-content-packet` + DO handover fact-find trigger packet, or updated PDP/listing change log + QA checklist result (recurring). For L1 Build 2 image-first mode: exemplar shot-board + image-heavy launch contract attached to WEBSITE-02 brief. |
