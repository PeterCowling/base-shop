...
const putBodySchema = z
  ...
  function resolveCmsDir(): string {
    ...
    while (true) {
      ...
      if (fsSync.existsSync(candidateDir)) return candidateDir;
      const parent = path.dirname(dir);
      if (parent === dir) break;
      dir = parent;
    }
    return path.resolve(process.cwd(), "data", "cms");
  }

  const CMS_DIR = resolveCmsDir();
  const FILE = path.join(CMS_DIR, "configurator-progress.json");
  const LEGACY_FILE = path.join(CMS_DIR, "wizard-progress.json");
  ...
  async function writeDb(db: DB): Promise<void> {
    ...
    try {
      if (fsSync.existsSync(LEGACY_FILE)) await fs.unlink(LEGACY_FILE);
    } catch {
      /* ignore */
    }
  }
