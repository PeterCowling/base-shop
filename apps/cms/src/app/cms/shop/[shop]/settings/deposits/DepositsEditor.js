...
function DepositsEditor(_a) {
    ...
    var handleChange = function (e) {
        ...
        setState(function (s) {
            var _a;
            return (__assign(__assign({}, s), (_a = {}, _a[name] = Number(value), _a)));
        });
    };
        ...
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0:
                    ...
                    return [4 /*yield*/, (0, shops_server_1.updateDepositService)(shop, fd)];
                case 1:
                    result = _b.sent();
                    if (result.errors) {
                        setErrors(result.errors);
                    }
                    else if ((_a = result.settings) === null || _a === void 0 ? void 0 : _a.depositService) {
    ...
    return (<form onSubmit={onSubmit} className="grid max-w-md gap-4">
      <label className="flex items-center gap-2">
        <shadcn_1.Checkbox name="enabled" checked={state.enabled} onCheckedChange={function (v) {
            return setState(function (s) { return (__assign(__assign({}, s), { enabled: Boolean(v) })); });
        }}/>
        <span>Enable deposit release service</span>
      </label>
      <label className="flex flex-col gap-1">
        <span>Interval (minutes)</span>
        <shadcn_1.Input type="number" name="intervalMinutes" value={state.intervalMinutes} onChange={handleChange}/>
        {errors.intervalMinutes && (<span className="text-sm text-red-600">
