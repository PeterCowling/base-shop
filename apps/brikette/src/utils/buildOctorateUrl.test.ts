...
describe("buildOctorateUrl — NR plan", () => {
  it("returns ok:true and result URL containing checkin/checkout/codice/date/room for NR rate code (double_room)", () => {
...
describe("buildOctorateUrl — validation guards", () => {
  ...
  it("returns ok:false with error invalid_dates when stay is longer than eight nights", () => {
    const result = buildOctorateUrl({
      ...BASE_PARAMS,
      checkin: "2025-07-01",
      checkout: "2025-07-10",
  ...
  it("returns ok:false with error invalid_dates when pax exceeds eight", () => {
    const result = buildOctorateUrl({
      ...BASE_PARAMS,
      pax: 9,
      plan: "nr",
