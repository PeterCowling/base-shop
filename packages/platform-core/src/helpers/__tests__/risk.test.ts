...
describe("helpers/risk", () => {
  ...
  test("extractSessionIdFromCharge resolves from latest_charge.invoice", async () => {
    const { extractSessionIdFromCharge } = require("../risk") as typeof import("../risk");
    const charge = {
      id: "ch_1",
      payment_intent: {
  ...
  test("persistRiskFromCharge calls updateRisk with derived values", async () => {
    ...
    const { persistRiskFromCharge } = require("../risk") as typeof import("../risk");
    const charge = {
      id: "ch_2",
