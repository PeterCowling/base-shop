...
- **Affects:**
  ...
  - `(no longer required)` migration was performed via direct baseline doc edits; ongoing gate is `scripts/src/startup-loop/__tests__/baseline-priors-migration.test.ts`
- **Acceptance:**
...
- `docs/plans/archive/advanced-math-algorithms-fact-find.md`
...
- [x] Review LC-00/LC-01 decisions with startup-loop owners.
- [x] Implement migration gate (LC-00) before enabling runtime writeback.
- [x] Add `prior_refs` fields to S10 experiment spec/readout templates.
- [x] Implement LC-02 through LC-06 in sequence with targeted validations.
- [x] Run LC-07 two-run + correction scenario before rollout.
