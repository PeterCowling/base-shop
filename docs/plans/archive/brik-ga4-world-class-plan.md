...
| GA4-09 | IMPLEMENT | Purchase event via Measurement Protocol | 70% ⚠️ | M | **Carried forward** → `docs/plans/brikette-cta-sales-funnel-ga4/fact-find.md` (Non-goals, deferred) | GA4-01, GA4-02 | - |

> Effort scale: S=1, M=2, L=3 (used for Overall-confidence weighting)

## Active tasks

No active tasks — all implementable tasks complete. GA4-09 **carried forward** to `docs/plans/brikette-cta-sales-funnel-ga4/fact-find.md` (remains deferred/blocked by Octorate one-directional redirect).
...
- **Status:** **Carried forward** → `docs/plans/brikette-cta-sales-funnel-ga4/fact-find.md` (Non-goals section; remains deferred)
- **Type:** IMPLEMENT
- **Confidence:** 70% ⚠️ BELOW THRESHOLD
- **Reason for carry-forward:** This plan is being archived (GA4-01–08 complete). GA4-09 remains blocked by Octorate's one-directional redirect (no callback mechanism). The broader CTA/GA4 conversion pipeline fact-find tracks this as a deferred item with four documented unblock options (webhook, redirect-back, periodic API sync, manual CSV import). Will be re-evaluated when Pete investigates Octorate admin panel.
- **Original acceptance:** `purchase` event sent to GA4 via Measurement Protocol when Octorate confirms a booking, including `transaction_id`, `value`, `currency`, `items`, and correlated `client_id`.
- **Notes / references:**
  - GA4 Measurement Protocol: `POST https://www.google-analytics.com/mp/collect?measurement_id=G-XXX&api_secret=YYY`
  - Existing Octorate integration: `functions/api/octorate/`
  - GA4-01 investigation findings: Octorate integration is one-directional, Options A-D documented
