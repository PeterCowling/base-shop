...
- `market-research-max-age-days=30`
...
Readiness rule:
- If `latest.user.md` is missing, `Status` is not `Active`, or `Last-reviewed` is older than `market-research-max-age-days`, treat market research as stale.

When stale:
1. Stop forecasting.
2. Present a detailed Deep Research prompt to the user using:
   - `docs/business-os/market-research/_templates/deep-research-market-intelligence-prompt.md`
3. Fill placeholders from current intake packet and return a copy-paste prompt block.
4. Resume forecast only after research output is supplied.
...
2. Check market-research freshness for the target business.
3. If missing/stale, present filled Deep Research prompt from template and pause.
4. Ask up to 3 high-impact clarification questions if blockers remain.
5. Freeze an intake packet for this run.
...
1. Run Deep Research using the filled forecast prompt (preferred).
...
- `docs/business-os/market-research/<BIZ>/<YYYY-MM-DD>-market-intelligence.user.md`
- Update: `docs/business-os/market-research/<BIZ>/latest.user.md`
...
7. Continues forecasting despite stale or missing market intelligence pack.
8. `.user.md` outputs were written but corresponding `.user.html` companions were not rendered.
