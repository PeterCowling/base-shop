      ...
      try {
        ...
        for (const [idx, row] of normalizedItems.entries()) {
          ...
          if (!Number.isFinite(qty) || !Number.isInteger(qty) || qty === 0) {
            setError(`Row ${idx + 1}: quantity must be a non-zero integer (negative allowed for reductions).`);
            return;
          }
          if (!row.reason) {
            setError(`Row ${idx + 1}: reason is required.`);
            return;
          }
        ...
        const res = await fetch(`/api/shop/${shop}/stock-adjustments`, {
          method: "POST",
          headers: {
        ...
        const json = (await res.json().catch(() => null)) as AdjustmentResult | null;
        if (!res.ok || !json) {
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground">
            Use this to increase or decrease stock with a required reason. Preview applies validation only. Commit updates inventory and appends an immutable adjustment log.
          </p>
        </div>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="w-[120px]">Qty Δ</TableHead>
              ...
              {items.map((row, idx) => {
                ...
                const delta =
                  Number.isFinite(parsedQty) && matchedInventory
                    ? Math.max(0, matchedInventory.quantity + parsedQty)
                    : null;
                return (
                  <TableRow key={idx}>
                    <TableCell>
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <h3 className="text-sm font-semibold">
                {result.event.id === "dry-run" ? "Preview report" : "Adjustment report"}
              </h3>
              <p className="mt-1 text-sm text-muted-foreground">
                {result.duplicate
                  ? "This idempotency key was already applied — showing the original result."
              ...
              <TableBody>
                {recent.map((event) => (
                  <TableRow key={event.id}>
                    <TableCell className="text-muted-foreground">
                      {new Date(event.adjustedAt).toLocaleString("en-GB", {
                        year: "numeric",
                        month: "2-digit",
                        day: "2-digit",
                        hour: "2-digit",
