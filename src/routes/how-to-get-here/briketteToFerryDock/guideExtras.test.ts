...
describe("createGuideExtras", () => {
  ...
  it("uses localized content and respects explicit toc items", () => {
    setTranslations("it", "guides", {
      content: {
        [GUIDE_KEY]: {
          ...
          toc: [{ href: "#custom", label: "Custom anchor" }],
    ...
    expect(extras.tocItems).toEqual([{ href: "#custom", label: "Custom anchor" }]);
    expect(extras.howToSteps).toEqual(["Step A", "Step B"]);
    expect(extras.labels.onThisPage).toBe("Localized On This Page");
  });

  it("falls back to locale fallback bundles when localized data is empty", () => {
  ...
  it("falls back to English data when both localized bundles are empty", () => {
    ...
    expect(extras.tocItems).toEqual([{ href: "#en", label: "English fallback" }]);
    expect(extras.howToSteps).toEqual(["English steps"]);
