---
name: lp-do-brand-03-promote
description: Brand dossier promotion gate (BRAND-03). Validates completeness of a Draft brand dossier and promotes to Active with operator confirmation. Lightweight gate between BRAND-02 output and lp-design-spec GATE-BD-07.
---

# lp-do-brand-03-promote — Brand Dossier Promotion (BRAND-03)
...
```
/lp-do-brand-03-promote --business <BIZ>
```
...
Does NOT:
- Create or fill brand dossier content (that is BRAND-02)
...
> "Brand dossier not found at `docs/business-os/strategy/<BIZ>/brand-dossier.user.md`. Run `/lp-do-brand-02-brand-identity --business <BIZ>` first."
...
| 4 | Colour Palette | >= 2 rows with HSL values (not TBD) |
| 5 | Typography | >= 1 row with Font Family and Source (not TBD) |
| 6 | Typography font source | Font Family references Google Fonts or system stack |
| 7 | Imagery Direction | >= 2 Do items and >= 2 Don't items |
| 8 | Voice & Tone | Formality and Humour set (not TBD) |
| 9 | Dark mode | Dark mode line present in Token Overrides (may state "inherits base") |
| 10 | Token Overrides | Section present (table may be empty if no overrides needed) |
...
If any fail, list the gaps and recommend `/lp-do-brand-02-brand-identity --business <BIZ>` to fill them.
...
**Upstream (BRAND-02):** Runs after `/lp-do-brand-02-brand-identity` creates a Draft dossier and the operator has reviewed it.
