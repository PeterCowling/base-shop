...
Harden the writer lock system so stale locks from dead subagent processes are recovered automatically, bypass mechanisms are removed from user-facing guidance, and the agent-bin/git guard closes a gap where `SKIP_WRITER_LOCK=1` as an env var prefix is not caught. The error message fix in `require-writer-lock.sh` is already done (see lp-do-fact-find). This plan covers the remaining items.
...
- Implementing shared lock token inheritance (Priority 4 from lp-do-fact-find; not needed yet)
...
- Related brief: `docs/plans/archive/writer-lock-stale-locks-lp-do-fact-find.md`
- Key findings:
...
- **Confidence:** 95%
  ...
  - Approach: 95% — Reasoning-first guidance (explain invariant → decision framework → tools) is well-established in the lp-do-fact-find
...
- 2026-02-07: Plan created from lp-do-fact-find. Error message fix already applied. Scoped to 4 remaining tasks.
