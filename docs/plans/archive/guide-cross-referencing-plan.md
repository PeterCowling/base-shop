...
- Related brief: `docs/plans/guide-cross-referencing-wf-fact-find.md`
- Key findings:
...
**Decision:** Option A chosen because it reduces editor friction and treats cross-references as first-class content feature per wf-fact-find recommendation.
...
- **Acceptance:**
  ...
  - Default scope matches the wf-fact-find baseline (`--locale=en`); script supports `--locale=<lang>` to expand reporting beyond EN
...
- **Test plan:**
  ...
  - Verify: Counts match wf-fact-find numbers (135 with relatedGuides, 30 with %LINK:, 4 with maps)
- **Planning validation:**
...
- **Notes / references:**
  ...
  - Existing: `assistanceKeywords.json` has Google Maps URLs (wf-fact-find line 88)
