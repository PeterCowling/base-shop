...
- Mechanism: `Business-OS-Integration: off` frontmatter field in the controlling lp-do-fact-find/plan document.
...
`docs/business-os/_meta/discovery-index.json` powers zero-argument discovery in `/lp-do-fact-find`, `/lp-do-plan`, and `/lp-do-build`. It can become stale because rebuild is currently described as manual. User-visible symptom: skills present outdated selectable items even though board/API state has moved.
...
- **Validation contract:**
  ...
  - VC-02: transition semantics are consistent across lp-do-fact-find/plan/build/docs.
...
- **Build completion (2026-02-09):**
  - Status: Complete
  - Validation evidence:
    - VC-01/VC-02: verified always-on integration and deterministic transitions across `lp-do-fact-find`, `lp-do-plan`, `lp-do-build`, and workflow guides.
...
- **Rollout / rollback:**
  ...
  - Rollback: hold and `/lp-do-replan` failed areas.
