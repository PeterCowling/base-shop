describe("Shop order flow", { tags: ["smoke"] }, () => {
  ...
  it("browses products, edits cart quantities, and completes an order", function () {
    cy.request("/login").then((resp) => {
      if (!resp.body.includes('name="customerId"')) {
        cy.log("Skipping shop-order spec: legacy shopper login form is not present");
        this.skip();
        return;
      }
  ...
  it("has accessible checkout inputs and validation", function () {
    cy.request("/login").then((resp) => {
      if (!resp.body.includes('name="customerId"')) {
        cy.log("Skipping shop-order spec: legacy shopper login form is not present");
        this.skip();
