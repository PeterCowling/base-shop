...
export async function initShop(): Promise<void> {
  ...
  const vaultFileIndex = argv.indexOf("--vault-file");
  let envFileVars: Record<string, string> = {};
  let envFilePath: string | undefined;
  let vaultFileVars: Record<string, string> = {};
  if (envFileIndex !== -1) {
  ...
  if (vaultFileIndex !== -1) {
    const vaultPath = argv[vaultFileIndex + 1];
    if (!vaultPath) {
      console.error("--vault-file flag requires a path");
      process.exit(1);
    }
    try {
      const raw = readFileSync(vaultPath, "utf8");
      vaultFileVars = JSON.parse(raw) as Record<string, string>;
    } catch (err) {
      console.error("Failed to load vault file:", (err as Error).message);
      process.exit(1);
    }
  }
  ...
  const usedVaultFileKeys = new Set<string>();
  for (const id of selectedPlugins) {
    ...
    for (const key of vars) {
      ...
      if (vaultFileVars[key] !== undefined) {
        envVars[key] = vaultFileVars[key];
        usedVaultFileKeys.add(key);
        continue;
      }
      if (autoEnv) {
  ...
  const unusedVaultFileKeys = Object.keys(vaultFileVars).filter(
    (k) => !usedVaultFileKeys.has(k)
  );
  ...
  writeFileSync(
    join("apps", prefixedId, ".env.template"),
    templateLines.join("\n")
  );
  ...
  if (unusedVaultFileKeys.length) {
    console.warn(
      `Unused variables in vault file: ${unusedVaultFileKeys.join(", ")}`
    );
  }
