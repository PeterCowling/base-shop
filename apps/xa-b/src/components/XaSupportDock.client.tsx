...
import { IconButton, OverlayScrim } from "@ui/components/atoms";
import {
  Drawer,
  DrawerContent,
  DrawerPortal,
  DrawerTitle,
  DrawerTrigger,
} from "@ui/components/atoms/primitives/drawer";
...
const XaFaqOverlayContent = dynamic(
  () => import("./XaFaqOverlayContent.client").then((m) => m.XaFaqOverlayContent),
  {
    ssr: false,
    loading: () => (
      <div className="p-4 text-sm text-muted-foreground">Loading…</div> // i18n-exempt -- XA-0014: loading state
    ),
  },
);
