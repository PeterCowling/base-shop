...
- [ ] TASK-02: Wire hook script into `scripts/git-hooks/pre-commit.sh`
- [x] TASK-03: Add `--check` mode and `runCheck()` export to `generate-process-improvements.ts` — Complete (2026-02-26)
- [ ] TASK-04: Unit tests for `--check` mode in `generate-process-improvements.test.ts`
...
| TASK-02 | IMPLEMENT | Wire hook into `pre-commit.sh` | 90% | S | Pending | TASK-01 | - |
| TASK-03 | IMPLEMENT | Add `--check` mode + `runCheck()` to generator | 85% | M | Complete (2026-02-26) | - | TASK-04 |
| TASK-04 | IMPLEMENT | Unit tests for `--check` mode | 85% | M | Pending | TASK-03 | - |
