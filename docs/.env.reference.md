...
| `OCTORATE_CLIENT_ID` | Client ID for Octorate Connect API OAuth 2.0 flow. Required when `NEXT_PUBLIC_OCTORATE_LIVE_AVAILABILITY=1`. Server-only — never expose to the browser. |
| `OCTORATE_CLIENT_SECRET` | Client secret for Octorate Connect API OAuth 2.0 flow. Required when `NEXT_PUBLIC_OCTORATE_LIVE_AVAILABILITY=1`. Server-only — never expose to the browser. |
| `NEXT_PUBLIC_OCTORATE_LIVE_AVAILABILITY` | Set to `1` to enable per-room live availability queries via the Octorate Connect ARI API. Defaults to disabled (omit or set to `0`). **Build-time inlined by Next.js — toggling this value requires a new build and deploy.** |
