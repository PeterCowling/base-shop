...
export default function useCanvasResize({ componentId, widthKey, heightKey, widthVal, heightVal, dispatch, gridEnabled = false, gridCols, containerRef, disabled = false, }) {
    ...
    const { guides, setGuides, siblingEdgesRef, computeSiblingEdges } = useGuides(containerRef);
    const [distances, setDistances] = useState({ x: null, y: null });
    useEffect(() => {
        if (!resizing)
            return;
        const handleMove = (e) => {
    ...
    const startResize = (e) => {
        ...
        const startHeight = heightVal && heightVal.endsWith("px")
