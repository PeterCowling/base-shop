...
## Key Distinction: lp-do-factcheck vs lp-do-fact-find

| Aspect | `/lp-do-fact-find` | `/lp-do-factcheck` |
|--------|-------------|---------------|
...
**lp-do-fact-find:** "What do we need to know to build this?"
**lp-do-factcheck:** "Is what we wrote still true? Fix it if not."
...
| After lp-do-factcheck... | Consider... |
|---------------------|-------------|
| Many fixes made | User reviews changes, commits |
| Architecture docs heavily outdated | `/lp-do-fact-find` to create fresh briefing |
| Plan doc inaccurate | `/lp-do-replan` to update the plan |
| Obsolete sections flagged | User decides to remove or archive |

## Quality Checks

Before completing the lp-do-factcheck:
...
>
> Run `/lp-do-factcheck <path>` again without `report-only` to apply fixes."
