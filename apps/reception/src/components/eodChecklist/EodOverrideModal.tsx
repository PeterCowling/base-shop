...
interface EodOverrideModalProps {
  onConfirm: (signoff: EodOverrideSignoff) => void;
  onCancel: () => void;
}

function EodOverrideModalBase({ onConfirm, onCancel }: EodOverrideModalProps) {
  ...
  const handleSubmit = useCallback(
    async (event?: FormEvent) => {
      ...
      onConfirm({
        overrideManagerName: managerName,
        overrideManagerUid: managerUid,
        overrideReason: reason.trim(),
      });
    },
    [email, password, reason, isSubmitting, onConfirm]
  );
    ...
    <ModalContainer widthClasses="w-[26rem]">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-3">
        ...
        <Textarea
          ...
          data-cy="eod-override-reason"
        />
        {error && (
          <div
            className="text-sm text-error-main"
            role="alert"
            data-cy="eod-override-error"
          >
            {error}
          </div>
        )}
...
EodOverrideModalBase.displayName = "EodOverrideModalBase";

export default withModalBackground(memo(EodOverrideModalBase));
