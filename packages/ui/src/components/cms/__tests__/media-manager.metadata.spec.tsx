...
describe("MediaManager — metadata", () => {
  ...
  it("saves metadata, updates list, shows toast, keeps panel open", async () => {
  ...
  it("re-enables form and shows error toast when save fails", async () => {
    const user = userEvent.setup();
    ...
    expect(
      await screen.findByText("Failed to update media metadata.")
    ).toBeInTheDocument();
