---
name: lp-do-discovery-03-option-picking
description: Option selection gate for new startups (DISCOVERY-03). Reads solution-space research results, builds an evaluation matrix, and produces a shortlist of 1-2 options with elimination rationale. Explicit kill gate — decision record required to continue to DISCOVERY-04.
---

# lp-do-discovery-03-option-picking — Option Picking (DISCOVERY-03)

Reads solution-space research results from the operator's deep research run, evaluates options against feasibility criteria, and produces a decision record with a shortlist of 1–2 viable options and elimination rationale for all others. An explicit decision record is required to continue to DISCOVERY-04 (naming handoff).

## Invocation

```
/lp-do-discovery-03-option-picking --business <BIZ>
```
...
- Applies a kill gate — if no viable option exists, stops the operator before DISCOVERY-04

## Required Inputs

Required:
- `docs/business-os/strategy/<BIZ>/<YYYY-MM-DD>-solution-space-results.user.md` — operator-filled research output from `/lp-do-discovery-02-solution-space-scan`

If no results file exists, stop: instruct the operator to complete the deep research run from `/lp-do-discovery-02-solution-space-scan` first.
...
From remaining options, select 1–2 to carry forward to DISCOVERY-04. If two options remain and both are genuinely viable, include both — the naming research phase can handle parallel tracks. Do not reduce to one arbitrarily.
...
> Explicit decision record required to continue to DISCOVERY-04. No viable option identified from current solution-space results. Do not advance to DISCOVERY-04 (naming handoff). Options:
> 1. Re-run `/lp-do-discovery-02-solution-space-scan --business <BIZ>` with a refined problem scope or broader option set
> 2. Return to `/lp-problem-frame --business <BIZ>` to reassess the problem statement

Write the kill gate output to the artifact and stop. Do not proceed to DISCOVERY-04.
...
[If GO: "Proceed to `/lp-do-discovery-04-business-name-options --business <BIZ>` with the option(s) above."]
[If NO-GO: Kill gate language — see Step 5.]
```

**Downstream consumer:** `/lp-do-discovery-04-business-name-options --business <BIZ>` (DISCOVERY-04) reads this file to scope the naming research to the selected option(s).
...
- Kill gate suppressed — if no viable option exists, the gate MUST fire; do not advance the operator to DISCOVERY-04

## Integration

**Upstream (DISCOVERY-02):** Reads `docs/business-os/strategy/<BIZ>/<YYYY-MM-DD>-solution-space-results.user.md` from the operator's deep research run via `/lp-do-discovery-02-solution-space-scan`.

**Downstream (DISCOVERY-04):** `/lp-do-discovery-04-business-name-options --business <BIZ>` reads `s0c-option-select.user.md` to scope naming research to the shortlisted option(s). The naming skill should be run with the shortlisted option names provided as context.
