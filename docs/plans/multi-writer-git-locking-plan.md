...
The current workflow relies on a single writer lock to serialize commits/pushes in a shared checkout. This prevents some lost-work incidents, but it does not scale: stale locks are inevitable, and a repo-wide lock blocks unrelated work.

This plan replaces “one checkout = one writer” with a multi-writer model that supports many concurrent agents in a single checkout without worktrees:

- Lease-based locks scoped to paths/resources (TTL + renew + deterministic expiry).
- Path-tree lock store (not hashed IDs) so hierarchical conflicts are fast and readable.
- Per-agent Git index (`GIT_INDEX_FILE`) so staging and tree creation do not contend on `.git/index.lock`.
- Branchless commits using Git plumbing (`read-tree` / `update-index` / `write-tree` / `commit-tree` + `update-ref` CAS) so agents can publish concurrently to separate branches without checking out branches.
- Isolated caches/outputs per agent (R2) so local checks and builds are deterministic and do not trample shared artifacts.
...
This is a hybrid coordination strategy:

- Default: optimistic collaboration is permitted at edit-time (humans can edit; agents can prepare patches).
- At publish-time: leases are required for any path/resource an agent wants to publish.
...
- Keep Git safety intact (no destructive commands; PR workflow remains canonical).
- Preserve compatibility with existing GitHub automation: push `work/**` → Auto PR → Merge Gate → merge.

## Non-goals

- Mandatory OS-level file locking (non-portable, unreliable across editors).
- Preventing humans from editing files in editors at edit-time (enforcement is at publish time).
- Distributed locking (Redis/D1/etc.) in v1; v1 targets one machine / shared checkout.
- Replacing GitHub Merge Gate; this plan integrates with it.
...
- Assumptions:
  ...
  - Humans may bypass wrappers; enforcement will still occur at publish time via git hooks (for standard commits) and via wrapper requirements for agent publishes.
...
- Related brief: none (this plan is based on observed repo behavior + design constraints).
- Key repo findings:
  - Current writer lock is global: `scripts/git/writer-lock.sh`.
  - Git hooks enforce writer lock: `scripts/git-hooks/require-writer-lock.sh` + `package.json` hooks.
  - Current model produces “stale-but-alive” locks (abandoned shells/processes).

## Existing System Notes

### Why file-level locking isn’t enforced by Git alone

- File locks are advisory unless every writer cooperates.
- Git hooks trigger at commit/push, not at edit-time.

Therefore:

- We can enforce “no unauthorized publish” reliably.
- We can only enforce “no unauthorized edit” for agents that use our tooling.
...
- Repo-wide writer lock: serializes all publishes.

## Proposed Approach

### Option C2 (Chosen): Index-only publish + isolated caches

Agents do not need to rely on the shared working tree for correctness. They publish using:
...
The shared working tree may still be used as a convenience surface for humans, but agents must treat it as non-authoritative.
...
We lock publish rights, not edit rights (edit can be concurrent; publish must be coordinated).
...
- Resource leases for shared hotspots (examples):
  ...
  - `resource/repo-checks` (optional umbrella for whole-repo operations)
  - `resource/build:<app>` (optional when build outputs cannot be isolated)

### Conflict rules

Minimal, scalable rules:
...
- Resources are their own namespace and conflict only by exact resource key.
...
${common_dir}/base-shop-locks/v1/
  ...
  audit/
    events.ndjson
  .mutex/   (a short-lived mutex for multi-path atomic acquisition)
```
...
Agents often need to claim multiple paths. To avoid partial acquisition deadlocks, `acquire` uses a short-lived global mutex (`.mutex`) only for lock-table operations:
...
A single CLI is the lock “control plane”:
...
- No forbidden paths are being modified (e.g. protected branches, `.env.local` patterns if applicable).
- If hybrid OCC is later added: verify base blob OIDs match before applying.

### Policy enforcement parity (what runs locally vs CI)
...
- env/secrets guard (equivalent of `scripts/git-hooks/pre-commit-check-env.sh`)
...
- Merge Gate remains the authoritative merge check.
...
If an agent does write to the working tree (for convenience), it must:
...
|---|---|---|---:|---:|---|---|
| MWGIT-00 | DECISION | Confirm v1 is C2+L1+R2; define “hot resources” + branch lease rules | 80% | S | Pending | - |
| MWGIT-01 | INVESTIGATE | Audit current hooks/scripts for wrapper parity + dangerous shared writers | 85% | S | Pending | - |
...
| MWGIT-08 | IMPLEMENT | Deprecate repo-wide writer lock; keep as emergency fallback only | 75% | S | Pending | MWGIT-05 |

> Effort scale: S=1, M=2, L=3.

## Detailed Tasks

### MWGIT-00: Decision capture for v1 policy (C2 + L1 + R2)
...
- **Scope:**
  ...
  - Define initial “hot resources” list:
    ...
    - optional: `resource/repo-checks` (for global tooling)
- **Acceptance:**
  - Decision recorded + initial resource list defined.
...
- **Acceptance outputs:**
  - Compatibility matrix:
    - what existing checks must be replicated in wrapper
...
- **What would make this ≥90%:**
  - Stress test results (high-concurrency acquire/renew/release loops) captured in docs.
...
- **Type:** IMPLEMENT
- **Confidence:** 60% ⚠️ (below build threshold until spike + tests)
...
- **Wrapper contract:**
  ...
  - Safety checks: env guard + protected ref rules + lock ownership.
- **Acceptance:**
...
### MWGIT-08: Deprecate repo-wide writer lock

- **Type:** IMPLEMENT
- **Confidence:** 75%
  - Implementation: 80% — Mechanical changes with a rollback path.
  - Approach: 75% — Reduces bottleneck; requires confidence in wrapper + tests first.
  - Impact: 75% — Must avoid breaking human workflows; staged rollout needed.
- **Acceptance:**
  - `require-writer-lock.sh` removed from default hooks or made warn-only.
  - Writer lock retained only as emergency tool, documented as such.

## Risks & Mitigations

- Risk: Agents bypass wrapper and publish without leases.
  - Mitigation: keep pre-commit hook enforcement for humans; agents are required to use wrapper; optionally add server-side checks (Merge Gate validates that agent commits include lock audit marker).
- Risk: Shared working tree still creates noise.
  - Mitigation: C2 reduces dependency on working tree; isolate caches; avoid global tooling without resource leases.
- Risk: Lock hoarding.
  - Mitigation: max total lifetime + TTL renew policy + steal logging.
- Risk: Clock skew.
  - Mitigation: single-host assumption; conservative TTL; log lease timing; avoid relying on wall clock across machines.
- Risk: Complexity and subtle Git plumbing bugs.
  - Mitigation: MWGIT-04 spike + MWGIT-07 integration harness before turning off writer lock.
...
- [ ] Stale locks are deterministically expired by TTL and auto-cleaned without humans.
- [ ] Multiple agents can publish concurrently to different branches from one checkout without worktrees.
- [ ] Publish-time enforcement prevents unauthorized paths/resources from landing.
- [ ] Operational playbook exists: who owns a lease, when it expires, how to wait, how to steal (audited).

## Decision Log

- 2026-02-01: Draft plan revised; target v1 policy is C2 + L1 + R2.
