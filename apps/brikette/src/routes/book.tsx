...
export async function clientLoader({ params }: LoaderFunctionArgs) {
  ...
  const ns = "bookPage";
  await preloadNamespacesWithFallback(lang, [ns], { optional: true });
  await preloadI18nNamespaces(lang, ["translation"], { optional: true });
  await i18n.changeLanguage(lang);
  const meta = resolveI18nMeta(lang, ns);
  return { lang, title: meta.title, desc: meta.description } satisfies LoaderOut;
}
...
export default function BookRoute() {
  const { lang } = useLoaderData() as LoaderOut;
  ...
  const checkin = search.get("checkin") ?? getTodayIso();
  const checkout = search.get("checkout") ?? getDatePlusTwoDays(checkin);
  const sku = search.get("sku");
  const adults = parseIntSafe(search.get("adults"), defaultAdultsForSku(sku));
  const children = parseIntSafe(search.get("children"), 0);

  // Pricing and availability are determined on Octorate during confirm preflight.

  // Head handled by meta()/links(); avoid inline head

  const octorateHref = useMemo(
    () => buildOctorateLink(checkin, checkout, adults, children, search),
    [checkin, checkout, adults, children, search]
  );
  ...
  async function handleConfirmClick(
    ...
    const childrenStr = search.get("children");
    const childrenAges = search.get("childrenAges");
  ...
  return (
    <Section padding="default" className="mx-auto max-w-4xl">
      ...
      <div className="mt-6 text-sm">
        <Link to={`/${lang}/${getSlug("rooms", lang)}`} className="inline-flex min-h-10 min-w-10 items-center underline">{t("backToRooms")}</Link>
      </div>
    </Section>
  );
}
...
export const meta: MetaFunction = ({ data }) => {
  ...
  const path = `/${lang}/${getSlug("book", lang)}`;
