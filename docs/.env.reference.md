...
| `INVENTORY_AUTHORITY_URL` | Base URL for the inventory authority API (used by Workers to validate inventory against Node). |
| `INVENTORY_AUTHORITY_TOKEN` | Service token required by `/api/inventory/validate` to authorize inventory authority requests. |
...
| `PAYMENTS_PROVIDER` | Payment gateway to use (`stripe` or `axerve`). When unset, no payment provider is initialised. |
...
| `AXERVE_SHOP_LOGIN` | Axerve/GestPay shop login identifier. Required when `PAYMENTS_PROVIDER=axerve`. |
| `AXERVE_API_KEY` | Axerve/GestPay API key for the commercial agreement. Required when `PAYMENTS_PROVIDER=axerve`. |
| `AXERVE_SANDBOX` | Set to `true` to route Axerve calls to the sandbox endpoint instead of production. Defaults to `false` — omitting this variable routes to the live endpoint. |
...
| `DISABLE_EXPIRED_INVENTORY_HOLD_RELEASE_SERVICE` | Set to `true` to disable automatic release of expired inventory holds. Service is enabled by default and runs every 60 seconds. |
...
| `NEXT_PUBLIC_OCTORATE_LIVE_AVAILABILITY` | Set to `1` to enable per-room live availability queries via the public Octobook HTML endpoint. Defaults to disabled (omit or set to `0`). **Build-time inlined by Next.js — toggling this value requires a new build and deploy.** |
| `CARYINA_ADMIN_KEY` | Secret key for the Caryina admin UI. Must be ≥ 32 random characters. Required in `.env.local` for local dev and as a Cloudflare Worker secret (`wrangler secret put CARYINA_ADMIN_KEY`) for production. Middleware returns 500 for all `/admin/*` routes when unset. |
| `CRON_SECRET` | Bearer token secret for protected cron endpoints (including Caryina checkout reconciliation at `/api/cron/checkout-reconciliation`). Must be set as a runtime secret in deployed environments. |
| `MERCHANT_NOTIFY_EMAIL` | Email address that receives order notification emails after each successful payment in Caryina. Optional — defaults to `peter.cowling1976@gmail.com` when unset. Requires `EMAIL_PROVIDER`, `GMAIL_USER`, and `GMAIL_PASS` to be set for delivery to work. |
| `CARYINA_CHECKOUT_AUTO_RECONCILE` | Caryina-only fail-safe toggle for `POST /api/checkout-session`. Set to `1` to run stale checkout reconciliation after hard checkout exceptions; leave unset (or set `0`) to disable automatic reconciliation. |

### Shop-specific admin keys

Some shop apps gate their admin UI behind an env-var-based key (e.g. `CARYINA_ADMIN_KEY`). These all follow the same contract:

- Must be ≥ 32 random characters.
- Required in `apps/<shop>/.env.local` for local development.
- Must be provisioned as a Cloudflare Worker secret before the admin UI is usable in any deployed environment:
  ```bash
  wrangler secret put <SHOP>_ADMIN_KEY
  ```
- The middleware returns 500 when the key is absent — the admin UI is intentionally unavailable, not silently open.

When adding a new shop app with admin auth, add its key to the table above.
