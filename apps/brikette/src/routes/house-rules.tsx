...
export default memo(function HouseRulesPage() {
  const { lang, title: loaderTitle, desc: loaderDesc } = useLoaderData() as {
    lang: AppLanguage;
    title?: string;
    desc?: string;
  };
  const { t } = useTranslation("houseRulesPage", { lng: lang });
  ...
  const fallbackHeadDescriptors = (() => {
    ...
    const fallbackMeta = resolveI18nMeta(lang, "houseRulesPage");
    const path = `/${lang}/${getSlug("houseRules", lang)}`;
