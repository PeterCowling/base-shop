...
async function getRepo() {
    if (!repoPromise) {
        repoPromise = (0, repoResolver_1.resolveRepo)(() => db_1.prisma.setting, () => Promise.resolve().then(() => __importStar(require("./settings.prisma.server"))).then((m) => m.prismaSettingsRepository), () => Promise.resolve().then(() => __importStar(require("./settings.json.server"))).then((m) => m.jsonSettingsRepository), { backendEnvVar: "SETTINGS_BACKEND" });
    }
    return repoPromise;
}
