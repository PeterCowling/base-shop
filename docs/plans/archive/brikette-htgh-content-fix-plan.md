...
- [x] `content-modules.ts` uses dynamic imports with `webpackInclude` ✅ (verified 2026-01-23)
- [x] `content.ts` throws developer error on missing content ✅ (verified 2026-01-23)
- [x] Regression test exists and validates all locales × all contentKeys ✅ (`content-files.test.ts`)
- [x] All 18 locales × 24 routes have content files (432 locale files present) ✅
- [x] Old `splitRouteModules` / `webpackContext` / `HAS_LOCALE_CONTENT` patterns removed ✅
- [ ] `pnpm --filter @apps/brikette build` completes without HTGH error (blocked by unrelated prerender issues)

## Implementation Verification (2026-01-23)

All code changes are complete and verified:
- `content-modules.ts`: dynamic `import()` with `webpackInclude` regex
- `content.ts`: throws with `lang`, `fallback`, `contentKey` context on missing content
- `content-files.test.ts`: validates every contentKey in `routes.json` has a JSON file per locale
- 18 real locales × 24 routes = 432 content files, all present
- `webpackGlob.ts` still exists but is NOT used by how-to-get-here code (used by guides, breakfast-menu, locale-loader)
- Build validation deferred: app has other prerender blockers unrelated to HTGH
