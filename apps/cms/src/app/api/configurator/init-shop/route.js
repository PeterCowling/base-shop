"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g = Object.create((typeof Iterator === "function" ? Iterator : Object).prototype);
    return g.next = verb(0), g["throw"] = verb(1), g["return"] = verb(2), typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.POST = POST;
require("@acme/lib/initZod");
var options_1 = require("@cms/auth/options");
var next_auth_1 = require("next-auth");
var server_1 = require("next/server");
var node_fs_1 = require("node:fs");
var node_path_1 = require("node:path");
var dataRoot_1 = require("@platform-core/dataRoot");
var shops_1 = require("@platform-core/shops");
var zod_1 = require("zod");
var _shared_utils_1 = require("@shared-utils");
/**
 * POST /cms/api/configurator/init-shop
 * Body: { id: string; csv?: string; categories?: string[] }
 * Seeds categories and product CSV for a shop.
 */
function POST(req) {
    return __awaiter(this, void 0, void 0, function () {
        var session, schema, parsed, _a, id, csv, categories, dir, buf, err_1;
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0: return [4 /*yield*/, (0, next_auth_1.getServerSession)(options_1.authOptions)];
                case 1:
                    session = _b.sent();
                    if (!session || !["admin", "ShopAdmin"].includes(session.user.role)) {
                        return [2 /*return*/, server_1.NextResponse.json({ error: "Forbidden" }, { status: 403 })];
                    }
                    _b.label = 2;
                case 2:
                    _b.trys.push([2, 9, , 10]);
                    schema = zod_1.z
                        .object({
                        id: zod_1.z
                            .string()
                            .transform(function (s) { return (0, shops_1.validateShopName)(s); }),
                        csv: zod_1.z
                            .string()
                            .optional()
                            .transform(function (s) { return s.replace(/\s+/g, ""); })
                            .refine(function (val) {
                            if (!val)
                                return true;
                            try {
                                return (Buffer.from(val, "base64").toString("base64").replace(/=+$/, "") ===
                                    val.replace(/=+$/, ""));
                            }
                            catch (_a) {
                                return false;
                            }
                        }, { message: "Invalid CSV encoding" }),
                        categories: zod_1.z.array(zod_1.z.string()).optional(),
                    })
                        .strict();
                    return [4 /*yield*/, (0, _shared_utils_1.parseJsonBody)(req, schema, "1mb")];
                case 3:
                    parsed = _b.sent();
                    if (!parsed.success)
                        return [2 /*return*/, parsed.response];
                    _a = parsed.data, id = _a.id, csv = _a.csv, categories = _a.categories;
                    dir = node_path_1.default.join((0, dataRoot_1.resolveDataRoot)(), id);
                    return [4 /*yield*/, node_fs_1.promises.mkdir(dir, { recursive: true })];
                case 4:
                    _b.sent();
                    if (!csv) return [3 /*break*/, 6];
                    buf = Buffer.from(csv, "base64");
                    return [4 /*yield*/, node_fs_1.promises.writeFile(node_path_1.default.join(dir, "products.csv"), buf)];
                case 5:
                    _b.sent();
                    _b.label = 6;
                case 6:
                    if (!categories) return [3 /*break*/, 8];
                    return [4 /*yield*/, node_fs_1.promises.writeFile(node_path_1.default.join(dir, "categories.json"), JSON.stringify(categories, null, 2), "utf8")];
                case 7:
                    _b.sent();
                    _b.label = 8;
                case 8: return [2 /*return*/, server_1.NextResponse.json({ success: true })];
                case 9:
                    err_1 = _b.sent();
                    return [2 /*return*/, server_1.NextResponse.json({ error: err_1.message }, { status: 400 })];
                case 10: return [2 /*return*/];
            }
        });
    });
}
