...
|---|---|---|---|
| `current` | Baseline snapshot set used to seed the current run | ASSESSMENT-09, S4 | ASSESSMENT-09 promotes from previous `next_seed`; S4 writes run-authoritative merge |
...
1. **Run start (ASSESSMENT-09):** Initialize `current` from previous run `next_seed`.
...
5. **Next run (ASSESSMENT-09):** Loader promotes previous `next_seed` to new run `current`.
...
- **ASSESSMENT-09:** Copies previous `next_seed` to new run `current` (read-only operation on previous manifest).
