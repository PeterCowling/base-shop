...
function CurrencyTaxEditor(_a) {
    ...
    var handleChange = function (e) {
        ...
        setState(function (s) {
            var _a;
            return (__assign(__assign({}, s), (_a = {}, _a[name] = value, _a)));
        });
    };
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    ...
                    return [4 /*yield*/, (0, shops_server_1.updateCurrencyAndTax)(shop, fd)];
                case 1:
                    result = _a.sent();
                    if (result.errors) {
    ...
    return (<form onSubmit={onSubmit} className="grid max-w-md gap-4">
      <label className="flex flex-col gap-1">
        <span>Currency</span>
        <shadcn_1.Input name="currency" value={state.currency} onChange={handleChange}/>
        {errors.currency && (<span className="text-sm text-red-600">
            {errors.currency.join("; ")}
          </span>)}
      </label>
      <label className="flex flex-col gap-1">
        <span>Tax Region</span>
        <shadcn_1.Input name="taxRegion" value={state.taxRegion} onChange={handleChange}/>
        {errors.taxRegion && (<span className="text-sm text-red-600">
