---
name: lp-do-discovery-02-solution-space-scan
description: Solution space scanning for new startups (DISCOVERY-02). Produces a structured deep-research-ready prompt covering 5-10 candidate product-type options with feasibility flags. Feasibility only — no demand scoring until S2.
---

# lp-do-discovery-02-solution-space-scan — Solution Space Scan (DISCOVERY-02)
...
```
/lp-do-discovery-02-solution-space-scan --business <BIZ>
```
...
This skill:
- Reads the problem statement artifact from DISCOVERY-01
...
The operator runs the compiled prompt in an external research tool. The results file from that run is the input to `/lp-do-discovery-03-option-picking`.

## Required Inputs (pre-flight)

Required:
- `docs/business-os/strategy/<BIZ>/problem-statement.user.md` — output from `/lp-do-discovery-01-problem-framing`

If this file is missing, stop and instruct the operator to run `/lp-do-discovery-01-problem-framing --business <BIZ>` first.
...
2. Run `/lp-do-discovery-03-option-picking --business <BIZ>` — it reads the results file as its primary input.

Do not skip the results file. `/lp-do-discovery-03-option-picking` requires it.
...
- [ ] Handoff note section present in prompt file pointing to `/lp-do-discovery-03-option-picking`

## Completion Message

> Research prompt saved to `docs/business-os/strategy/<BIZ>/<YYYY-MM-DD>-solution-space-prompt.md`.
>
> Drop this prompt into a deep research tool (OpenAI Deep Research or equivalent). When complete, save the full output to `docs/business-os/strategy/<BIZ>/<YYYY-MM-DD>-solution-space-results.user.md` (same date), then run `/lp-do-discovery-03-option-picking --business <BIZ>`.

## Integration

**Upstream (DISCOVERY-01):** Reads `docs/business-os/strategy/<BIZ>/problem-statement.user.md` from `/lp-do-discovery-01-problem-framing`.

**Downstream (DISCOVERY-03):** `/lp-do-discovery-03-option-picking --business <BIZ>` reads the results artifact filled by the operator after the research run.

**Note on S2 boundary:** DISCOVERY-02 establishes feasibility signals only. Demand scoring, willingness-to-pay assessment, and market size estimation all belong to S2 (Market Intelligence). Do not blur this boundary in the prompt or in guidance to the operator.
