...
- Related brief: `docs/plans/business-os-unified-writes-lp-do-fact-find.md`
- Key findings:
...
|-------|--------|----------------|
| **lp-do-fact-find** | Added `Feature-Slug` to card frontmatter during BOS integration | TASK-02 must support `Feature-Slug` field in POST/PATCH |
| **lp-do-fact-find** | Creates fact-finding stage doc via BOS integration | TASK-02b (stage-docs API) required before TASK-11 |
| **lp-do-plan** | Reads `Feature-Slug` from card (Slug Stability rule) | API must return `Feature-Slug` in GET responses |
| **lp-do-plan** | Added Test Foundation Check | No direct impact (documentation change) |
| **lp-do-build** | Automatic `Planned → In progress` lane transition on first task | TASK-10 must handle lane PATCH on build start |
| **lp-do-build** | Creates `build.user.md` stage doc on first task | TASK-10 depends on TASK-02b |
| **lp-do-build** | Updates `Last-Progress` field after each task | TASK-02 must support `Last-Progress` field in PATCH |
...
| TASK-10 | IMPLEMENT | Migrate `/lp-do-build` skill to API writes | 82% | M | Complete (2026-02-02) | TASK-08, TASK-02b |
| TASK-11 | IMPLEMENT | Migrate remaining skills (`/lp-do-fact-find`, etc.) | 82% | M | Complete (2026-02-02) | TASK-08 |
...
- **Acceptance:**
  ...
  - Request/response schemas match existing `/api/cards` endpoints **plus skill-required fields:**
    - `Feature-Slug` (string, optional) — for slug stability across lp-do-fact-find → plan → build
    - `Last-Progress` (date string, optional) — for lp-do-build progress tracking
...
- **Confidence:** 82%
  ...
  - Impact: 80% — new endpoint; needed for `/lp-do-fact-find` skill migration
- **Acceptance:**
...
- **What would make this ≥90%:** Test with `/lp-do-fact-find` skill creating stage doc
- **Rollout / rollback:**
  - Rollout: Deploy endpoint
  - Rollback: Remove endpoint; skills fail with clear error (fail-closed)
- **Documentation impact:**
...
- **Confidence:** 82%
  ...
  - Approach: 85% — cursor-based delta is specified in lp-do-fact-find
...
- **Planning validation:**
  ...
  - Test stubs written: N/A (M effort)
  - Unexpected findings: Need to handle `.agent.md` derivation rules from lp-do-fact-find
...
- **Documentation impact:**
  - None
- **Notes / references:**
  - Serialization rules: These canonicalization rules supersede any conflicting guidance in lp-do-fact-find
...
### TASK-10: Migrate `/lp-do-build` skill to API writes
...
- **Validation:**
  - Not run: tests (documentation-only change)
- **Documentation updated:** `.claude/skills/lp-do-build/SKILL.md`
...
- **Test contract:**
  ...
  - **Run:** Invoke `/lp-do-build` on test plan; verify API calls in D1 audit log
- **Planning validation:**
  - Tests run: Reviewed skill structure
  - Test stubs written: N/A (M effort)
...
- **Documentation impact:**
  - None
- **Notes / references:**
  - Skill: `.claude/skills/lp-do-build/SKILL.md`

### TASK-11: Migrate remaining skills (`/lp-do-fact-find`, `/idea-advance`, etc.)
...
- **Type:** IMPLEMENT
- **Affects:**
  - `.claude/skills/lp-do-fact-find/SKILL.md` (creates stage docs, cards)
  - `.claude/skills/lp-do-plan/SKILL.md` (creates/updates cards, stage docs)
...
- **Acceptance:**
  ...
  - **lp-do-fact-find BOS integration:**
...
- **Test contract:**
  - **TC-01:** lp-do-fact-find allocates ID via API → no scan-based allocation
    - Invoke `/lp-do-fact-find` with Business-Unit → calls POST `/api/agent/allocate-id` → returns valid ID
  - **TC-02:** lp-do-fact-find creates card via API → card visible in UI
    - Invoke `/lp-do-fact-find` → POST `/api/agent/cards` with `Feature-Slug` → card appears in UI
  - **TC-03:** lp-do-fact-find creates stage doc via API → stage doc visible
    - Invoke `/lp-do-fact-find` → POST `/api/agent/stage-docs` → fact-finding stage doc created
  - **TC-04:** lp-do-plan reads Feature-Slug from card → not re-derived
    - Card has `Feature-Slug: my-feature` → plan uses `my-featulp-do-replan.md` (not derived from title)
...
- **Rollout / rollback:**
  - Rollout: Update skills incrementally (lp-do-fact-find first, then lp-do-plan, then idea-advance)
...
- **Investigation performed:**
  ...
  - Found 8 skills total: lp-do-build, lp-do-plan, lp-do-fact-find, ops-inbox, biz-update-people, idea-develop, idea-scan, biz-update-plan
...
- 2026-02-02 (lp-do-replan): Skill migration scope — Skills writing to scans/, strategy/, people/ excluded from D1 migration. These paths added to CI guard allowlist. Rationale: These are reference/metadata paths, not core business entities; adding API endpoints would expand scope without proportional benefit.
- 2026-02-02 (skill review): API schema expansion — Added `Feature-Slug`, `Last-Progress`, `Plan-Link` fields to TASK-02 acceptance criteria. These fields are required by updated skill BOS integration: lp-do-fact-find sets Feature-Slug on card creation; lp-do-build updates Last-Progress and performs lane transitions; lp-do-plan reads Feature-Slug for slug stability.
- 2026-02-02 (skill review): lp-do-build dependencies — Added TASK-02b dependency to TASK-10. lp-do-build creates build stage docs on first task start, requiring the stage-docs API endpoint.
- 2026-02-02 (lp-do-replan TDD): Test contract compliance — Added TC-XX enumeration to all IMPLEMENT tasks per revised `/lp-do-replan` TDD Compliance Requirement. Tasks TASK-00, TASK-01, TASK-02, TASK-02a, TASK-02b, TASK-03, TASK-04, TASK-05a, TASK-06, TASK-07, TASK-08, TASK-09, TASK-12, TASK-13 now have complete test contracts. All tasks meet minimum TC requirements (S: ≥1, M: ≥3).
