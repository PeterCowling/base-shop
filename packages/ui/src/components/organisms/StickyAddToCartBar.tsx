...
export function StickyAddToCartBar({
  ...
  const label = ctaLabel ?? (t("actions.addToCart") as string);
  return (
    <div
      className={cn(
        "sticky end-0 bottom-0 start-0 flex items-center justify-between gap-4 border-t bg-bg shadow-elevation-2", // i18n-exempt: CSS utility classes only
        padding,
        className
      )}
      data-token="--color-bg"
      {...props}
    >
      <div className="flex flex-col">{/* i18n-exempt: CSS utility classes only */}
