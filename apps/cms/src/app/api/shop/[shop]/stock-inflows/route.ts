...
export async function GET(
  ...
  try {
    ...
    console.error("Stock inflows GET failed", err); // i18n-exempt -- non-UX log
    return NextResponse.json({ error: "Failed to list stock inflows" }, { status: 500 });
  }
}
...
export async function POST(
  ...
  try {
    ...
    const result = await receiveStockInflow(shop, body, { actor });
    if (!result.ok) {
      return NextResponse.json(result, { status: 400 });
    }
    return NextResponse.json(result, { status: result.duplicate ? 200 : 201 });
  } catch (err) {
    console.error("Stock inflows POST failed", err); // i18n-exempt -- non-UX log
    return NextResponse.json({ error: "Failed to receive stock inflow" }, { status: 500 });
  }
}
