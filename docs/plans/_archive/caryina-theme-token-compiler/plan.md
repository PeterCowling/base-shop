...
- [x] TASK-03: Generate `theme-tokens.generated.css` via a one-off script — Complete (2026-03-14)
- [x] TASK-04: Write `generated-parity.test.ts` — Complete (2026-03-14)
- [x] TASK-05: Write `coverage-parity.test.ts` — Complete (2026-03-14)
- [x] TASK-06: Update `global.css` import and add dark-mode alias — Complete (2026-03-14)
...
| TASK-03 | IMPLEMENT | Generate `theme-tokens.generated.css` via script | 85% | S | Complete (2026-03-14) | TASK-01 | TASK-04, TASK-06 |
| TASK-04 | IMPLEMENT | Write `generated-parity.test.ts` | 85% | S | Complete (2026-03-14) | TASK-01, TASK-02, TASK-03 | - |
| TASK-05 | IMPLEMENT | Write `coverage-parity.test.ts` | 85% | S | Complete (2026-03-14) | TASK-01, TASK-02, TASK-03 | - |
| TASK-06 | IMPLEMENT | Update `global.css` import and dark-mode alias | 90% | S | Complete (2026-03-14) | TASK-03 | - |
...
- **Effort:** S
- **Status:** Complete (2026-03-14)
- **Build evidence:** `scripts/caryina/generate-theme-tokens.ts` created. `apps/caryina/src/styles/theme-tokens.generated.css` generated and committed (36 CSS vars: 21 in `:root`, 15 in `html.theme-dark`). `packages/themes/caryina/src/index.ts` barrel added (required for tsx path resolution; exports assets, profile, recipes, themeCSSConfig, tokens). `scripts/package.json` entry `caryina:generate-theme-tokens` added. Script runs correctly from repo root via `node --import tsx scripts/caryina/generate-theme-tokens.ts`. Committed in `6776ed4154`.
...
- **Effort:** S
- **Status:** Complete (2026-03-14)
- **Build evidence:** `packages/themes/caryina/__tests__/generated-parity.test.ts` created. Adapts brikette pattern: dark selector is `"html.theme-dark"` (not `".dark"`). Tests: `:root` var presence, `:root` value match, `html.theme-dark` var presence, `html.theme-dark` value match, no unexpected vars in either block. Committed in `6776ed4154`.
...
- **Effort:** S
- **Status:** Complete (2026-03-14)
- **Build evidence:** `packages/themes/caryina/__tests__/coverage-parity.test.ts` created. Uses `COLOR_BRIDGE` (15 color vars), `FONT_BRIDGE` (2 font vars), `RADIUS_BRIDGE` (3 radius vars), `PROFILE_BRIDGE` (`--theme-transition-duration`). Reads values from `tokens.ts` directly (not `assets.brandColors`). No gradients/keyframes/recipes/RGB sections (caryina has none). Exhaustiveness tests for `:root` and `html.theme-dark`. Reverse coverage test. `--brand-mark-color` and `--brand-accent-color` correctly excluded (they're in `global.css :root`, not in the generated file). Committed in `6776ed4154`.
...
- **Effort:** S
- **Status:** Complete (2026-03-14)
- **Build evidence:** `@import "@themes/caryina/tokens.css"` replaced with `@import "./theme-tokens.generated.css"`. New `@media (prefers-color-scheme: dark) :root {}` block added with all 15 dark color values (direct HSL triplets from `tokens.ts`). Placed BEFORE the existing `not(.theme-dark)` override block (correct cascade order). All three dark mode paths preserved: (1) OS dark first-paint via new `@media` block, (2) post-hydration JS toggle via `html.theme-dark {}` in generated file, (3) explicit light override via `not(.theme-dark)`. No companion var references found in caryina components. Typecheck and lint both pass. Committed in `6776ed4154`.
