...
import { getStockAlertConfig } from "../repositories/stockAlerts.server";
  ...
  const recipients =
    config.recipients.length > 0
      ...
      : (coreEnv.STOCK_ALERT_RECIPIENTS ?? coreEnv.STOCK_ALERT_RECIPIENT ?? "")
          ...
          .map((r) => r.trim())
          .filter(Boolean);
  const webhook = config.webhook ?? coreEnv.STOCK_ALERT_WEBHOOK;
  const defaultThreshold =
    config.threshold ?? coreEnv.STOCK_ALERT_DEFAULT_THRESHOLD;
