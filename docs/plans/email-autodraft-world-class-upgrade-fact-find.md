...
Related-Plan: docs/plans/email-autodraft-world-class-upgrade-plan.md  <!-- not yet created; will be generated by /lp-plan -->
Business-Unit: BRIK
...
## Provisional Confidence Inputs (for /lp-plan)
...
- Confidence should be recalculated in `/lp-plan` after open questions are answered.

## Planning Boundary (Fact-Find vs Planning)
The following are explicitly planning-owned and intentionally not finalized in this lp-fact-find:
...
These are retained as planning inputs only. They are intentionally non-atomic and must be decomposed in `/lp-plan`.
