...
export async function listProviders(
  ...
  const builtIns =
    kind === 'payment' ? [...defaultPaymentProviders] : [...defaultShippingProviders];
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        if (kind === 'payment' && typeof plugin?.registerPayments === 'function') {
          builtIns.push(plugin.id ?? entry.name);
        }
        if (kind === 'shipping' && typeof plugin?.registerShipping === 'function') {
          builtIns.push(plugin.id ?? entry.name);
        }
      } catch {
