...
export const MemoryRouter = ({
  ...
  const entries = initialEntries.length > 0 ? initialEntries : ["/"];
  const safeIndex = Math.min(Math.max(initialIndex ?? 0, 0), entries.length - 1);
  const entry = entries[safeIndex] ?? "/";
  const [location, setLocation] = useState<Location>(() => locationFromUrl(entry));

  const navigate = useCallback<(to: To | number, options?: NavigateOptions) => void>(
    (to, options) => {
      if (typeof to === "number") {
        fallbackNavigate(to, options);
        return;
      ...
      const href = toHref(to);
      setLocation(locationFromUrl(href));
      ...
      if (typeof window !== "undefined") {
        ...
        if (options?.replace) {
          ...
          window.history.pushState(null, "", absolute);
        }
      }
    },
    [],
