...
- **Completion evidence (2026-03-09):**
  - `apps/reception/src/lib/inbox/thread-context.ts` now owns the bounded `buildThreadContext()` implementation and exported `MAX_THREAD_CONTEXT_MESSAGES`.
  - `apps/reception/src/lib/inbox/sync.server.ts` now reuses that bounded helper for sync-generated drafts instead of maintaining an unbounded local implementation.
  - `apps/reception/src/lib/inbox/recovery.server.ts` continues to consume the shared `buildThreadContext()` export via `sync.server.ts`, so stale-thread recovery now uses the bounded path too.
...
- **Completion evidence (2026-03-09):**
  - `apps/reception/src/app/api/mcp/inbox/[threadId]/draft/regenerate/route.ts` already used `boundMessages()` for regenerate context construction.
  - `apps/reception/src/app/api/mcp/__tests__/inbox-actions.route.test.ts` now includes a long-thread assertion proving regenerate passes only the 20 most recent stored messages into `generateAgentDraft()`.
...
- **Completion evidence (2026-03-09):**
  - `apps/reception/src/lib/inbox/__tests__/build-thread-context.test.ts` covers the bounded helper for 0, 5, 20, 25, and 50 messages plus null-date ordering.
  - The regenerate route test now complements the helper tests by asserting the bounded message count at the route boundary.
...
- [x] `buildThreadContext()` returns at most 20 messages (configurable via constant).
- [x] Regenerate route uses the same bounding logic (no duplicated unbounded construction).
- [x] Unit tests cover bounding at 0, 5, 20, 25, 50 messages.
- [x] All existing tests continue to pass.
- [x] `ThreadContext` and `ThreadMessage` types are unchanged.
