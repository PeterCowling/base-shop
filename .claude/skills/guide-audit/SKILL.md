...
Use `guide-translate` for translation after EN is clean.
...
- **Any translation or localization work** (use guide-translate instead)
...
After this skill completes successfully, the user should run `guide-translate` to propagate the fixed EN content to all locales.

Or use `guide-improve` (the orchestrator) to run both sequentially in one command.
