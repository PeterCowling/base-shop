...
function DealsBanner({ beds, time, lang }: DealsBannerProps): JSX.Element {
  ...
  const { t } = useTranslation("dealsPage", translationOptions);
  ...
  const ft = useCallback(
    (key: string, fallbackTemplate: string, options?: TranslationParams) => {
      // react-i18next returns the key when no instance is available, but in some
      // environments it may instead echo the defaultValue string without
      // interpolation (e.g. "Only {{beds}} ..."). Treat either case as a miss
      // and fall back to the human string we construct with real values.
      const raw = options ? t(key, options) : t(key);
      ...
      const resolved = normalised(raw) ?? normalised(options ? tEn(key, options) : tEn(key));
      if (resolved) {
        return resolved;
      }
  ...
  return (
    <div
      ref={setBannerRef}
      className="sticky top-0 flex w-full items-center justify-center bg-brand-primary px-6 py-3 text-fg-inverse shadow-md motion-safe:animate-slide-down"
    >
