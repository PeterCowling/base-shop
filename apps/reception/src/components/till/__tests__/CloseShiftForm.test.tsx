...
vi.mock("../CreditCardReceiptCheck", () => ({
  CreditCardReceiptCheck: () => null,
}));

vi.mock("../../../hooks/mutations/useCCReceiptConfirmations", () => ({
  useCCReceiptConfirmations: () => ({ confirmReceipt: vi.fn() }),
}));

let blindClose = false;
const getUserByPinMock = vi.fn((pin: string) => {
  if (pin === "111111") return { user_name: "alice", email: "a@test" };
  if (pin === "222222") return { user_name: "bob", email: "b@test" };
  return null;
});

vi.mock("../../../utils/getUserByPin", () => ({
