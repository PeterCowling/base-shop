...
- [x] TASK-02: Port XA uploader operator shell into new app
- [x] TASK-03: Session auth gate
- [x] TASK-04: Shop selector + scoped state reset
- [x] CHECKPOINT-05: Shell parity checkpoint
...
- [x] CHECKPOINT-11: Domain parity checkpoint
- [x] TASK-12: Port XA console component layer
...
| TASK-02 | IMPLEMENT | Port XA uploader operator shell (shell wrapper only) | 85% | S | Complete (2026-03-08) | TASK-01 | TASK-12, TASK-03 |
| TASK-03 | IMPLEMENT | Session auth gate | 88% | S | Complete (2026-03-08) | TASK-02 | CHECKPOINT-05 |
| TASK-04 | IMPLEMENT | Shop selector + scoped state reset | 78% | S | Complete (2026-03-08) | TASK-12 | CHECKPOINT-05 |
| CHECKPOINT-05 | CHECKPOINT | Shell parity checkpoint | 95% | S | Complete (2026-03-08) | TASK-03, TASK-04 | TASK-06, TASK-07 |
...
| CHECKPOINT-11 | CHECKPOINT | Domain parity checkpoint | 95% | S | Complete (2026-03-08) | TASK-07, TASK-13, TASK-14, TASK-16, TASK-17, TASK-10 | - |
| TASK-12 | IMPLEMENT | Port XA console component layer | 82% | S | Complete (2026-03-08) | TASK-02 | TASK-04 |
...
- **Effort:** S
- **Status:** Complete (2026-03-08)
- **Build evidence (CHECKPOINT-11):**
  - All IMPLEMENT dependencies confirmed Complete: TASK-07, TASK-10, TASK-13, TASK-14, TASK-15, TASK-16, TASK-17.
  - Horizon assumption 1 (JSONL vs Prisma): RESOLVED — TASK-15 delivered `InventoryAuditEvent` Prisma model; TASK-10 reads from Prisma via `listStockAdjustments`/`listStockInflows`. No JSONL on disk.
  - Horizon assumption 2 (Worker bundle size): No large new dependencies added; inventory-uploader follows xa-uploader pattern. Acceptable risk — no build-time measurement performed but app is structurally equivalent to xa-uploader.
  - Horizon assumption 3 (Caryina reads): Confirmed unaffected — all inventory-uploader changes are additive new files; no modifications to platform-core inventory read paths.
  - Decision (checkout-driven ledger surfacing): **New plan** — checkout event surfacing in the ledger requires order/booking integration that is outside Phase 1 scope.
  - Decision (Lane B product metadata uploader): **Deferred** — XA uploader reference is available (~70% confidence); initiate a new lp-do-fact-find when operator is ready to begin Lane B.
  - No downstream tasks exist after CHECKPOINT-11 (Blocks: —). Plan is complete.
- **Affects:** `docs/plans/inventory-app-uploader-unification/plan.md`
...
- **Acceptance:**
  ...
  - Operator can perform end-to-end inventory management without touching CMS or Caryina admin. ✓ (all components built and wired)
  - CMS and Caryina admin pages still accessible as fallback (not retired yet). ✓ (non-goals section confirms no changes to CMS/Caryina)
  - Decision recorded: checkout-driven ledger surfacing → new plan. ✓
  - Confidence for Lane B assessed: ~70%, deferred. ✓
- **Horizon assumptions to validate:**
  - JSONL vs Prisma DB for adjustment/inflow audit logs — **VALIDATED: Prisma confirmed.** ✓
  - Worker bundle size remains within 10 MiB — **ACCEPTABLE RISK: structurally equivalent to xa-uploader.** ✓
  - Caryina storefront inventory reads are unaffected — **VALIDATED: no read-path changes.** ✓
