...
- [x] TASK-01: Add FailedCheckDetail type and update QualityCheckResult in reception app
- [x] TASK-02: Mirror type and logic changes in MCP server draft-quality-check.ts
- [x] TASK-03: Add unit tests for failed_check_details in both implementations
...
|---|---|---|---:|---:|---|---|---|
| TASK-01 | IMPLEMENT | Add FailedCheckDetail type and update quality checker in reception app | 85% | M | Complete (2026-03-07) | - | TASK-02, TASK-03 |
| TASK-02 | IMPLEMENT | Mirror type and logic changes in MCP server | 85% | S | Complete (2026-03-07) | TASK-01 | TASK-03 |
| TASK-03 | IMPLEMENT | Add unit tests for failed_check_details | 85% | S | Complete (2026-03-09) | TASK-01, TASK-02 | - |
...
- **Notes / references:**
  ...
  - Build evidence (2026-03-09):
    - Reception quality checker and MCP duplicate already carried `failed_check_details` in code.
    - Reception test coverage was extended to assert reference applicability and contradicted-thread detail, closing the remaining acceptance gap.
    - `toParitySnapshot()` remained lossy by design and continues to exclude `failed_check_details`.
    - `pnpm --filter @apps/reception typecheck` passed.
    - `pnpm --filter @apps/reception lint` passed with warnings only.
...
- 2026-03-09: Archived after build-close reconciliation; the remaining acceptance gap was reception-side detail coverage for reference and contradiction cases, which is now present.
