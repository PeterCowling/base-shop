...
- Do not use command-mode shells such as `scripts/agents/integrator-shell.sh -- bash` to hold the lock; that path is forbidden. Use `--interactive-write-shell` only for a rare bounded repair, then exit immediately.
