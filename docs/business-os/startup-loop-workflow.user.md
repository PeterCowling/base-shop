...
## Canonical stage IDs (loop-spec v3.9.0)
...
Canonical source: `docs/business-os/startup-loop/loop-spec.yaml` (spec_version 3.8.0).
...
|---|---|---|---|
| BD-1 Brand Dossier bootstrap | S1 advance | GATE-BD-01 (Hard): check brand-identity.user.md Draft/Active status in strategy index; block S1 advance if missing | Run `/lp-assessment-bootstrap <BIZ>` → `brand-identity.user.md` at Draft minimum |
...
| DO Plan/Build | Available in process, but depends on stronger upstream inputs | Available in process, but depends on stronger upstream inputs | Available in process; confidence improved after MARKET-01/S3/S4/S5/S6 completion, still constrained by instrumentation completeness |
...
5. BRIK MARKET-01/S6 Deep Research freshness checks (monthly or on major outcome/ICP/channel/product change) are required.
...
## Deep Research Gate (MARKET-01 + S6) [Engineering Reference]

### 9.1 Mandatory hand-off rule

When either MARKET-01 (market intelligence) or S6 (site-upgrade brief) is in a seed/draft state, the workflow must:
...
- For `website-live` businesses, if S2A historical baseline is missing, `Draft`, or `Blocked`, MARKET-01/S6 must remain blocked.
...
- For `pre-website` businesses, MARKET-01 onward should be treated as `warning` quality unless S1B measurement bootstrap output exists and its verification checklist has passed.
...
```text
Deep Research completion is required for {{BIZ}} at stage {{MARKET-01|S6}}.
...
MARKET-01/S6 is complete only when:
...
>
> For HEAD and PET: brand-identity.user.md is bootstrapped by `/lp-assessment-bootstrap <BIZ>` at S1 entry (GATE-BD-01).
...
→ Bootstrapped at S1 entry via `/lp-assessment-bootstrap <BIZ>`. See `docs/business-os/strategy/<BIZ>/brand-identity.user.md` once created.
...
- **GATE-BD-01** (Hard): S1 advance blocked until `brand-identity.user.md` exists at Draft minimum in strategy index.
