...
// Moved from repo root to live alongside the CMS app. Root config re-exports this.
...
// Resolve paths relative to this app's Cypress directory
...
export default defineConfig({
  e2e: {
    ...
    // Run both `.cy.ts` and `.spec.ts` test files (absolute paths so this config
    // works whether invoked from the repo root or the CMS app)
    specPattern: [
      join(__dirname, "cypress/e2e/**/*.cy.{js,ts}"),
      join(path.resolve(__dirname, "../../test/e2e/**/*.spec.{js,ts}"))
    ],
    // Load global support commands from the app's Cypress folder
  ...
  component: {
    ...
    supportFile: join(__dirname, "cypress/support/component.ts"),
    retries: { runMode: 2, openMode: 0 },
