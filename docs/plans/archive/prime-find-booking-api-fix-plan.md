...
- Related brief: `docs/plans/prime-find-booking-api-fix-wf-fact-find.md`
- Key findings:
...
- **Scouts:**
  ...
  - `guestsDetails/{ref}` returns all occupant details as `Record<string, GuestDetailsRecord>` → confirmed via wf-fact-find Firebase query evidence
...
- **Scouts:**
  - `guestsDetails/{bookingId}/{occId}` returns `GuestDetailsRecord` with `firstName` and `lastName` → confirmed via wf-fact-find evidence and `guest-booking.ts:149`
