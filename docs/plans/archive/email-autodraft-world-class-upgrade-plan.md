...
- Operator ergonomics overhaul of `/ops-inbox` (Phase 2).
...
- Related brief: `docs/plans/email-autodraft-world-class-upgrade-wf-fact-find.md`
- Key findings:
...
- Resolved questions from wf-fact-find:
...
- Key modules/files:
  ...
  - `.claude/skills/ops-inbox/SKILL.md` — Operator workflow (queue → interpret → compose → quality → present)
- Patterns to follow:
...
| 4 | TASK-07 (CHECKPOINT) | Waves 1–3: all tasks | Re-assessment gate. Auto-continue pauses here for `/wf-replan`. |
...
- **Confidence:** 82%
  - Implementation: 85% — Escalation triggers are enumerated in the wf-fact-find; implementation follows `classifyScenario` pattern (line 406).
...
- **Documentation impact:** None
- **Notes / references:**
  - Fact-find escalation triggers: `docs/plans/email-autodraft-world-class-upgrade-wf-fact-find.md` (lines 174–178)
...
- **Acceptance:**
  - Add ≥8 new fixtures covering gaps identified in wf-fact-find:
...
- **Confidence:** 95%
- **Acceptance:**
  - Run `/wf-replan` on TASK-08, TASK-09, TASK-10 using evidence from completed tasks.
...
- **Notes / references:**
  - Non-refundable policy: wf-fact-find resolved Q (line 158) — no concessions, no exceptions.
...
- **Notes / references:**
  - Canonical non-refundable wording: Gmail template (wf-fact-find Q, line 171). Use existing repo text as baseline if Gmail text not provided.
  - Tone policy: factual + professional, not cold (wf-fact-find resolved Q, line 161).
...
|------|-----------|------------|---------------------|
| Guest email GAS migration | A9 (partial) | 75% | Requires new MCP tool; unclear requirements for `useEmailGuest` migration. Needs `/wf-fact-find`. |
