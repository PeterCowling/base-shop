...
export default function useInView<T extends HTMLElement = HTMLDivElement>(enabled: boolean): [(node: T | null) => void, boolean] {
  ...
  useEffect(() => {
    ...
    try {
      const media = window.matchMedia(/* i18n-exempt -- DS-1234 [ttl=2025-11-30] */ "(prefers-reduced-motion: reduce)");
      if (media.matches) {
        setInView(true);
        return;
      }
    ...
    const obs = new IntersectionObserver(
      ...
      { rootMargin: /* i18n-exempt -- DS-1234 [ttl=2025-11-30] */ "0px 0px -10% 0px", threshold: 0.1 },
    );
    obs.observe(node);
