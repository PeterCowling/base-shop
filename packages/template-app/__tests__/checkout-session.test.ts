...
import { createCartId, encodeCartCookie } from "../../platform-core/src/cartCookie";
import { setCart } from "../../platform-core/src/cartStore";
...
test("returns 400 when returnDate is invalid", async () => {
  ...
  const cartId = createCartId();
  setCart(cartId, cart);
  const req = createRequest({ returnDate: "invalid" }, cartId);
  const res = await POST(req);
