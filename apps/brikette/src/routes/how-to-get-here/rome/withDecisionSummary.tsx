...
export function DecisionSummary({ top, className }: DecisionSummaryProps) {
  ...
  return (
    <section className={classes}>
      ...
      <h3 className="mt-2 text-lg font-semibold text-brand-heading dark:text-brand-text">{t(top.route.titleKey)}</h3>
      <p className="mt-1 text-sm text-brand-text/80 dark:text-brand-text/80">{t(top.route.shortSummaryKey)}</p>
      {top.reasonKeys.length ? (
        <div className="mt-4 rounded-xl bg-brand-surface/80 p-4 dark:bg-brand-surface/30">
          <div className="text-sm font-semibold text-brand-heading dark:text-brand-text">{t("romePlanner.reco.why")}</div>
