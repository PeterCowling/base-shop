...
**Output format:** Appends Decision Log block to dossier with priority, potential-impact, traction objective (under `grow-business`), strategic fit, plan target, stance weight, and rationale. For P1/P2 under `grow-business`, also outputs a Rigor Pack that pre-populates the lp-fact-find stage doc.
...
Each P1/P2 must include all of the following. Target: one page total. This content pre-populates the lp-fact-find stage doc to accelerate `/lp-fact-find`; it does not replace `/lp-fact-find`.
...
1. **Flag as critical finding** in sweep report:
   ...
   Recommendation: Create business plan using `/lp-plan` or manual documentation.
   ```
...
**Rigor Pack output (pre-populates lp-fact-find stage doc):**
...
**Key insight:** Traction-mode P1 includes a complete Rigor Pack that can directly pre-populate the lp-fact-find stage doc and make `/lp-fact-find` faster.
