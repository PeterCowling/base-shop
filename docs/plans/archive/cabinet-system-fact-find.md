---
Type: Fact-Find
Outcome: Planning          # doc intent: this lp-do-fact-find exists to enable /lp-do-plan
Status: Proposed
Domain: Business-OS
Created: 2026-02-09
...
**Critical finding from the initial redesign lp-do-fact-find** (2026-02-09, `docs/plans/idea-generate-redesign-lp-do-fact-find.md`):
...
[ASSUMPTION] Platform telemetry is disabled by default. No telemetry configuration code was found in `platform-core` during this audit — the claim originates from the initial redesign lp-do-fact-find but could not be re-verified. If telemetry code exists, it is not in an obvious location.
...
| Card → Fact-finding | `/lp-do-fact-find` | Evidence gathered about current state, blast radius, constraints |
| Fact-finding → Planned | `/lp-do-plan` | Implementation plan created with tasks, acceptance criteria, confidence scores |
| Planned → In progress | `/lp-do-build` | Tasks implemented via TDD, one at a time, with validation |
| In progress → Done | `/lp-do-build` | All tasks complete, proposes lane transition |
| Done → Reflected | Manual | Post-mortem captures learnings |
| (Any stage) | `/lp-do-replan` | When confidence drops below 80%, investigate and update the plan |

**Confidence gating** is central: every task in a plan has a confidence score (0-100%). Tasks below 80% cannot be built — they must go through `/lp-do-replan` first. This prevents building on uncertain foundations.
...
```
/lp-do-fact-find → /lp-do-plan → /lp-sequence → /lp-do-build → /lp-do-replan (if needed)
```

Each produces a specific artifact:
- `/lp-do-fact-find` → `docs/plans/<slug>-lp-do-fact-find.md` (evidence brief)
- `/lp-do-plan` → `docs/plans/<slug>-plan.md` (implementation plan with tasks)
- `/lp-do-build` → code changes, commits, plan updates
- `/lp-do-replan` → updated plan with revised confidence scores
...
- `/lp-guide-improve` → orchestrates `/lp-guide-audit` + `/guide-translate`

Skills reference each other's outputs. For example, `/idea-generate` generates raw ideas that feed into `/idea-develop`, which creates cards that feed into `/lp-do-fact-find`, which produces briefs that feed into `/lp-do-plan`.
...
7. **Auto-works-up the top 3 ideas** into cards with lp-do-fact-find stage documents (tagged `sweep-auto`)
...
| Idea pipeline | Two-stage: raw ideas created via API → auto-work-up top 3 to cards with lp-do-fact-find stage docs | `idea-generate/SKILL.md` |
...
| Kanban execution | Cards flow through lp-do-fact-find → plan → build | Fact-finding → Planned → In progress → Done | Uses existing feature workflow |
...
| After a large build completes | Profile: capabilities (what skills were exercised), responsibilities (ownership changes) | `/biz-update-people` triggered by `/lp-do-build` completion |
...
- Q: Can skills invoke other skills?
  - A: Yes. Precedent: `guide-improve` invokes `guide-audit` and `guide-translate`. `lp-do-plan` auto-invokes `lp-do-build`.
  - Evidence: `.claude/skills/lp-guide-improve/SKILL.md:43`, `.claude/skills/lp-do-plan/SKILL.md:725`
...
10. **Traction mode for Drucker/Porter (v2.1 section — grow-business acceleration)**
   ...
   - Key behaviors:
     ...
     - Rigor Pack pre-populates lp-do-fact-find stage doc (Stage 7), enabling fast-track execution
   ...
   - Rationale: Early-stage market-facing businesses need speed-to-traction, not heavy analysis. Rigor Pack ensures minimum viable rigor without becoming a delay mechanism. Reconciles with lp-do-fact-find by making the Rigor Pack the lp-do-fact-find doc for traction P1/P2.
   - Impact: [PROPOSAL] Stage 5 outputs Rigor Pack alongside Decision Log for traction-mode P1/P2. Stage 7 uses Rigor Pack as lp-do-fact-find template instead of standard template. Weekly traction cadence documented as recommended rhythm.
...
- Open questions: 7 remain in "Open (User Input Needed)" — all have documented default assumptions that are safe to proceed with. None block planning; all can be confirmed or overridden during `/lp-do-plan`.
...
- Recommended next step: Proceed to `/lp-do-plan cabinet-system`
