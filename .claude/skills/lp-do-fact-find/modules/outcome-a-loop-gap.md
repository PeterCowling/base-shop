...
**Downstream consumer**: Operator review. May feed manual `/lp-do-replan` invocation if block persists across runs.

---

## Trigger: `bottleneck` — Bottleneck Root-Cause Investigation

**Outcome**: `planning` (Outcome A posture — feeds `/lp-do-replan`)
...
6. Replan trigger state
   ...
   - If trigger is `open`: flag that `/lp-do-replan` must be invoked before this cycle advances.

**Output format**: Use `docs/plans/_templates/fact-find-planning.md`.

**Downstream consumer**: `/lp-do-replan` — include constraint key(s) and root cause hypotheses in frontmatter.
