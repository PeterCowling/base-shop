...
Integrate Business OS card lifecycle management into the wf-fact-find, wf-plan, and wf-build skills so that completing work through these skills automatically creates and updates Business OS cards. Currently, running `/wf-fact-find` -> `/wf-plan` -> `/wf-build` produces artifacts in `docs/plans/` but the Business OS kanboard remains stale with no corresponding card tracking the work.

### Goals

- When `/wf-fact-find` completes for a new topic, automatically create a Business OS card (or link to existing one) and create a fact-finding stage document
- When `/wf-plan` completes, create a planned stage document and propose lane transition to "Planned"
- When `/wf-build` starts/completes tasks, update card status and progress
...
- Constraints:
  ...
  - Should reuse existing skills where possible (`/idea-develop`, `/idea-advance`)
...
- `.claude/skills/wf-fact-find/SKILL.md` - produces `docs/plans/<feature>-wf-fact-find.md`
- `.claude/skills/wf-plan/SKILL.md` - produces `docs/plans/<feature>-plan.md`
- `.claude/skills/wf-build/SKILL.md` - updates plan doc as tasks complete
...
- `.claude/skills/idea-advance/SKILL.md` - proposes lane transitions based on evidence

### Key Modules / Files

#### Skill Implementations
- `/Users/petercowling/base-shop/.claude/skills/wf-fact-find/SKILL.md` - 436 lines, produces two outcomes (Planning Brief or Briefing Note)
- `/Users/petercowling/base-shop/.claude/skills/wf-plan/SKILL.md` - 446 lines, creates confidence-gated implementation plans
- `/Users/petercowling/base-shop/.claude/skills/wf-build/SKILL.md` - 308 lines, implements tasks with TDD
...
#### Card Creation Pattern (from `/idea-develop`)
...
Evidence-gated transitions (from `/idea-advance`):
...
interface StageFrontmatter {
  Type: "Stage";
  Stage: StageType;  // wf-fact-find | plan | build | reflect
  "Card-ID": string;
  Created?: string;
  Updated?: string;
}
...
- Current wf-fact-find briefs already have `Related-Plan` in frontmatter
...
- Existing skill implementations (wf-fact-find, wf-plan, wf-build)
- Business catalog (`docs/business-os/strategy/businesses.json`)

#### Downstream Dependents
- Users running `/wf-fact-find`, `/wf-plan`, `/wf-build`
...
#### Likely Blast Radius
- **Direct:** 3 skill instruction files (wf-fact-find, wf-plan, wf-build)
...
- `82ffa55ba8` - feat(skills): add interactive workflow selection to guide-improve
- `55fb13b564` - feat(skills): split guide-improve into focused EN and translation skills
- `82825687a3` - Work up idea: BRIK-OPP-0003 -> Card: BRIK-ENG-0020 (example of /idea-develop)
...
- **A:** Via `/idea-develop` from inbox ideas, which creates card files, agent.md files, and initial fact-finding stage docs
- **Evidence:** `.claude/skills/idea-develop/SKILL.md` lines 62-323
...
- **A:** Via `/idea-advance` skill which reads card + stage docs, analyzes evidence against criteria, updates `Proposed-Lane` frontmatter field, adds comment with rationale
- **Evidence:** `.claude/skills/idea-advance/SKILL.md` lines 79-122
...
- **Decision impacted:** How wf-fact-find/plan/build track their associated card
- **Options:**
  - A) Add frontmatter to plan docs: `Card-ID: BRIK-ENG-0020`
  - B) Use skill arguments: `/wf-fact-find --card BRIK-ENG-0020`
...
- **Approach:** 80%
  ...
  - Reuses existing patterns (idea-develop, idea-advance)
...
4. **Extend /wf-fact-find** - After creating wf-fact-find brief, optionally create/link Business OS card + fact-finding stage doc
5. **Extend /wf-plan** - After creating plan, create planned stage doc; propose lane move to Planned
6. **Extend /wf-build** - On task completion, update card progress; on all tasks complete, propose lane move to Done
...
- **Recommended next step:** Proceed to `/wf-plan` with following notes:
...
| File | Purpose |
|------|---------|
| `.claude/skills/wf-fact-find/SKILL.md` | Current wf-fact-find implementation |
| `.claude/skills/wf-plan/SKILL.md` | Current wf-plan implementation |
| `.claude/skills/wf-build/SKILL.md` | Current wf-build implementation |
...
| `.claude/skills/wf-replan/SKILL.md` | Re-planning workflow |
...
| `docs/plans/commerce-core-readiness-wf-fact-find.md` | Example wf-fact-find without card link |
