...
Source: `docs/plans/ci-integration-speed-control-lp-fact-find.md` (captured 2026-02-09 UTC).
...
## Fact-Find Reference
- `docs/plans/ci-integration-speed-control-lp-fact-find.md`
...
5. CI-SC-06 and CI-SC-08 outputs feed lp-replan decisions
...
- **Updated confidence:** 75%
  ...
  - Implementation: 77% — Test fan-out evidence from lp-fact-find: run `21838129021` launched 57 turbo `*:test` groups. This gives a baseline count for threshold calibration.
...
- **Investigation performed:**
  - Repo: lp-fact-find baseline data (run `21838129021`, 57 turbo test groups)
...
- **Exit criteria:**
  ...
  - **Fail:** CMS tests exceed 15 minutes or fail → CI-SC-03 needs approach (b) with shard support in nightly, lp-replan CI-SC-03
...
- 2026-02-09 (lp-replan): Created CI-SC-09 (SPIKE) as precursor for CI-SC-03 to resolve CMS nightly timeout uncertainty.
- 2026-02-09 (lp-replan): Promoted CI-SC-06 from 72% to 82% based on comprehensive cache inventory (E1).
- 2026-02-09 (lp-replan): Promoted CI-SC-08 from 70% to 75% based on test fan-out evidence (E1).
- 2026-02-09 (lp-replan): Added test contracts (TC-XX) to all IMPLEMENT tasks: CI-SC-01, 02, 03, 05, 07.
- 2026-02-09 (lp-replan): Added test contract to CI-SC-09 (SPIKE).
- 2026-02-09 (lp-replan): CI-SC-03 confidence recorded as conditional: 79% → 84% on CI-SC-09 completion.
- 2026-02-09 (lp-replan): CI-SC-05 confidence recorded as conditional: 74% → 84% on CI-SC-04 completion.
...
- 2026-02-09 (spike): CI-SC-09 completed with FAIL result. Unsharded `--runInBand` CMS tests exceeded 21min (384/421 suites). CI-SC-03 needs lp-replan: approach (a) not viable, consider approach (b) shard support in nightly or (c) unsharded with Jest parallelism.
...
- 2026-02-10 (lp-replan): CI-SC-03 lp-replanned after CI-SC-09 FAIL. Approach (a) rejected (unsharded >21min). Approach (b) selected: dedicated 4-way sharded CMS coverage job in nightly `test.yml`. Also: `collectCoverage: false` in jest.config to make `test` script truly coverage-free. Confidence promoted 79% → 84%.
...
| Condition | Action |
|---|---|
| IMPLEMENT task <80% and on merge path | Run `/lp-replan` for that task before build |
| IMPLEMENT task >=80% with prerequisites met and test contracts complete | Proceed with `/lp-build` |
...
| No eligible IMPLEMENT tasks | Continue investigation track and lp-replan |
