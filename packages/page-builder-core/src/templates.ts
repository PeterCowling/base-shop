...
export function cloneTemplateComponents(
  ...
  const clone = (components: PageComponent[]): PageComponent[] =>
    components.map((c) => {
      const copy: PageComponent = { ...c };
      const children = (c as unknown as { children?: PageComponent[] }).children;
      if (Array.isArray(children)) {
        (copy as unknown as { children?: PageComponent[] }).children = clone(children);
      }
      return copy;
    });
 ...
 */
export function scaffoldPageFromTemplate(
  descriptor: TemplateDescriptor,
  ctx: ScaffoldContext,
  overrides?: Partial<Omit<Page, "components" | "seo">> & {
