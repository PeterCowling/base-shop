...
export function useSafeNavigate(): NavigateFunction {
  ...
  return useCallback(
    (to: To | number, options?: NavigateOptions) => {
      if (typeof to === "number") {
        if (!rawNavigator && import.meta.env?.DEV && !warnedRef.current && typeof window !== "undefined") {
          // i18n-exempt -- UI-1000 [ttl=2026-12-31] Dev-only diagnostic message.
      ...
      const target = typeof to === "string" ? to : `${to.pathname ?? ""}${to.search ?? ""}${to.hash ?? ""}`;
      if (!rawNavigator && import.meta.env?.DEV && !warnedRef.current && typeof window !== "undefined" && target) {
        // i18n-exempt -- UI-1000 [ttl=2026-12-31] Dev-only diagnostic message.
        console.warn("[ModalContext] Falling back to window navigation outside router context");
        warnedRef.current = true;
      }
...
export function useSafeLocation(): Location {
  try {
    ...
    if (
      ...
      // i18n-exempt -- UI-1000 [ttl=2026-12-31] React Router error string match.
