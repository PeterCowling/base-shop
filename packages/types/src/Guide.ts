...
export type GuideRiskTier = 0 | 1 | 2;

export interface GuideCore {
  key: string;
  slug: string;
  contentKey: string;
  areas: string[];
  ...
  riskTier: GuideRiskTier;
  schemaVersion: number;
}

export interface GuidePublication extends GuideCore {
  id: string;
  shop: string;
  status: GuidePublicationStatus;
  row_version: number;
  created_at: string;
  updated_at: string;
...
const guideIntroSchema = z.union([
  z.object({
    ...
    body: z.union([z.string().trim().min(1), z.array(z.string().trim().min(1))]),
  }),
...
export const guideContentSchema = z
  .object({
    ...
    lastUpdated: z.union([z.string().datetime(), z.string().date()]).optional(),
