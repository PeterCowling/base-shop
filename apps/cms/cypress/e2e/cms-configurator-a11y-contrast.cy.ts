...
describe('CMS configurator color-contrast [a11y]', { tags: ['a11y'] }, () => {
  ...
  it('dashboard /cms/configurator has no serious+ color-contrast issues', function () {
    cy.session('admin-session', login);
    ...
    cy.document().then(function (doc) {
      ...
      if (errorRoot) {
        cy.log(
          'Skipping cms-configurator-a11y-contrast dashboard: /cms/configurator shows Next.js error overlay in this environment.',
        );
  ...
  steps.forEach((id) => {
    it(`step /cms/configurator/${id} has no serious+ color-contrast issues`, function () {
      cy.session('admin-session', login);
      ...
      cy.document().then(function (doc) {
        ...
        if (errorRoot) {
          cy.log(
            `Skipping cms-configurator-a11y-contrast: /cms/configurator/${id} shows Next.js error overlay in this environment.`,
          );
        ...
        cy.checkA11y(
          excluded,
          { ...runOnly, includedImpacts: ['critical', 'serious'] },
          (violations) => {
