...
TASK-05 unified terminology ("published" → "live") and TASK-06 added content filtering using `isGuideLive()`. Both depend on the GUIDES_INDEX as the status source. This wf-fact-find addresses the remaining gap.
...
- Recommended next step: Proceed to `/wf-plan guide-status-single-source`
