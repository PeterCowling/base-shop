# lp-do-discovery-07-our-stance — Our Stance (DISCOVERY-07)

Elicits and records what the operator directly knows about the business before DISCOVERY intake runs. Captures launch surface, inventory/product readiness, commercial architecture, and pre-locked channel decisions. All fields are either confirmed or explicitly marked as open gaps.

## Invocation

```
/lp-do-discovery-07-our-stance --business <BIZ>
```
...
- Writes the final DISCOVERY-07 artifact

Does NOT:
- Research external sources (that is DISCOVERY-01–DISCOVERY-06 territory)
...
- `docs/business-os/strategy/<BIZ>/s0c-option-select.user.md` — may contain execution constraints (from DISCOVERY-03)
- Any other strategy or brief docs under `docs/business-os/strategy/<BIZ>/`

If upstream DISCOVERY-01–DISCOVERY-06 artifacts are absent, note the gap but proceed — DISCOVERY-07 captures operator-direct knowledge only and does not depend on research outputs.
...
# Operator Evidence — <BIZ> (DISCOVERY-07)
...
**Downstream consumer:** `modules/discovery-intake-sync.md` reads `s0e-operator-evidence.user.md` as its seventh precursor (DISCOVERY-07). Fields from Sections A–D populate operator-context rows in intake Sections A, B, C, and D. Section E gaps populate intake Section F (Missing-Data Checklist).
...
>
> "All seven DISCOVERY precursors (DISCOVERY-01–DISCOVERY-07) now present for <BIZ>. The next `/startup-loop start` or `advance` call will trigger DISCOVERY intake sync automatically."

---

## Integration

**Upstream (DISCOVERY-06):** Runs after `/lp-do-discovery-06-measurement-plan --business <BIZ>` produces measurement-plan.user.md.

**Downstream:** `modules/discovery-intake-sync.md` reads this artifact as `Precursor-DISCOVERY-07`. Section E gaps feed intake Section F (Missing-Data Checklist). Section D channel pre-decisions are preserved as operator-locked rows in intake Section D.
