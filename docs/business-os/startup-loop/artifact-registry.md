...
| `results-review` | Operator (human) | Post-build observation | `docs/plans/<feature-slug>/results-review.user.md` | Observed Outcomes, Standing Updates (or explicit `No standing updates: <reason>`), New Idea Candidates; frontmatter: `Status`, `Feature-Slug`, `Review-date`, `artifact: results-review` — **hard gate: required before plan archival** | Layer A standing-information refresh; startup-loop feedback loop | frontmatter `artifact: results-review` |
...
| `market_pack` (optional) | `docs/business-os/strategy/<BIZ>/market-pack.user.md` | MARKET-11 |
| `sell_pack` (optional) | `docs/business-os/strategy/<BIZ>/sell-pack.user.md` | SELL-07 |
