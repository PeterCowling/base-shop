...
`/lp-build` executes plan tasks safely, one task per cycle, with gate enforcement and explicit handoffs.
...
`/lp-build` can execute one runnable task per cycle for these task types:
...
If task confidence is below its threshold, stop and route to `/lp-replan`.
...
- canonical stored values use no slash (`lp-build`, `lp-replan`, `draft-email`, `biz-product-brief`, etc.)

- If missing after normalization -> STOP -> `/lp-replan`.
- If task type is `CHECKPOINT` and normalized value is `lp-replan` (legacy), treat it as `lp-build` and route to checkpoint executor.
- If normalized value is not `lp-build` -> dispatch to listed skill and then return to post-task gates.
- If normalized value is `lp-build` -> route by type/track:
...
If Red/Green/Refactor steps cannot be executed as defined, stop and route to `/lp-replan`.
...
- invoke `/lp-replan` for downstream tasks,
...
> Build progressed. Some tasks remain blocked or below threshold. See updated plan and run `/lp-replan` for blocked tasks.
