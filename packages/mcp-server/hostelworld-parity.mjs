...
/**
 * Hostelworld parity capture for BRIK.
 *
...
import {
  ...
  requireOneOf,
  round2,
} from "./dist/startup-loop/parity-hostelworld.js";
...
const BRIK_HOSTELWORLD_PROPERTY_ID = "7763";
...
async function main() {
  ...
  const url = buildHostelworldUrl({
    propertyId: BRIK_HOSTELWORLD_PROPERTY_ID,
    checkIn: parsed.checkIn,
    checkOut: parsed.checkOut,
    travellers: parsed.pax,
  });

  console.info("Hostelworld parity capture - auto-only mode");
  ...
  try {
    ...
    // Attempt auto-extraction from Hostelworld
    try {
      const priceData = await tool("browser_act", {
        ...
        action: {
          ...
          expression: `
            (() => {
              // Common Hostelworld price selectors
              ...
              // Try to find deposit/payment split
              ...
              for (const sel of depositSelectors) {
                const elem = document.querySelector(sel);
                if (elem && elem.textContent && /\\d/.test(elem.textContent)) {
                  deposit = elem.textContent.trim();
                }
              }

              const payAtPropSelectors = [
                '[data-test="pay-at-property"]',
                '[class*="pay-at"]',
                '[class*="remainder"]',
              ];

              for (const sel of payAtPropSelectors) {
                const elem = document.querySelector(sel);
                if (elem && elem.textContent && /\\d/.test(elem.textContent)) {
                  payAtProperty = elem.textContent.trim();
                }
              }

              return { totalPrice, deposit, payAtProperty };
            })()
          `,
        },
      });

      const extracted = priceData?.result;
      if (!extracted || !extracted.totalPrice) {
        throw new Error("Price element not found on page");
      }

      const totalPriceAllIn = round2(parseNumberLike(extracted.totalPrice));
      const depositAmount = extracted.deposit ? round2(parseNumberLike(extracted.deposit)) : 0;
      const payAtPropertyAmount = extracted.payAtProperty ? round2(parseNumberLike(extracted.payAtProperty)) : 0;
    ...
    const rowLine = buildHostelworldParityCsvRow({
  ...
  if (capture) {
    const rowLine = buildHostelworldParityCsvRow({
      ...
      depositAmount: capture.depositAmount,
      payAtPropertyAmount: capture.payAtPropertyAmount,
