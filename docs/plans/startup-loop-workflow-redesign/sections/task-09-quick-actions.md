...
| Business is pre-website and measurement is not set up (S1B) | Run the Pre-website Measure checklist | Measure stage document is at Active status |
| Business is website-live and Results baseline is missing or blocked (S2A) | Run the Results Data Request prompt to identify what data is needed | Data request document sent to the data owner |
| Required S2A data pack has arrived | Run the Results Baseline Consolidation prompt | Results baseline document is at Active status |
