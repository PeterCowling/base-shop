...
| Force push to `staging` | ❌ **BLOCKED** - Same protection |
...
**Note:** In Claude Code sessions, `--no-verify` is hard-blocked by the git guard (Layer 4 protection). The git guard intercepts git commands before they reach the shell.
...
**Note:** In Claude Code sessions, `--no-verify` is hard-blocked by the git guard (Layer 4 protection).
