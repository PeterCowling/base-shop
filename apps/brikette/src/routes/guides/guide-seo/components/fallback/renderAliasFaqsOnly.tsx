...
export function renderAliasFaqsOnly(params: {
  ...
  try {
    ...
    const localizedStructuredExists = (() => {
      ...
      try {
        const introArr = Array.isArray(context?.intro) ? context.intro : [];
        const hasIntro = introArr.some((p) => typeof p === "string" && p.trim().length > 0);
        if (hasIntro) return true;
      } catch { /* noop */ }
      try {
        const sectionsArr = Array.isArray(context?.sections) ? context.sections : [];
        const hasSections = sectionsArr.some((section) => {
          if (!section || typeof section !== "object") return false;
          const title = typeof section.title === "string" ? section.title.trim() : "";
          if (title.length > 0) return true;
          const body = Array.isArray(section.body) ? (section.body as unknown[]) : [];
