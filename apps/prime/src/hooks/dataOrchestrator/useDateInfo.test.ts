...
vi.mock('../../utils/dateUtils', () => ({
  parseCheckInDate: vi.fn((date: string) => new Date(date)),
  parseDateString: vi.fn((date: string) => new Date(date)),
  formatToYYYYMMDD: vi.fn((date: Date) => date.toISOString().split('T')[0]),
  formatDateToDDMM: vi.fn((date: Date) => {
    const day = date.getDate().toString().padStart(2, '0');
    const month = (date.getMonth() + 1).toString().padStart(2, '0');
    return `${day}/${month}`;
  }),
  getDaysBetween: vi.fn((from: string, to: string) => {
    const fromDate = new Date(from);
    const toDate = new Date(to);
    return Math.floor((toDate.getTime() - fromDate.getTime()) / (1000 * 60 * 60 * 24));
  }),
  getLocalTimestamp: vi.fn(() => '2025-07-20T10:00:00'),
  computeOrderDate: vi.fn((checkInDate: string, nightKey: number) => {
...
describe('useDateInfo', () => {
  beforeEach(() => {
    ...
    vi.useFakeTimers();
    vi.setSystemTime(new Date('2025-07-20T10:00:00'));
  });
