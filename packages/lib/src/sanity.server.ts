...
export async function fetchPublishedPosts(shopId: string): Promise<BlogPost[]> {
  try {
    ...
    const query = `*[_type == "post" && defined(slug.current) && published == true && (!defined(publishedAt) || publishedAt <= now()) && !(_id in path('drafts.**'))]{title, "slug": slug.current, excerpt}`;
...
export async function fetchPostBySlug(
  ...
  try {
    const client = await getClient(shopId);
    const query = `*[_type == "post" && slug.current == $slug && published == true && (!defined(publishedAt) || publishedAt <= now())][0]{title, "slug": slug.current, excerpt, body}`;
