...
- Related brief: `docs/plans/database-backed-business-os-lp-fact-find.md`
...
- **Type:** FACT-FIND
- **Affects:** `docs/plans/database-backed-business-os-lp-fact-find.md`, `docs/plans/database-backed-business-os-plan.md`
- **Depends on:** -
- **Confidence:** 85%
...
- **Test plan:**
  - Run a minimal smoke of the chosen dev loop (e.g., build + `wrangler pages dev` for the Business OS app root) and capture the command in the lp-fact-find.
...
- **Rollout / rollback:**
  - Rollout: N/A (lp-fact-find only).
  - Rollback: N/A.
- **Documentation impact:** Updates required in this plan and the lp-fact-find brief.
- **Notes / references:**
...
- **Rollout / rollback:** N/A (lp-fact-find only).
- **Documentation impact:** Update lp-fact-find with the final access pattern and example usage.
- **Notes / references:**
...
- **Acceptance:**
  ...
  - [x] Record perf/complexity notes (DX, typing, migrations workflow) and make a recommendation in the lp-fact-find (raw D1 SQL + Zod).
...
- **Rollout / rollback:** N/A (lp-fact-find only).
- **Documentation impact:** Update both the lp-fact-find and this plan with the chosen approach and rationale.
- **Notes / references:**
...
- **Type:** FACT-FIND
- **Affects:** `docs/business-os/business-os-charter.md`, `docs/business-os/security.md`, `docs/plans/database-backed-business-os-lp-fact-find.md`, `docs/plans/database-backed-business-os-plan.md`
- **Depends on:** BOS-D1-FF-01
- **Confidence:** 82%
...
- **Type:** DECISION
- **Affects:** `docs/plans/database-backed-business-os-plan.md`, `docs/plans/database-backed-business-os-lp-fact-find.md`
...
- **Rollout / rollback:** N/A.
- **Documentation impact:** Ensures plan and lp-fact-find remain aligned and build-ready.
...
- **Type:** IMPLEMENT
- **Affects:** `apps/business-os/wrangler.toml` (NEW), `apps/business-os/README.md` (update), `docs/plans/database-backed-business-os-lp-fact-find.md` (evidence)
...
- **Type:** IMPLEMENT
- **Affects:** `apps/business-os/scripts/migrate-business-os-to-d1.ts` (NEW), `apps/business-os/package.json` (optional script), `docs/plans/database-backed-business-os-lp-fact-find.md` (evidence)
...
- **Investigation performed:**
  - **Existing source data is small and enumerable:**
    ...
    - `docs/business-os/cards/PLAT-OPP-0001/lp-fact-find.user.md`
...
- **Decision / resolution:**
  ...
  - **Idempotency:**
    ...
    - Stage docs: deterministic `id` = `${cardId}/${stage}` (e.g. `PLAT-OPP-0001/lp-fact-find`) so re-runs overwrite the same row.
...
- **Type:** DOC
- **Affects:** `docs/business-os/business-os-charter.md`, `docs/business-os/security.md`, `apps/business-os/src/lib/repo/README.md`, `docs/plans/database-backed-business-os-lp-fact-find.md`
- **Depends on:** BOS-D1-DEC-01
- **Confidence:** 85%
...
- **Documentation updated:**
  ...
  - `docs/plans/database-backed-business-os-lp-fact-find.md` — Removed contradictions about local-only deployment; updated status to "Superseded (Implementation Complete)"
...
- 2026-01-30 (lp-replan): **Data access layer: Raw D1 SQL + Zod** — product-pipeline precedent, no Prisma Edge runtime complexity (BOS-D1-FF-03).
- 2026-01-30 (lp-replan): **Canonical store: D1-canonical + git mirror (Option A)** — lower complexity, zero-cost hosting, eventual consistency acceptable (BOS-D1-FF-04).
- 2026-01-30 (lp-replan): **Concurrency token: `updated_at` timestamp** — simple, SQLite-native, no version column needed (BOS-D1-06).
- 2026-01-30 (lp-replan): **Auto-refresh: MAX(updated_at) polling** — 30s interval, indexed query, no SSE/DO for MVP (BOS-D1-07).
- 2026-01-30 (lp-replan): **Git export: CI scheduled job** — hourly export via Node script, no runtime secrets needed (BOS-D1-09).
...
- 2026-01-31 (lp-replan): **Edge build is an explicit acceptance gate** — current failures stem from Node-only auth and git/fs server actions imported by Edge pages (BOS-D1-FF-01, BOS-D1-06).
- 2026-01-31 (lp-replan): **Git mirror should use repo pipeline PR workflow** — exporter commits land via `dev` → `staging`, not direct pushes to `staging`/`main` (BOS-D1-09).
