// src/app/[lang]/house-rules/page.tsx
// House Rules page - App Router version
...
const SECTION_KEYS: readonly string[] = Array.from({ length: 14 }, (_, i) => `s${i + 1}`);
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const meta = await resolveI18nMetaForApp(validLang, "houseRulesPage");
  const localizedSlug = getSlug("houseRules", validLang);
...
export default async function HouseRulesPage({ params }: Props) {
  const { lang } = await params;
  const validLang = toAppLanguage(lang);
  const t = await getTranslations(validLang, ["houseRulesPage", "translation"]);
