...
function assertPrismaConfiguration(prismaDelegate, sourceLabel) {
    const url = process.env.DATABASE_URL;
    if (!url) {
        throw new Error(`Prisma backend selected via ${sourceLabel} but DATABASE_URL is not set.`);
    }
    let delegate;
    try {
        ...
        throw new Error(`Prisma backend selected via ${sourceLabel} but Prisma client is not available.`, error instanceof Error ? { cause: error } : undefined);
    }
    if (!delegate) {
        throw new Error(`Prisma backend selected via ${sourceLabel} but Prisma model delegate is missing.`);
    }
}
async function resolveRepo(prismaDelegate, prismaModule, jsonModule, options = {}) {
    ...
    if (rawBackend && !backend) {
        throw new Error(`Unsupported backend "${rawBackend}" for ${envVarName}. Expected "prisma" or "json".`);
    }
    if (!backend && rawDbMode && !dbMode) {
        throw new Error(`Unsupported backend "${rawDbMode}" for DB_MODE. Expected "prisma" or "json".`);
    }
