...
async function signGetForProvider(imageUrl: string): Promise<string> {
  try {
    ...
    // Accept both public base and r2.cloudflarestorage.com; always sign to ensure private bucket access
...
export async function POST(req: NextRequest): Promise<Response> {
  try {
    ...
    try {
      const cached = await kvGet(`idem:${parsed.data.idempotencyKey}:segment`);
      if (cached) return NextResponse.json({ maskUrl: cached, metrics: { preprocessMs: 0 } });
    } catch {}
    const provider = getProvider();
    if (!provider.segmenter) return NextResponse.json({});
    const signedUrl = await signGetForProvider(imageUrl);
    const resp = await provider.segmenter.run(signedUrl);
    const ms = Date.now() - t0;
    try {
      const host = new URL(imageUrl).host;
      console.info("tryon.segment", { jobId: parsed.data.idempotencyKey, host, ms, ok: !resp.error });
    } catch {}
    if (resp.error) return NextResponse.json({ error: resp.error }, { status: 502 });
    const url = resp.result?.url;
    try { if (url) await kvPut(`idem:${parsed.data.idempotencyKey}:segment`, url, 86400); } catch {}
    return NextResponse.json({ maskUrl: url, metrics: { preprocessMs: resp.metrics?.preprocessMs ?? ms } });
  } catch (err: unknown) {
