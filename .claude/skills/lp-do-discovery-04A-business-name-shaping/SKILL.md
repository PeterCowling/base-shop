---
name: lp-do-discovery-04A-business-name-shaping
description: Produce a naming-generation-spec.md for a business. Reads DISCOVERY stage docs, extracts ICP, product, brand personality, competitive set, and any prior eliminated names, then writes a fully-structured agent-executable spec that generates 250 scored candidate names. Part 1 of a 4-part naming pipeline (spec → generate → RDAP batch check → rank).
---

# lp-do-discovery-04A-business-name-shaping — Business Name Shaping (DISCOVERY-04A)
...
This skill replaced `lp-do-discovery-04-business-name-options` (which produced a deep research prompt — obsolete since deep research tools cannot verify domain availability).

## When to use

Use when a brand name is open (not yet committed) and the business has sufficient DISCOVERY stage context. Requires at minimum: a problem statement, a product/option decision, and a brand personality sketch.

Run on user instruction: `/lp-do-discovery-04A-business-name-shaping --business <BIZ>`
...
|------|------|-------------|
| **1 — Spec** | This skill. Reads DISCOVERY docs, writes `naming-generation-spec.md` | This skill |
...
**Allowed:** read DISCOVERY docs, synthesize context, write `naming-generation-spec.md`.
...
The output spec must contain all of the following sections. Sections §3, §4, and §5 are business-specific and must be populated from the DISCOVERY docs. Sections §3.3 and §5 (generation patterns and output format) are universal infrastructure — copy them verbatim from the template below, do not rewrite.

---

### §1. What this brand is

A dense, precise summary distilled from the DISCOVERY docs. Cover:
...
- **What the brand must NOT sound like** — extracted from anti-criteria in DISCOVERY docs + any regulatory constraints (e.g. EU MDR proximity, luxury-house adjacency, clinical framing)
...
Example (do not copy verbatim — write from the actual DISCOVERY docs):
...
#### I — ICP resonance (1–5)
*[Write this dimension from the DISCOVERY docs. Reference the specific buyer, platform, and context from §2. The 1–5 descriptors must describe what resonance failure and success look like for this specific ICP — not generic "fashion buyer" language.]*
...
**This section has two parts: universal infrastructure (copy verbatim) and business-specific morpheme pools (write from DISCOVERY docs).**
...
*[Write this subsection from the DISCOVERY docs. It is the primary business-specific content in this section.]*
...
- [ ] §1 populated from DISCOVERY docs (not generic placeholder text)
...
**Downstream (DISCOVERY-05):** `/lp-do-discovery-05-distribution-planning` runs after the operator has confirmed a working name from the shortlist.
