...
- [ ] TASK-02: Update `validateGuestToken` return type + all callers and test mocks
- [ ] TASK-03: Create `AuthSessionContext` and populate it in `GuardedGate`
- [ ] TASK-04: Rewrite `useUuid` to read from context with localStorage fallback + mismatch telemetry
- [ ] TASK-05: Fix `find-booking` to issue token for `matchedOccupantId`
- [ ] TASK-06: Fix `digital-assistant` stale token — switch to `prime_session` cookie auth in `/api/assistant-query`
- [ ] TASK-07: Clean up stale `GuestSessionSnapshot` type + test mocks for `validateGuestToken` / `readGuestSession`
- [ ] TASK-08: Add unit tests for `useUuid` (context, localStorage fallback, null-both, mismatch event)
...
|---|---|---|---:|---:|---|---|---|
| TASK-01 | IMPLEMENT | Extend `/api/guest-session` GET to return `guestUuid` | 90% | S | Pending | - | TASK-02 |
| TASK-07 | IMPLEMENT | Clean up stale `GuestSessionSnapshot` type + test mocks | 85% | S | Pending | - | TASK-02 |
| TASK-05 | IMPLEMENT | Fix `find-booking` token issuance to use `matchedOccupantId` | 90% | S | Pending | - | - |
| TASK-09 | IMPLEMENT | Capture pre-release baseline for `/error` redirect rate | 85% | S | Pending | - | - |
| TASK-02 | IMPLEMENT | Update `validateGuestToken` return type + all 4 callers | 85% | M | Pending | TASK-01, TASK-07 | TASK-03 |
| TASK-06 | IMPLEMENT | Fix `digital-assistant` auth — switch `/api/assistant-query` to cookie auth | 80% | S | Pending | - | - |
| TASK-03 | IMPLEMENT | Create `AuthSessionContext` + populate in `GuardedGate` | 85% | M | Pending | TASK-02 | TASK-04 |
| TASK-04 | IMPLEMENT | Rewrite `useUuid` — context-first, localStorage fallback, mismatch telemetry | 85% | M | Pending | TASK-03 | TASK-08 |
| TASK-08 | IMPLEMENT | Add unit tests for `useUuid` (context, fallback, null-both, mismatch) | 80% | M | Pending | TASK-04 | - |
