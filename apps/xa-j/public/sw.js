/* XA offline service worker (runtime caching) */

const VERSION = new URL(self.location.href).searchParams.get("v") || "v1";
const PREFIX = "xa-offline";
...
const PRECACHE_ROUTES = ["/", "/search", "/access"];
...
function isSearchSync(url) {
  return url.pathname === "/api/search/sync";
}
...
self.addEventListener("activate", (event) => {
  event.waitUntil(
    (async () => {
      const keep = new Set([ASSETS_CACHE, PAGES_CACHE, API_CACHE, IMAGES_CACHE]);
...
self.addEventListener("fetch", (event) => {
  ...
  if (sameOrigin) {
    ...
    if (isSearchSync(url)) {
      event.respondWith(
        networkFirst(request, API_CACHE, { maxEntries: MAX_API_ENTRIES }),
      );
      return;
    }
  }
