---
name: lp-do-discovery-05-distribution-planning
description: Distribution planning for new startups (DISCOVERY-05). Elicits ≥2 launch channels with cost/effort estimates and ICP fit rationale. Produces a distribution-plan artifact ready for measurement planning. Upstream of lp-do-discovery-06-measurement-plan.
---

# lp-do-discovery-05-distribution-planning — Distribution Planning (DISCOVERY-05)
...
```
/lp-do-discovery-05-distribution-planning --business <BIZ>
```
...
This skill:
- Reads DISCOVERY-01 (ICP — who is buying) and DISCOVERY-03 (selected product — what is being sold)
...
- Writes the final DISCOVERY-05 artifact
...
- Only 1 channel (minimum 2 required for DISCOVERY GATE to pass)
...
> "Distribution plan recorded: `docs/business-os/strategy/<BIZ>/distribution-plan.user.md`. [N] channels identified; [channel name] primary. Ready for DISCOVERY-06 Measurement Plan."

---

## Integration

**Upstream (DISCOVERY-04):** Runs after `/lp-do-discovery-04-business-name-options --business <BIZ>` returns a naming shortlist.

**Downstream consumer:** `modules/discovery-intake-sync.md` reads `distribution-plan.user.md` as its fifth precursor (DISCOVERY-05). Section A channels inform intake Section C (Channel Packet). Section C priority order informs intake Section A (Execution posture).

**Downstream:** `/lp-do-discovery-06-measurement-plan --business <BIZ>` reads this artifact as input to determine which channels need measurement instrumentation.
