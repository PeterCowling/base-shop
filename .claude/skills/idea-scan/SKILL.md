...
- Creating cards directly (use `/idea-develop` for that)
...
2. Use /idea-develop to convert promising ideas to cards
3. Use /idea-advance to progress unblocked cards
...
- **After scan:** Use `/idea-develop` to convert scan-generated ideas to cards
- **Blocked cards:** Use scan to identify blockers, then `/idea-advance` when unblocked
- **Business plan updates:** Scan detects changes, use `/biz-update-plan` to sync plan with reality
