...
function ResetPasswordPage() {
    ...
    var tokenParam = (_a = searchParams.get("token")) !== null && _a !== void 0 ? _a : "";
    function handleSubmit(e) {
        return __awaiter(this, void 0, void 0, function () {
            var form, token, password, csrfToken, body, res;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        e.preventDefault();
                        form = e.currentTarget.elements;
                        token = form.namedItem("token").value;
                        password = form.namedItem("password").value;
                        csrfToken = (0, _shared_utils_1.getCsrfToken)();
                        body = { token: token, password: password };
                        return [4 /*yield*/, fetch("/api/account/reset/complete", {
                                method: "POST",
                                headers: {
    ...
    return (<form onSubmit={handleSubmit} className="space-y-2">
      ...
      <input name="password" type="password" placeholder="New password" pattern="^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d).{8,}$" title="Password must be at least 8 characters and include uppercase, lowercase, and number" className="border p-1"/>
