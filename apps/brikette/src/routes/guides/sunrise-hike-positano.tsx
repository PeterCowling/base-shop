...
function getEnglishTranslator(
  ...
  try {
    ...
    if (typeof fixed === "function") {
      return ((key: string, options?: Record<string, unknown>) =>
        fixed(key, options)) as GenericContentTranslator;
    }
  } catch {
    /* noop */
  }
  try {
    const fallback = (i18n as unknown as {
      __tGuidesFallback?: GenericContentTranslator;
    }).__tGuidesFallback;
...
function SunriseHikeExtras({ context }: { context: GuideSeoTemplateContext }): JSX.Element {
  ...
  }, [context, englishGuidesTranslator]);
...
if (!manifestEntry) {
  throw new Error("guide manifest entry missing for sunriseHike"); // i18n-exempt -- TECH-000 [ttl=2026-12-31]
}
