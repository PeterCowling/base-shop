 ...
 * When the allowlist is non-empty but INVENTORY_TRUST_PROXY_IP_HEADERS is disabled,
...
function checkIpCoherence(allowlisted: Set<string>): void {
  ...
  const trustProxy = process.env.INVENTORY_TRUST_PROXY_IP_HEADERS;
  if (trustProxy && trustProxy !== "0" && trustProxy !== "false") return;

  coherenceWarningEmitted = true;
  inventoryLog("warn", "security_coherence_mismatch", {
    message:
      "IP allowlist is configured but INVENTORY_TRUST_PROXY_IP_HEADERS is disabled. " +
...
export function isInventoryIpAllowed(headers: Headers): boolean {
  const allowlisted = parseAllowlistedIps(process.env.INVENTORY_ALLOWED_IPS);
...
export function inventoryAccessDeniedResponse(): NextResponse {
