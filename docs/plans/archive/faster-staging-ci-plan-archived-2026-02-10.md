...
## Fact-Find Reference
- Related brief: `docs/plans/faster-staging-ci-wf-fact-find.md`
- Key findings:
...
- **Documentation impact:**
  - Note shard count and rationale in `docs/plans/faster-staging-ci-wf-fact-find.md` (post-build update section).
- **Notes / references:**
  - Shard estimate source: `docs/plans/faster-staging-ci-wf-fact-find.md`.
...
- **Validation:**
  - `actionlint .github/workflows/reusable-app.yml .github/workflows/brikette.yml` — PASS
- **Documentation updated:** included in TASK-04 wf-fact-find update.
...
### TASK-04: Capture post-change telemetry and enforce speed acceptance checks
- **Type:** IMPLEMENT
- **Affects:** `docs/plans/faster-staging-ci-wf-fact-find.md`, `[readonly] scripts/src/ci/collect-workflow-metrics.ts`
...
- **Test contract:**
  ...
  - **Test location:** generated telemetry JSON and updated wf-fact-find section.
...
- **Planning validation:**
  ...
  - Unexpected findings: None.
- **Rollout / rollback:**
  - Rollout: publish telemetry snapshot in plan/wf-fact-find.
  - Rollback: N/A (measurement only).
- **Documentation impact:**
  - Update `docs/plans/faster-staging-ci-wf-fact-find.md` with post-change measured baseline.
- **Notes / references:**
...
- **Status:** Complete
- **Commit:** `9380974106`
- **Implementation notes:** added an explicit telemetry gate section to wf-fact-find with executable commands, target thresholds, and a current baseline snapshot; updated testing policy with Brikette shard/cache behavior.
...
- **Documentation updated:**
  - `docs/plans/faster-staging-ci-wf-fact-find.md`
...
- **Planning validation:**
  - Tests run: baseline suite durations observed from latest CI log in wf-fact-find.
  - Test stubs written: N/A (M effort).
...
- **Documentation impact:**
  - Update performance notes in wf-fact-find with achieved suite-level deltas.
- **Notes / references:**
  - Hotspot list sourced from `docs/plans/faster-staging-ci-wf-fact-find.md`.
...
## Decision Log
- 2026-02-07: Created new active plan from updated wf-fact-find and superseded archived implementation plan format.
