...
## Active tasks
- [x] TASK-01: Consolidate DateOfBirth — delete hand-written duplicate and redundant alias
- [x] TASK-02: Consolidate Activity — wire activitiesData.ts to schema; fix checkoutrow.ts divergence
- [x] TASK-03: Consolidate PayType — single definition in cityTaxData.ts; re-export from cityTaxDomain.ts
- [x] TASK-04: Typecheck verification gate
...
|---|---|---|---:|---:|---|---|---|
| TASK-01 | IMPLEMENT | Consolidate DateOfBirth | 90% | S | Complete (2026-03-08) | - | TASK-04 |
| TASK-02 | IMPLEMENT | Consolidate Activity | 90% | S | Complete (2026-03-08) | - | TASK-04 |
| TASK-03 | IMPLEMENT | Consolidate PayType | 90% | S | Complete (2026-03-08) | - | TASK-04 |
| TASK-04 | IMPLEMENT | Typecheck verification gate | 90% | S | Complete (2026-03-08) | TASK-01, TASK-02, TASK-03 | - |
...
- **Build evidence:** Deleted `occupantDetails.ts` and `dob.ts`; updated imports in `usePrepaymentData.ts`, `buildCheckinRows.ts`, `useCheckinsData.ts`; Wave 1 commit `8bc242019f`; typecheck+lint 0 errors.
- **Affects:**
...
- **Effort:** S
- **Status:** Complete (2026-03-08)
- **Build evidence:** Replaced hand-written `Activity` interface with `z.infer<typeof activitySchema>` in `activitiesData.ts`; removed diverged local `Activity` from `checkoutrow.ts`; import type/sort fixes applied; Wave 1 commit `8bc242019f`; typecheck+lint 0 errors.
- **Affects:**
...
- **Effort:** S
- **Status:** Complete (2026-03-08)
- **Build evidence:** `cityTaxData.ts` order standardised to `"CASH" | "CC"`; `cityTaxDomain.ts` now re-exports via `export type { PayType } from "../hooks/data/cityTaxData"`; Wave 1 commit `8bc242019f`; typecheck+lint 0 errors.
- **Affects:**
