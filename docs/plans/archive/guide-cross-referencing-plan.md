...
- Related brief: `docs/plans/guide-cross-referencing-lp-fact-find.md`
- Key findings:
...
**Decision:** Option A chosen because it reduces editor friction and treats cross-references as first-class content feature per lp-fact-find recommendation.
...
- **Acceptance:**
  ...
  - Default scope matches the lp-fact-find baseline (`--locale=en`); script supports `--locale=<lang>` to expand reporting beyond EN
...
- **Test plan:**
  ...
  - Verify: Counts match lp-fact-find numbers (135 with relatedGuides, 30 with %LINK:, 4 with maps)
- **Planning validation:**
...
- **Notes / references:**
  ...
  - Existing: `assistanceKeywords.json` has Google Maps URLs (lp-fact-find line 88)
