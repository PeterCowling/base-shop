...
export async function POST(req: NextRequest): Promise<Response> {
  try {
    ...
    try {
      const cached = await kvGet(`idem:${parsed.data.idempotencyKey}:depth`);
      if (cached) return NextResponse.json({ depthUrl: cached, metrics: { preprocessMs: 0 } });
    } catch {}
    const provider = getProvider();
    if (!provider.depth) return NextResponse.json({});
    const signedUrl = await signGetForProvider(imageUrl);
    const resp = await provider.depth.run(signedUrl);
    const ms = Date.now() - t0;
    try {
      const host = new URL(imageUrl).host;
      console.info("tryon.depth", { jobId: parsed.data.idempotencyKey, host, ms, ok: !resp.error });
    } catch {}
    if (resp.error) return NextResponse.json({ error: resp.error }, { status: 502 });
    const url = resp.result?.url;
    try { if (url) await kvPut(`idem:${parsed.data.idempotencyKey}:depth`, url, 86400); } catch {}
    return NextResponse.json({ depthUrl: url, metrics: { preprocessMs: resp.metrics?.preprocessMs ?? ms } });
  } catch (err: unknown) {
