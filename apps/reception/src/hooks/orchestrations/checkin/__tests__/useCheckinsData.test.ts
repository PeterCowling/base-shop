...
/* ------------------------------------------------------------------ */
vi.mock("../../../data/useBookingsData", () => ({ default: vi.fn() }));
vi.mock("../../../data/useGuestDetails", () => ({ default: vi.fn() }));
vi.mock("../../../data/useFinancialsRoom", () => ({ default: vi.fn() }));
vi.mock("../../../data/useCityTax", () => ({ default: vi.fn() }));
vi.mock("../../../data/useLoans", () => ({ default: vi.fn() }));
vi.mock("../../../data/useActivitiesData", () => ({ default: vi.fn() }));
vi.mock("../../../data/useCheckins", () => ({ useCheckins: vi.fn() }));
vi.mock("../../../data/useGuestByRoom", () => ({ default: vi.fn() }));
vi.mock("../../../data/useActivitiesByCodeData", () => ({ default: vi.fn() }));

/* ------------------------------------------------------------------ */
/*  Import mocked modules *after* registration so we can access       */
/*  the vi.fn instances. `vi.mocked` gives correctly‑typed handles.   */
/* ------------------------------------------------------------------ */
...
const useBookingsMock = vi.mocked(useBookingsData);
const useGuestDetailsMock = vi.mocked(useGuestDetails);
const useFinancialsRoomMock = vi.mocked(useFinancialsRoom);
const useCityTaxMock = vi.mocked(useCityTax);
const useLoansMock = vi.mocked(useLoans);
const useActivitiesMock = vi.mocked(useActivitiesData);
const useCheckinsMock = vi.mocked(useCheckinsHook);
const useGuestByRoomMock = vi.mocked(useGuestByRoom);
const useActivitiesByCodeMock = vi.mocked(useActivitiesByCodeData);
