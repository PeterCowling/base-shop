...
[MANUAL] Write initial EN content                             ← GAP
    ↓
/lp-guide-audit (AI: SEO audit + iterative fixes)           ✅ WORKS
    ↓
...
/create-guide <topic> (AI: scaffold + initial content + manifest entry)
    ↓
/lp-guide-audit (AI: SEO audit + iterative fixes)
    ↓
...
- Replacing `/lp-guide-audit` or `/guide-translate` (battle-tested, 100% success rate)
...
|-----------|-----------|----------|
| EN content SEO audit + iterative fixes | `/lp-guide-audit` | Production — score-gated, snapshot-safe |
| 17-locale parallel translation | `/guide-translate` | Production — 100% success rate with structure-first |
| Interactive workflow selection | `/lp-guide-improve` | Production — orchestrates audit + translate |
...
- Leaves guide in `"draft"` status, ready for `/lp-guide-audit`
...
- Optionally: auto-runs `/lp-guide-audit` if SEO score is below threshold
...
- `/lp-guide-improve` — `.claude/skills/lp-guide-improve/SKILL.md` (orchestrator)
- `/lp-guide-audit` — `.claude/skills/lp-guide-audit/SKILL.md` (EN audit + fixes)
