...
type FallbackFaq = { q: string; a: string[] };
...
function normaliseHowToSteps(input: unknown): HowToStep[] {
  return ensureArray<{ name?: unknown; text?: unknown }>(input)
    .map((item) => {
      ...
      const text = ensureStringArray(item?.text)
...
function buildArticleExtras(ctx: GuideSeoTemplateContext): JSX.Element {
  const baseGuidesTranslator =
    typeof ctx.translateGuides === "function"
      ? (ctx.translateGuides as TranslatorLike)
      : typeof ctx.translator === "function"
        ? ((key: string, options?: Record<string, unknown>) => ctx.translator?.(key, options))
        : undefined;
  const guidesTranslator = (key: string, options?: Record<string, unknown>) =>
    callTranslator(baseGuidesTranslator ?? (() => undefined), key, options ?? {});
  const fallbackTranslator = resolveGuidesFallbackTranslator(ctx.lang);

  const guidesT = (key: string, options?: Record<string, unknown>) => guidesTranslator(key, options);
      ...
      <ul>
        {twoDaysItems.map((option, index) => (
          <li key={`two-days-${index}`}>
            <Trans
              t={(baseGuidesTranslator ?? (() => "")) as unknown as TFunction}
...
function buildAfterArticle(ctx: GuideSeoTemplateContext): JSX.Element | null {
  try {
    const fallbackTranslator = resolveGuidesFallbackTranslator(ctx.lang);
    const labels = buildFallbackTocItems(fallbackTranslator);
...
function buildGuideFaqFallback(lang: string): NormalizedFaqEntry[] {
  try {
    const translator = wrapTranslator(
      i18n.getFixedT(lang, "guidesFallback") as TFunction<"guidesFallback">,
    );
    const entries = callTranslator(translator, `${GUIDE_KEY}.faqs`, { returnObjects: true });
...
function buildTocItems(ctx: GuideSeoTemplateContext) {
  if (ctx.hasLocalizedContent && Array.isArray(ctx.toc) && ctx.toc.length > 0) {
    return ctx.toc;
  }
  return buildFallbackTocItems(resolveGuidesFallbackTranslator(ctx.lang));
}
...
if (!manifestEntry) {
  throw new Error("guide manifest entry missing for itinerariesPillar"); // i18n-exempt -- TECH-000 [ttl=2026-12-31] Developer-only invariant surfaced in build logs
}
...
const { Component, clientLoader, meta: baseMeta } = defineGuideRoute(manifestEntry, {
  template: () => ({
    ...
    afterArticle: buildAfterArticle,
    buildTocItems,
    buildHowToSteps,
