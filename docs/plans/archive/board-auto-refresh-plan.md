...
- Related brief: `docs/plans/board-auto-refresh-lp-fact-find.md`
- Key findings:
...
- **Planning validation:** (M-effort)
  ...
  - Evidence collected: Authorization layer compatible (lp-fact-find verification), Repo lock compatible (lp-fact-find verification)
- **What would make this ≥90%:**
...
- **Planning validation:** (S-effort)
  ...
  - Test stubs written: N/A
  - Unexpected findings: None
  - Pattern reference: Toast system already integrated (lp-fact-find: ToastProvider exists)
- **What would make this ≥90%:**
...
- **Notes / references:**
  - Toast system: Already integrated (lp-fact-find: ToastProvider in layout)
...
- **Risk**: Authorization or lock mechanisms fail in worktree context
  - **Mitigation**: Already validated in lp-fact-find (authorize.ts:19-41, RepoLock.ts compatible)
