...
const mockUpload = uploadMedia;
const mockOrientation = useImageOrientationValidation;
beforeEach(() => {
    mockOrientation.mockReturnValue({ actual: "landscape", isValid: true });
    mockUpload.mockResolvedValue({ url: "/img.png", altText: "" });
});
it("updates progress during upload", async () => {
    const file = new File(["x"], "x.png", { type: "image/png" });
    const { result } = renderHook(() => useMediaUpload({ shop: "s", requiredOrientation: "landscape" }));
    act(() => {
        result.current.onFileChange({ target: { files: [file] } });
    });
