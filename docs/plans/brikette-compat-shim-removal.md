...
| Pages Router routes | 9 files | Static via `getStaticPaths` + compat shim |
| Files importing compat | 253 | See breakdown below |

### Import Breakdown by Category

| Category | Count | Action |
|----------|-------|--------|
| Route files (`routes/*.tsx`) | ~160 | Delete after App Router migration |
| Components (`components/*.tsx`) | ~25 | Migrate to `next/link` + `next/navigation` |
| Pages Router files (`pages/*.tsx`) | 9 | Delete after migration |
| Compat shim files (`compat/*.ts`) | 6 | Delete in Phase 4 |
| Other utilities | ~53 | Migrate or delete |

### RouterStateProvider/useLoaderData Usage (18 files)

Files using `useRouterState`, `useLoaderData`, or `useMatches` that will break when `RouterStateProvider` is removed:

| File | Usage |
|------|-------|
| `routes/rooms.tsx` | `useLoaderData` |
| `routes/terms.tsx` | `useLoaderData` |
| `routes/privacy-policy.tsx` | `useLoaderData` |
| `routes/rooms.$id.tsx` | `useLoaderData` |
| `routes/how-to-get-here.$slug.tsx` | `useLoaderData` |
| `routes/how-to-get-here.tsx` | `useLoaderData` |
| `routes/home.tsx` | `useLoaderData` |
| `routes/house-rules.tsx` | `useLoaderData` |
| `routes/guides/layout.tsx` | `useLoaderData`, `useMatches` |
| `routes/guides/draft.index.tsx` | `useLoaderData` |
| `routes/experiences/useExperiencesPageContent.ts` | `useLoaderData` |
| `routes/cookie-policy.tsx` | `useLoaderData` |
| `routes/book.tsx` | `useLoaderData` |
| `utils/safeUseLoaderData.ts` | Wrapper for `useLoaderData` |
| `test/components/deals-page.test.tsx` | Mocks `useLoaderData` |

**Action:** These files are all in `routes/` and will be deleted when routes are migrated. The test file needs updating.

## Active tasks

Tasks are ordered by dependency (later tasks depend on earlier ones completing).

- **BRK-COMPAT-01** Capture legacy URL inventory and commit fixtures. Depends on: —. DoD: `docs/plans/brikette-url-inventory.txt` and `apps/brikette/src/test/fixtures/legacy-urls.txt` checked in.
- **BRK-COMPAT-02** Create URL coverage matrix; choose catch-all replacement strategy. Depends on: 01. DoD: 100% legacy URLs accounted for (serve or redirect); strategy documented.
- **BRK-COMPAT-03** Migrate shared components off `react-router(-dom)` imports. Depends on: —. DoD: Component tests pass; no accidental server→client promotion.
- **BRK-COMPAT-04** Migrate remaining routes to App Router with `generateStaticParams()`; create `src/routing/routeInventory.ts` (required for URL coverage test). Depends on: 02, 03. DoD: All target routes render statically; URL parity holds; `routeInventory.ts` enumerates all App Router URLs.
- **BRK-COMPAT-05** Migrate `_app.tsx` / `_document.tsx` into `app/layout.tsx` + client provider(s). Depends on: 03. DoD: Providers, styles, theme init, metadata parity verified.
- **BRK-COMPAT-06** Implement legacy redirects via Cloudflare `_redirects` file (see note below). Depends on: 02. DoD: `/directions/:slug` redirects work on deployed static site.
- **BRK-COMPAT-06b** Replace root `index.tsx` SSR with static language gateway. Depends on: 02. DoD: `/` serves static HTML with client-side language detection, redirect, and hreflang alternates; no `getServerSideProps`.
- **BRK-COMPAT-07** Add targeted regression tests (links, language, URL coverage). Depends on: 01, 04. DoD: Tests validate coverage and don't depend on `src/compat/*`.
...
### Phase 0 (Discovery & Coverage)
- [ ] Legacy URL inventory captured (`docs/plans/brikette-url-inventory.txt`)
- [ ] URL coverage matrix complete (serve/redirect for every legacy URL)
- [ ] `/directions/:slug` redirect planned via Cloudflare `_redirects` (not middleware)

### Phase 1 (Components)
- [ ] All `"use client"` directives added where needed
- [ ] `useLocationCompat` works without hash (or hash handled separately)
- [ ] `useCurrentLanguage` works with App Router params
- [ ] Server-safe `getLanguageFromParams` available
- [ ] Component tests pass

### Phase 2 (Routes)
- [ ] URL coverage matrix verified (from 0.1b) - every legacy URL accounted for
- [ ] Catch-all strategy decided (Option A or B) and implemented
- [ ] `src/routing/routeInventory.ts` created and tested (required for URL coverage test)
- [ ] All routes have `generateStaticParams()` for dynamic segments
- [ ] Metadata uses cached/static data only (not async fetches)
- [ ] Book, apartment, draft routes migrated
- [ ] Root language gateway (`app/page.tsx`) implemented (BRK-COMPAT-06b)
- [ ] `/directions/[slug]` redirects configured in `public/_redirects` (Cloudflare)

### Phase 3 (Providers)
- [ ] Theme init script in root layout
- [ ] Site-wide metadata (og:site_name, twitter:site) in root layout
- [ ] i18n hydration in client layout
- [ ] RouterStateProvider removed (not needed)

### Phase 4 (Cleanup)
- [ ] URL inventory verified (no 404s)
- [ ] No `react-router` imports remain: `grep -r "from ['\"]react-router" apps/brikette/src/`
- [ ] Build produces fully static `out/` directory (HTML/JS/CSS only)
- [ ] `out/_redirects` file present with `/directions/:slug` rule
- [ ] All tests pass (including URL coverage test)
- [ ] E2E smoke test passes on staging (sample of URLs from inventory)
...
| Phase | Files Affected | Primary Work |
|-------|----------------|--------------|
| Phase 0 | 0 | Discovery scripts, documentation |
| Phase 1 | ~25 components | Add `"use client"`, change imports |
| Phase 2 | ~5 routes (Option B) or more (Option A) | Create new App Router pages |
| Phase 3 | 2 layouts, 1 provider | i18n provider, metadata, theme script |
| Phase 4 | ~170 deletions | Delete routes/, pages/, compat/, update config |

**Total:** ~200 files touched, ~170 deleted, ~30 modified

**Note on Phase 2 effort:** The "~5 routes" estimate assumes Option B (catch-all with route inventory). If Option A (explicit routes) is chosen after completing the URL coverage matrix, the number of new `page.tsx` files could be significantly higher depending on how many URLs the catch-alls currently serve. The Phase 0.1b coverage matrix will clarify this.
