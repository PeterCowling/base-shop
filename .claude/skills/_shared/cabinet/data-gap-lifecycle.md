...
For DGPs tagged `gate-unresolved`, the wf-fact-find should target the specific incomplete artifacts:
...
**Resolution path:** When the missing artifact data is gathered (via wf-fact-find or next sweep), the idea re-enters Stage 4 for Contrarian Gate re-evaluation. If the gate now returns PASS, the idea proceeds to Munger-Buffett verdict and onward through the pipeline.
...
**Output**: Investigation produces a **wf-fact-find stage doc** (not a build card). The wf-fact-find doc answers the blocking questions listed in the DGP's "Blocking Questions" section.
...
- Skipping the wf-fact-find stage of the feature workflow
...
DGP (decision-gap idea)
    |
    v
Investigation card (wf-fact-find task)
    |
    v
...
Agent completes wf-fact-find, produces stage doc with findings:
...
- **v1.2** (2026-02-10): Contrarian Gate integration: added Contrarian Gate → DGP path for UNRESOLVED ideas (presentable but gate-blocked), +15 VOI boost for gate DGPs, gate resolution wf-fact-find template, promotion path requires gate re-evaluation to PASS.
