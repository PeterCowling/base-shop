...
export default function useActivitiesMutations() {
   ...
   * Reminder/cancellation progression (2/3/4) now routes through MCP templates
   * and produces Gmail drafts for staff review (no auto-send).
   * TASK-05: Added code 27 (CANCELLED) for cancellation confirmation emails
   */
  const relevantCodes = useMemo<number[]>(
    () => [2, 3, 4, 21, 5, 6, 7, 8, 27],
    []
  );
