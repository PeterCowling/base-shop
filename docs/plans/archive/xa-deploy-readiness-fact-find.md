...
The XA member rewards storefront (`apps/xa`) has a CI workflow (`.github/workflows/xa.yml`) but **cannot deploy** due to multiple infrastructure gaps. This lp-do-fact-find compares against the working Brikette deployment and identifies exactly what must change.
