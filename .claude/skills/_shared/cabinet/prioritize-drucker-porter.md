...
**Output format:** Appends Decision Log block to dossier with priority, potential-impact, traction objective (under `grow-business`), strategic fit, plan target, stance weight, and rationale. For P1/P2 under `grow-business`, also outputs a Rigor Pack that pre-populates the wf-fact-find stage doc.
...
Each P1/P2 must include all of the following. Target: one page total. This content pre-populates the wf-fact-find stage doc to accelerate `/wf-fact-find`; it does not replace `/wf-fact-find`.
...
1. **Flag as critical finding** in sweep report:
   ...
   Recommendation: Create business plan using `/wf-plan` or manual documentation.
   ```
...
**Rigor Pack output (pre-populates wf-fact-find stage doc):**
...
**Key insight:** Traction-mode P1 includes a complete Rigor Pack that can directly pre-populate the wf-fact-find stage doc and make `/wf-fact-find` faster.
