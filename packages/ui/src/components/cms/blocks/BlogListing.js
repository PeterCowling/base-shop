...
export default function BlogListing({ posts = [] }) {
    if (!posts.length)
        return null;
    return (_jsx("section", { className: "space-y-4", children: posts.map((p) => (_jsxs("article", { className: "space-y-1", children: [p.url ? (_jsx("h3", { className: "text-lg font-semibold", children: _jsx(Link, { href: p.url, children: p.title }) })) : (_jsx("h3", { className: "text-lg font-semibold", children: p.title })), p.excerpt && (_jsx("p", { className: "text-muted", "data-token": "--color-muted", children: p.excerpt })), p.shopUrl && (_jsx("p", { children: _jsx(Link, { href: p.shopUrl, className: "text-primary underline", children: "Shop the story" }) }))] }, p.title))) }));
}
