...
const detectTestRuntime =
  Boolean((globalThis as Record<string, unknown> | undefined)?.__VITEST_ENV__) ||
...
export interface DocumentShim {
  ...
  createElement: (tagName: string) => ShimNode & Record<string, unknown>;
  createElementNS: (ns: string, tagName: string) => ShimNode & Record<string, unknown>;
