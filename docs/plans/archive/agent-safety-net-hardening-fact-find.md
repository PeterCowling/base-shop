...
See **`docs/git-safety.md` § Command Policy Matrix** — that is the canonical, stable source of truth for all deny/ask/allow decisions. The lp-do-fact-find brief does not duplicate the matrix; all enforcement points reference the canonical doc.
...
- Recommended next step: Proceed to `/lp-do-plan agent-safety-net-hardening`
