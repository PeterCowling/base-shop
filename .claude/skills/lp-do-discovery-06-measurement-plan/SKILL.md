---
name: lp-do-discovery-06-measurement-plan
description: Measurement planning for new startups (DISCOVERY-06). Elicits tracking setup, ≥2 key metrics, success thresholds, and data collection feasibility. Produces a measurement-plan artifact ready for operator evidence capture. Upstream of lp-do-discovery-07-our-stance.
---

# lp-do-discovery-06-measurement-plan — Measurement Plan (DISCOVERY-06)
...
```
/lp-do-discovery-06-measurement-plan --business <BIZ>
```
...
This skill:
- Reads DISCOVERY-03 (selected product), DISCOVERY-05 (channels identified)
...
- Writes the final DISCOVERY-06 artifact
...
- Fewer than 2 metrics in Section B (minimum 2 required for DISCOVERY GATE to pass)
...
> "Measurement plan recorded: `docs/business-os/strategy/<BIZ>/measurement-plan.user.md`. [N] metrics defined; tracking via [method]. Ready for DISCOVERY-07 Our Stance."

---

## Integration

**Upstream (DISCOVERY-05):** Runs after `/lp-do-discovery-05-distribution-planning --business <BIZ>` produces distribution-plan.user.md.

**Downstream consumer:** `modules/discovery-intake-sync.md` reads `measurement-plan.user.md` as its sixth precursor (DISCOVERY-06). Section B metrics inform intake Section F (Missing-Data Checklist — any metric with feasibility gap). Section A tracking setup informs intake Section A (Execution posture).

**Downstream:** `/lp-do-discovery-07-our-stance --business <BIZ>` runs after this artifact is active. DISCOVERY GATE checks ≥2 metrics and a named tracking method before allowing DISCOVERY→S1 advance.
