...
3. Documents findings in lp-do-fact-find brief: `docs/plans/{feature-slug}-lp-do-fact-find.md`
...
- Agent lp-do-fact-finds to understand current state
...
**Workflow:**
1. Reads lp-do-fact-find brief (if exists)
...
Generated by Claude Code /idea-scan skill.
...
export async function GET(req: Request) {
  const stream = new ReadableStream({
    async start(controller) {
      // Start agent task
      const task = await startAgentTask("idea-develop", { ideaId: "BRIK-OPP-0003" });
...
function PlanApprovalCard({ plan }: { plan: AgentGeneratedPlan }) {
  ...
  const handleReject = async () => {
    ...
    setStatus("rejected");
    // Agent lp-do-replans with feedback
  };
