...
- Enumerate test files first: `rg --files -g "*.{test,spec}.{ts,tsx,js,jsx,mjs,cjs}"`
- Run each file individually with `--maxWorkers=1` (or `--runInBand`), stop on first failure, fix, then resume.
