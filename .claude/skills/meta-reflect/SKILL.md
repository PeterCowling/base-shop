...
This skill is a post-step that closes the loop after `/wf-fact-find`, `/wf-plan`, `/wf-replan`, and `/wf-build`.
...
**What session/workstream are we reflecting on?** (e.g., "the feature build we just completed", "the wf-replan for TASK-07", "the briefing request")
...
- A task was forced into `/wf-replan` due to <80% confidence.
...
**Quantitative cost:** minutes lost, number of retries, number of CI failures, number of wf-replans, number of times user corrected behavior
...
- If the issue was unclear intent / insufficient initial user context → update `/wf-fact-find` intake gates (L1).
- If the issue was task confidence inflation or missing evidence → update `/wf-plan` confidence rubric / evidence requirements (L1).
- If the issue was mid-build surprises → update `/wf-build` stopping conditions and preflight/baseline checks (L1).
- If the issue was tasks under-specified (missing acceptance/test/rollout) → update `/wf-replan` plan-repair requirements (L1).
