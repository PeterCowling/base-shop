...
| CAP-07 — Measurement and inference | Measure stage docs (MEASURE-01 + MEASURE-02) + rendered sales-funnel audit + denominator validity check in weekly decision memo | Operator + `GATE-SELL-ACT-01` measurement requirements | `GATE-SELL-ACT-01` (Hard; pre-SELL-08 paid spend activation); S10 (weekly denominator validity gate) | Measure stage doc at Active status with ≥1 confirmed conversion event; latest rendered sales-funnel audit is Active, covers `mobile, fullscreen`, and passes with zero High/Critical blockers; weekly memo denominator section: each selected KPI has denominator ≥ applicable threshold (global or profile override); uncertainty bound is explicit | `.claude/skills/lp-channels/SKILL.md`, `.claude/skills/lp-sell-funnel-audit/SKILL.md`, `docs/business-os/startup-loop/schemas/bottleneck-diagnosis-schema.md`, scale/kill decision | Partial — SELL-08 now enforces both measurement and rendered-funnel proof before paid channels, but weekly denominator gate and uncertainty bound are not yet explicit pass/fail checks |
